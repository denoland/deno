...
`desc1` was granted so the new status is returned and execution will continue if
`--allow-read=/foo` was specified on the CLI. The request for `desc2` was denied
so the permission state is downgraded from "prompt" to "denied".

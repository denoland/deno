...
// === promote-to-release-windows job ===
...
const winInstallDeno = step.dependsOn(winClone)({
  name: "Install deno",
  uses: "denoland/setup-deno@v2",
  with: { "deno-version": "v2.x" },
});

const winDownloadBinaries = step.dependsOn(winInstallDeno)({
...
const winCodeSign = step.dependsOn(winAuthAzure)({
...
const winVerifySignature = step.dependsOn(winCodeSign)({
...
const winCreateArchives = step.dependsOn(winVerifySignature)({
...
const winUploadArchives = step.dependsOn(winCreateArchives)({
...
const windowsJob = job("promote-to-release-windows", {
  ...
  steps: [winUploadArchives],
});

// === promote-to-release job ===
...
const macAuthGcloud = step.dependsOn(macClone)({
...
const macSetupGcloud = step.dependsOn(macAuthGcloud)({
  name: "Setup gcloud",
  uses: "google-github-actions/setup-gcloud@v3",
  with: { project_id: "denoland" },
});

const macInstallDeno = step.dependsOn(macSetupGcloud)({
  name: "Install deno",
  uses: "denoland/setup-deno@v2",
  with: { "deno-version": "v2.x" },
});

const macInstallRcodesign = step.dependsOn(macInstallDeno)({
...
const macPromote = step.dependsOn(macInstallRcodesign)({
...
const macDownloadWindows = step.dependsOn(macPromote)({
  name: "Download Windows binaries",
...
const macCreateVersionFile = step.dependsOn(macDownloadWindows)({
...
const macUploadArchives = step.dependsOn(macCreateVersionFile)({
...
const workflow = createWorkflow({
  ...
  jobs: [
    windowsJob,
    {
      ...
      if: isDenoland,
      needs: [windowsJob],
      steps: [macUploadArchives],
    },
  ],
});

const header =
  "# GENERATED BY ./promote_to_release.generate.ts -- DO NOT DIRECTLY EDIT";

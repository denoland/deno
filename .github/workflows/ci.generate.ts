  ...
  const setupS3Step = step(
    {
      ...
      run: [
        'echo "AWS_ACCESS_KEY_ID=${{ vars.S3_ACCESS_KEY_ID }}" >> $GITHUB_ENV',
        'echo "AWS_SECRET_ACCESS_KEY=${{ secrets.S3_SECRET_ACCESS_KEY }}" >> $GITHUB_ENV',
        'echo "AWS_ENDPOINT_URL_S3=${{ vars.S3_ENDPOINT }}" >> $GITHUB_ENV',
        'echo "AWS_DEFAULT_REGION=${{vars.S3_REGION }}" >> $GITHUB_ENV',
      ],
    },
  );
  return {

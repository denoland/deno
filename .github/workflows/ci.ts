    ...
    jobIdForJob("build"),
    {
      ...
      steps: (() => {
        ...
        const preRelease = step(
          ...
          {
            name: "Generate delta patches (linux)",
            if: isLinux.and(isDenoland).and(isTag),
            run: [
              `TARGET="${buildItem.arch}-unknown-linux-gnu"`,
              `NEW_VERSION=\${GITHUB_REF#refs/tags/v}`,
              'BSDIFF=./target/release/bsdiff_helper',
              'EXE=target/release/deno',
              // Fetch the last 3 stable versions from GitHub releases API
              'VERSIONS=$(curl -sf "https://api.github.com/repos/denoland/deno/releases?per_page=4" | grep \'"tag_name":\' | sed \'s/.*"v\\(.*\\)".*/\\1/\' | head -3)',
              'for PREV_VERSION in $VERSIONS; do',
              '  echo "Generating delta from $PREV_VERSION to $NEW_VERSION for $TARGET"',
              '  curl -fSL -o "prev-${PREV_VERSION}.zip" "https://github.com/denoland/deno/releases/download/v${PREV_VERSION}/deno-${TARGET}.zip" || { echo "Release v${PREV_VERSION} not found for ${TARGET}, skipping"; continue; }',
              '  rm -rf prev && mkdir prev',
              '  unzip -o "prev-${PREV_VERSION}.zip" -d prev/',
              '  $BSDIFF prev/deno "$EXE" "target/release/deno-${TARGET}.from-${PREV_VERSION}.bsdiff"',
              '  cd target/release && shasum -a 256 "deno-${TARGET}.from-${PREV_VERSION}.bsdiff" > "deno-${TARGET}.from-${PREV_VERSION}.bsdiff.sha256sum" && cd ../..',
              '  echo "  -> $(ls -lh "target/release/deno-${TARGET}.from-${PREV_VERSION}.bsdiff" | awk \'{print $5}\')"',
              'done',
            ],
          },
          {
            name: "Generate delta patches (mac)",
          ...
          {
            name: "Generate delta patches (windows)",
            if: isWindows.and(isDenoland).and(isTag),
            shell: "pwsh",
            run: [
              `$Target = "${buildItem.arch}-pc-windows-msvc"`,
              '$NewVersion = $env:GITHUB_REF -replace "refs/tags/v", ""',
              '$Bsdiff = ".\\target\\release\\bsdiff_helper.exe"',
              '$Exe = "target\\release\\deno.exe"',
              // Fetch last 3 versions
              '$releases = (Invoke-RestMethod "https://api.github.com/repos/denoland/deno/releases?per_page=4") | Select-Object -First 3',
              'foreach ($rel in $releases) {',
              '  $PrevVersion = $rel.tag_name -replace "^v", ""',
              '  Write-Host "Generating delta from $PrevVersion to $NewVersion for $Target"',
              '  $url = "https://github.com/denoland/deno/releases/download/v$PrevVersion/deno-$Target.zip"',
              '  try { Invoke-WebRequest -Uri $url -OutFile "prev-$PrevVersion.zip" } catch { Write-Host "Release v$PrevVersion not found for $Target, skipping"; continue }',
              '  Remove-Item -Recurse -Force prev -ErrorAction SilentlyContinue; New-Item -ItemType Directory -Path prev | Out-Null',
              '  Expand-Archive -Force -Path "prev-$PrevVersion.zip" -DestinationPath prev',
              '  & $Bsdiff "prev\\deno.exe" $Exe "target\\release\\deno-$Target.from-$PrevVersion.bsdiff"',
              ...
              '  Write-Host "  -> $((Get-Item "target\\release\\deno-$Target.from-$PrevVersion.bsdiff").Length) bytes"',
              '}',
            ],
          },
          step({

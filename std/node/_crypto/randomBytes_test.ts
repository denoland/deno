...
Deno.test('randomBytes sync works correctly', function() {
  ...
  assertEquals(randomBytes(17 + MAX_RANDOM_VALUES).length, 17 + MAX_RANDOM_VALUES, 'len: ' + 17 + MAX_RANDOM_VALUES)
  assertEquals(randomBytes(MAX_RANDOM_VALUES * 100).length, MAX_RANDOM_VALUES * 100, 'len: ' + MAX_RANDOM_VALUES * 100)
...
Deno.test('randomBytes async works correctly', function () {
  ...
  randomBytes(17 + MAX_RANDOM_VALUES, function (err, resp) {
    assert(!err);
    assertEquals(resp?.length, 17 + MAX_RANDOM_VALUES, 'len: ' + 17 + MAX_RANDOM_VALUES)
  })
  randomBytes(MAX_RANDOM_VALUES * 100, function (err, resp) {
    assert(!err);
    assertEquals(resp?.length, MAX_RANDOM_VALUES * 100, 'len: ' + MAX_RANDOM_VALUES * 100)
  })
  assertThrows(() => randomBytes(MAX_SIZE + 1, function (err) {
    //Shouldn't throw async
    assert(!err);
  }));
  assertThrows(() => randomBytes(-1, function (err) {
    //Shouldn't throw async
    assert(!err);
  }));
})
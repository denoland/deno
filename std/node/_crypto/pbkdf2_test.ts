...
const fixtures: Array<Pbkdf2Fixture> = [
  {
    "key": "password",
    "salt": "salt",
    "iterations": 1,
  ...
  },
  {
    ...
    "dkLen": 64,
    "results": {
      "md5": "f31afb6d931392daa5e3130f47f9a9b6e8e72029d8350b9fb27a9e0e00b9d9915a5f18928639ca8bbc3d1c1cb66d4f27b9dfe39156774c6798b42adc57ed253f",
  ...
  },
  {
    ...
    "dkLen": 64,
    "results": {
      "md5": "042407b552be345ad6eee2cf2f7ed01dd9662d8f0c6950eaec7124aa0c82279ed0b7e2a854d0f29ec82ddcabe9760368e5821af8745d74846ccbd17afbfe5ff0",
  ...
  },
  {
    ...
    "dkLen": 40,
    "results": {
      "md5": "8d5d0aad94d14420429fbc7e5b087d7a5527e65dfd0d486a310e8a7b6ff5a21bed000b118b2c26a6",
  ...
  },
  {
    ...
    "salt": "6d6e656d6f6e6963e383a1e383bce38388e383abe382abe38299e3838fe38299e382a6e38299e382a1e381afe3829ae381afe38299e3818fe38299e3829de38299e381a1e381a1e38299e58d81e4babae58d81e889b2",
    "iterations": 2048,
    "dkLen": 64,
    "results": {
      "md5": "029d0e4484f56d9cf7ab7ca972c8991aeb2be275cba9683db4143e9b72f67d49551ec4c70ca6d051538fc7a86b8568d08244fdea24ba826b7927babac4f62cf2",
  ...
  },
  {
    ...
    "dkLen": 100,
    "results": {
      "md5": "f31afb6d931392daa5e3130f47f9a9b6e8e72029d8350b9fb27a9e0e00b9d9915a5f18928639ca8bbc3d1c1cb66d4f27b9dfe39156774c6798b42adc57ed253f44fc731edccf067904ce2e317b9ef45767add4dfe53f8c190dac43d90cda5e66e627d4f2",
...
Deno.test("pbkdf2 hashes data correctly", () => {
  fixtures.forEach(({
    ...
    for(const algorithm in results){
      pbkdf2(key, salt, iterations, dkLen, algorithm as Algorithms, (err, res) => {
        assert(!err);
        assertEquals(
          res?.toString('hex'),
...
Deno.test("pbkdf2Sync hashes data correctly", () => {
  fixtures.forEach(({
    ...
    for(const algorithm in results){
      assertEquals(
        pbkdf2Sync(key, salt, iterations, dkLen, algorithm as Algorithms).toString('hex'),
        results[algorithm as Algorithms],
      );
    }
  });

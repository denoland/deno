...
export async function runTestModules({
  ...
  // Create temporary module which statically imports all matched modules.
  const testFilePath = join(cwd(), ".deno.test.js");
  const testFileData = new TextEncoder().encode(renderTestFile(testModules));
  await Deno.writeFile(testFilePath, testFileData);

  // Import the temporary module and then delete it.
  try {

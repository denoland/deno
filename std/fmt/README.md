...
- The behaviour of `%v` verb. In Golang, this is a shortcut verb to "print the
...
- The `%j` verb. This is an extension particular to this implementation. Currently
...
- `<` verb. This is an extension that assumes the argument is an array and will
  format each element according to the array format (surrounded by [ ] and separated by
  commas) (`<` Mnemonic: pull each element out of array).
...
- Currently contains little in the way of checking for correctness. Conceivably,
  there will be a 'strict' form, e.g. that ensures only Numerical arguments are
  passed to %f flags.

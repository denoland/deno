...
- If you don't provide `opt.header`, `opt.parse`, and `opt.columns`, it returns
  `string[][]`.
...
- **`header: boolean;`**: If you provide `header: true` and `columns`, the first
  line will be skipped. If you provide `header: true` but not `columns`, the
  first line will be skipped and used as header definitions.

...
Deno.test(function ensureSymlinkIfItNotExist(): Promise<void> {
...
Deno.test(function ensureSymlinkSyncIfItExist(): Promise<void> {
...
Deno.test(function ensureSymlinkSyncDirectoryIfItExist(): Promise<void> {
  const testDir = path.join(testdataDir, "link_file_origin_3");
  const linkDir = path.join(testdataDir, "link_file_link_3");
  const testFile = path.join(testDir, "test.txt");

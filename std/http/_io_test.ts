...
Deno.test("bodyReader", async () => {
  ...
  const r = bodyReader(
    text.length,
    new BufReader(new Deno.Buffer(encoder.encode(text))),
  );
  assertEquals(decoder.decode(await Deno.readAll(r)), text);
});
...
Deno.test("chunkedBodyReader", async () => {
  ...
  const r = chunkedBodyReader(
    h,
    new BufReader(new Deno.Buffer(encoder.encode(body))),
  );
...
Deno.test("chunkedBodyReader with trailers", async () => {
  ...
  const act = decoder.decode(await Deno.readAll(r));
  const exp = "aaabbbbbcccccccccccdddddddddddddddddddddd";
...
Deno.test("readTrailers", async () => {
  ...
  await readTrailers(
    h,
    new BufReader(new Deno.Buffer(encoder.encode(trailer))),
  );
  assertEquals(h.has("trailer"), false);
  assertEquals(h.get("deno"), "land");
  assertEquals(h.get("node"), "js");

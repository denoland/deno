    ...
    let files = reports
      ...
      .filter_map(|(report, _)| {
        ...
        let branches = report
          ...
          .iter()
          .filter_map(|b| {
            Some(lsp_custom::TestCoverageBranch {
              line: u32::try_from(b.line_index).ok()?,
              branch: u32::try_from(b.branch_number).ok()?,
              count: u32::try_from(b.taken.unwrap_or(0).max(0)).ok()?,
            })
          })
          .collect::<Vec<_>>();

        if lines.is_empty() && branches.is_empty() {
          None
        } else {

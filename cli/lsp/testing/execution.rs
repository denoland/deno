...
pub struct TestRun {
  ...
  /// `--env-file` paths inherited from the `test` task in deno.json.
...
impl TestRun {
  ...
  pub async fn exec(
    ...
    // When coverage mode is active: collect V8 coverage data, convert it to
    // per-file line coverage, publish it through the diagnostics server, then
    // clean up the temporary directory.
    if self.kind == lsp_custom::TestRunKind::Coverage {
      if let Some(coverage_dir) = self.coverage_dir() {
        match diagnostics::collect_coverage_from_dir(&coverage_dir) {
          Ok(files) => {
            diagnostics::update_coverage(files);
            client.refresh_diagnostics();
          }
          Err(err) => {
  ...
  fn get_args(&self) -> Vec<Cow<'_, str>> {
    ...
    // Inherit `--env-file` paths from the `test` task in deno.json.
...
mod tests {
  ...
  fn test_char_offset_to_line_start_of_file() {
    let src = "hello\nworld\nfoo";
    assert_eq!(diagnostics::char_offset_to_line(src, 0), 1);
    assert_eq!(diagnostics::char_offset_to_line(src, 4), 1);
  }

  #[test]
  fn test_char_offset_to_line_second_line() {
    let src = "hello\nworld\nfoo";
    assert_eq!(diagnostics::char_offset_to_line(src, 6), 2);
    assert_eq!(diagnostics::char_offset_to_line(src, 10), 2);
  }

  #[test]
  fn test_char_offset_to_line_empty_source() {
    assert_eq!(diagnostics::char_offset_to_line("", 0), 1);
  }

  #[test]
  fn test_char_offset_to_line_beyond_end() {
    assert_eq!(diagnostics::char_offset_to_line("ab", 999), 1);
  }

  #[test]
  fn test_collect_coverage_nonexistent_dir() {
    let result =
      diagnostics::collect_coverage_from_dir(Path::new("/nonexistent/deno/lsp/coverage"))
        .unwrap();
  ...
  fn test_collect_coverage_empty_dir() {
    ...
    let result = diagnostics::collect_coverage_from_dir(dir.path()).unwrap();
    assert!(result.is_empty());
  }
  ...
  fn test_collect_coverage_skips_data_urls() {
    let dir = tempfile::tempdir().unwrap();
    let json = r#"{"scriptId":"1","url":"data:text/javascript,test","functions":[]}"#;
  ...
  #[test]
  fn test_collect_coverage_computes_percent() {
    let dir = tempfile::tempdir().unwrap();
    let json = r#"{
      "scriptId": "42",

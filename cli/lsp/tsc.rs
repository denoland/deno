...
impl TsServer {
  pub fn new(performance: Arc<Performance>, cache: Arc<HttpCache>) -> Self {
...
mod tests {
  ...
  fn setup(
    ...
    let cache = Arc::new(HttpCache::new_global(location.clone()));
  ...
  fn test_modify_sources() {
    ...
    let cache = Arc::new(HttpCache::new_global(location));
    let specifier_dep =

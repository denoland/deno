...
impl DiagnosticSource {
  pub fn as_lsp_source(&self) -> &'static str {
    match self {
      ...
      Self::Lint => "deno-lint",
      Self::Ts => "deno-ts",
      Self::Coverage => "deno-coverage",
    }
  }
}

// -----------------------------------------------------------------------------
// V8 coverage parsing and per-file line coverage state
// -----------------------------------------------------------------------------
...
/// Line-based coverage for a single source file.
#[derive(Debug, Clone)]
...
static COVERAGE_STATE: LazyLock<RwLock<HashMap<ModuleSpecifier, FileCoverage>>> =
  LazyLock::new(|| RwLock::new(HashMap::new()));

/// Replaces the in-memory coverage state with the result of a test run.
pub fn update_coverage(files: Vec<FileCoverage>) {
  let mut state = COVERAGE_STATE.write();
  state.clear();
  for file in files {
    state.insert(file.specifier.clone(), file);
  }
}

/// Converts a 0-based character offset into a 1-based line number.
pub fn char_offset_to_line(src: &str, offset: usize) -> u32 {
  let offset = offset.min(src.len());
  src[..offset].chars().filter(|c| *c == '\n').count() as u32 + 1
}

/// Reads all V8 ScriptCoverage JSON files in `dir` and returns line-based
/// coverage for each local source file. Non-file URLs, ext: URLs, data: URLs,
/// anonymous scripts, and the test runner bootstrap are filtered out.
pub fn collect_coverage_from_dir(
  dir: &Path,
  ...
  for entry in entries.flatten() {
    ...
    let coverage: ScriptCoverage = match serde_json::from_str(&text) {
      Ok(c) => c,
      Err(_) => continue,
    };

    if should_skip_coverage_url(&coverage.url) {
      continue;
    }
    let specifier = match ModuleSpecifier::parse(&coverage.url) {
      Ok(s) => s,
      Err(_) => continue,
    };
    ...
    let mut line_status: HashMap<u32, bool> = HashMap::new();
    for func in &coverage.functions {
      for range in &func.ranges {
        let start_line = char_offset_to_line(&source, range.start_offset);
        let end_line = char_offset_to_line(&source, range.end_offset);
        for line in start_line..=end_line {
          if range.count > 0 {
            line_status.insert(line, true);
          } else {
            line_status.entry(line).or_insert(false);
          }
        }
      }
    }

    let covered_lines: Vec<u32> = line_status
      .iter()
      .filter(|(_, covered)| **covered)
      .map(|(line, _)| *line)
      .collect();
    let uncovered_lines: Vec<u32> = line_status
      .iter()
      .filter(|(_, covered)| !**covered)
      .map(|(line, _)| *line)
      .collect();
    let total = line_status.len();
...
fn should_skip_coverage_url(url: &str) -> bool {
...
fn coverage_line_range(text: &str, line_idx: u32) -> lsp::Range {
...
fn generate_coverage_diagnostics(
  module: &DocumentModule,
) -> Vec<lsp::Diagnostic> {
  let state = COVERAGE_STATE.read();
  let Some(coverage) = state.get(&module.specifier) else {
    return Vec::new();
  };

  let text = module.text.to_string();
  coverage
    .uncovered_lines
    .iter()
    .map(|line| lsp::Diagnostic {
      range: coverage_line_range(&text, *line),
      severity: Some(lsp::DiagnosticSeverity::HINT),
      code: Some(lsp::NumberOrString::String("uncovered".to_string())),
      code_description: None,
      source: Some(DiagnosticSource::Coverage.as_lsp_source().to_string()),
...
pub async fn generate_module_diagnostics(
  ...
  diagnostics.extend(generate_coverage_diagnostics(module));

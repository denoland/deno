...
pub fn import_map_lookup(
  ...
  import_map_lookup_inner(import_map, specifier, referrer)
}
  ...
  for entry in import_map.entries_for_referrer(referrer) {
    if let Some(address) = entry.value {
      ...
      if referrer.as_str().starts_with(address_str) {
        // Only skip if the key is a passthrough mapping, i.e. the address path
        // ends with the key preceded by '/' (e.g. ".../src/" ends with

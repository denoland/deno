...
async fn sync_resolution_with_fs(
  ...
  if !packages_with_scripts_not_run.is_empty() {
    ...
    This may cause the packages to not work correctly. To run them, use the `--allow-scripts` flag with `deno install --entrypoint` or `deno install`
    (e.g. `deno install --entrypoint --allow-scripts=pkg1,pkg2 <entrypoint>` or `deno install --allow-scripts=pkg1,pkg2`):\n      {packages}", crate::colors::yellow("Warning"));

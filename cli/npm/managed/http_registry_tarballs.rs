...
/// Loads and coordinates package tarballs being loaded from
/// the npm registry.
///
/// This is shared amongst all the workers.
#[derive(Debug)]
pub struct HttpNpmRegistryTarballApi {
  cache: Arc<NpmCache>,
  fs: Arc<dyn FileSystem>,
  npmrc: Arc<ResolvedNpmRc>,
  progress_bar: ProgressBar,
  memory_cache: Mutex<HashMap<PackageNv, CacheItem>>,
}

impl HttpNpmRegistryTarballApi {
  pub fn new(
    cache: Arc<NpmCache>,
    fs: Arc<dyn FileSystem>,
    npmrc: Arc<ResolvedNpmRc>,
  ...
  async fn ensure_package_inner(
    ...
    let (created, cache_item) = {
      ...
      if let Some(cache_item) = mem_cache.get(package_nv) {
        ...
        let future =
          self.create_setup_future(package_nv, dist, http_client_for_runtime);
    ...
    match cache_item {
      CacheItem::Cached => Ok(()),
      CacheItem::Errored(err) => Err(anyhow!("{}", err)),
  ...
  fn create_setup_future(
    ...
    deno_core::unsync::spawn(async move {
      ...
      if should_use_cache && package_folder_exists {
        return Ok(());
      } else if self.cache.cache_setting() == &CacheSetting::Only {

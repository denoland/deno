...
pub(crate) fn cat(deno_exe: &PathBuf, megs: usize) -> Result<Value> {
  let size = megs * MB;
  let shell_cmd = format!("{} run --allow-read cli/tests/cat.ts /dev/zero | head -c {}", deno_exe.to_str().unwrap(), size);
  println!("{}", shell_cmd);
  ...
  Ok(Value::Number(Number::from_f64((end - start).as_secs_f64()).unwrap()))
}
...
pub(crate) fn tcp(deno_exe: &PathBuf, megs: usize) -> Result<Value> {
  ...
  let nc_command = if cfg!(target_os = "linux") { "nc -N" } else { "nc" };

  let shell_cmd = format!("head -c {} /dev/zero | {} {}", size, nc_command, CLIENT_ADDR);
  println!("{}", shell_cmd);
  let cmd = &["sh", "-c", &shell_cmd];

  // Run deno echo server in the background.
  let mut echo_server = Command::new(deno_exe.to_str().unwrap()).args(&["run", "--allow-net", "cli/tests/echo_server.ts", SERVER_ADDR]).spawn()?;

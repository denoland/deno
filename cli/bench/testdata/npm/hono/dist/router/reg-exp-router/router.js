...
class RegExpRouter {
  ...
  match(method, path) {
    const [primaryMatchers, secondaryMatchers, hasAmbiguous] = this
      .buildAllMatchers();
    this.match = hasAmbiguous
      ...
      : (method, path) => {
        let matcher = primaryMatchers[method] ||
          primaryMatchers[router_1.METHOD_NAME_ALL];
        let match = path.match(matcher[0]);
        if (!match) {
          const matchers = secondaryMatchers[method] ||
  ...
  buildAllMatchers() {
    ...
    this.routeData.routes.sort(({ hint: a }, { hint: b }) => {
      ...
      for (
        let i = 0,
          len = Math.min(a.paramIndexList.length, b.paramIndexList.length) + 1;
        i < len;
        i++
      ) {
    ...
    this.routeData.methods.forEach((method) => {
      let _hasAmbiguous;
      [primaryMatchers[method], secondaryMatchers[method], _hasAmbiguous] = this
        .buildMatcher(method);
      hasAmbiguous = hasAmbiguous || _hasAmbiguous;
    });
    primaryMatchers[router_1.METHOD_NAME_ALL] ||
      (primaryMatchers[router_1.METHOD_NAME_ALL] = nullMatcher);
    secondaryMatchers[router_1.METHOD_NAME_ALL] ||
      (secondaryMatchers[router_1.METHOD_NAME_ALL] = []);
  ...
  buildMatcher(method) {
    ...
    // @ts-ignore
    const routes = this.routeData.routes.filter(({ method }) =>
      targetMethods.has(method)
    );
    ...
    for (let i = 0, len = routes.length; i < len; i++) {
      for (let j = i + 1; j < len; j++) {
        ...
        if (compareResult === 1) {
          ...
          if (
            routes[j].hint.components.length < routes[i].hint.components.length
          ) {
    ...
    if (hasAmbiguous) {
      return [
        buildMatcherFromPreprocessedRoutes(routes, hasAmbiguous),
        [],
        hasAmbiguous,
      ];
    }

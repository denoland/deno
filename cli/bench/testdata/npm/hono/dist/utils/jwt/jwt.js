...
const signing = async (data, secret, alg = types_1.AlgorithmTypes.HS256) => {
  ...
  const cryptoKey = await crypto.subtle.importKey(
    CryptoKeyFormat.RAW,
    (0, encode_1.utf8ToUint8Array)(secret),
    param(alg),
    false,
    [CryptoKeyUsage.Sign],
  );
  return await crypto.subtle.sign(
    param(alg),
    cryptoKey,
    (0, encode_1.utf8ToUint8Array)(data),
  );
};
const sign = async (payload, secret, alg = types_1.AlgorithmTypes.HS256) => {
  const encodedPayload = await (0, encode_1.encodeBase64URL)(
    JSON.stringify(payload),
  );
  const encodedHeader = await (0, encode_1.encodeBase64URL)(
    JSON.stringify({ alg, typ: "JWT" }),
  );
  const partialToken = `${encodedHeader}.${encodedPayload}`;
  const signature = await (0, encode_1.arrayBufferToBase64URL)(
    await signing(partialToken, secret, alg),
  );

  ...
  return async (ctx, next) => {
    ...
    if (!token) {
      ctx.res = new Response("Unauthorized", {
        ...
        headers: {
          "WWW-Authenticate":
            `Bearer realm="${ctx.req.url}",error="invalid_request",error_description="no authorization included in request"`,
        },
      });
      return;
    }
    ...
    if (!authorized) {
      ctx.res = new Response("Unauthorized", {
        ...
        headers: {
          "WWW-Authenticate":
            `Bearer realm="${ctx.req.url}",error="invalid_token",error_description="token verification failure"`,
        },
      });
      return;
    }

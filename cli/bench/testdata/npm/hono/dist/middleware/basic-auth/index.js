...
const basicAuth = (options, ...users) => {
  if (!options) {
    throw new Error(
      'basic auth middleware requires options for "username and password"',
    );
  }
  if (!options.realm) {
  ...
  return async (ctx, next) => {
    ...
    if (requestUser) {
      for (const user of users) {
        const usernameEqual = await (0, buffer_1.timingSafeEqual)(
          user.username,
          requestUser.username,
          options.hashFunction,
        );
        const passwordEqual = await (0, buffer_1.timingSafeEqual)(
          user.password,

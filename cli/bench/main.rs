...
const EXEC_TIME_BENCHMARKS: &[(&str, &[&str], Option<i32>)] = &[("hello", &["run", "cli/tests/002_hello.ts"], None), ("relative_import", &["run", "cli/tests/003_relative_import.ts"], None), ("error_001", &["run", "cli/tests/error_001.ts"], Some(1)), ("cold_hello", &["run", "--reload", "cli/tests/002_hello.ts"], None), ("cold_relative_import", &["run", "--reload", "cli/tests/003_relative_import.ts"], None), ("workers_startup", &["run", "--allow-read", "cli/tests/workers_startup_bench.ts"], None), ("workers_round_robin", &["run", "--allow-read", "cli/tests/workers_round_robin_bench.ts"], None), ("text_decoder", &["run", "cli/tests/text_decoder_perf.js"], None), ("text_encoder", &["run", "cli/tests/text_encoder_perf.js"], None), ("check", &["cache", "--reload", "std/examples/chat/server_test.ts"], None), ("no_check", &["cache", "--reload", "--no-check", "std/examples/chat/server_test.ts"], None)];
...
fn run_exec_time(deno_exe: &PathBuf, target_dir: &PathBuf) -> Result<Value> {
  ...
  for (_, args, return_code) in EXEC_TIME_BENCHMARKS {
    ...
    command.push(format!("{} {} {}", deno_exe.to_str().unwrap(), args.join(" "), ret_code_test));
  }
  ...
  for ((name, _, _), data) in EXEC_TIME_BENCHMARKS.iter().zip(hyperfine_results.as_object().unwrap().get("results").unwrap().as_array().unwrap()) {
    let data = data.as_object().unwrap().clone();
    results.insert(name.to_string(), Value::Object(data.into_iter().filter(|(key, _)| RESULT_KEYS.contains(&key.as_str())).collect::<Map<String, Value>>()));
  }

  Ok(Value::Object(results))
}

const BINARY_TARGET_FILES: &[&str] = &["CLI_SNAPSHOT.bin", "COMPILER_SNAPSHOT.bin"];
fn get_binary_sizes(target_dir: &PathBuf) -> Result<Value> {
  let mut sizes = Map::new();
  let mut mtimes = std::collections::HashMap::new();

  sizes.insert("deno".to_string(), Value::Number(Number::from(test_util::deno_exe_path().metadata()?.len())));
...
const BUNDLES: &[(&str, &str)] = &[("file_server", "./std/http/file_server.ts"), ("gist", "./std/examples/gist.ts")];
fn bundle_benchmark(deno_exe: &PathBuf) -> Result<Value> {
  let mut sizes = Map::new();

  for (name, url) in BUNDLES {
    let path = format!("{}.bundle.js", name);
    test_util::run(&[deno_exe.to_str().unwrap(), "bundle", "--unstable", url, &path], None, None, None, true);

    let file = PathBuf::from(path);
    assert!(file.is_file());
    sizes.insert(name.to_string(), Value::Number(Number::from(file.metadata()?.len())));
...
fn run_http(target_dir: &PathBuf, new_data: &mut Map<String, Value>) -> Result<()> {
  let stats = http::benchmark(target_dir)?;

  new_data.insert("req_per_sec".to_string(), Value::Object(stats.iter().map(|(name, result)| (name.clone(), Value::Number(Number::from(result.requests)))).collect::<Map<String, Value>>()));

  new_data.insert("max_latency".to_string(), Value::Object(stats.iter().map(|(name, result)| (name.clone(), Value::Number(Number::from_f64(result.latency).unwrap()))).collect::<Map<String, Value>>()));

  Ok(())
}

fn run_strace_benchmarks(deno_exe: &PathBuf, new_data: &mut Map<String, Value>) -> Result<()> {
  use std::io::Read;
  ...
  for (name, args, _) in EXEC_TIME_BENCHMARKS {
    ...
    Command::new("strace").args(&["-c", "-f", "-o", file.path().to_str().unwrap(), deno_exe.to_str().unwrap()]).args(args.iter()).stdout(Stdio::null()).spawn()?.wait()?;

    let mut output = String::new();
    file.as_file_mut().read_to_string(&mut output)?;

    let strace_result = test_util::parse_strace_output(&output);
    thread_count.insert(name.to_string(), Value::Number(Number::from(strace_result.get("clone").unwrap().calls + 1)));
    syscall_count.insert(name.to_string(), Value::Number(Number::from(strace_result.get("total").unwrap().calls)));
  }
...
fn run_max_mem_benchmark(deno_exe: &PathBuf) -> Result<Value> {
  ...
  for (name, args, return_code) in EXEC_TIME_BENCHMARKS {
    let proc = Command::new("time").args(&["-v", deno_exe.to_str().unwrap()]).args(args.iter()).stdout(Stdio::null()).stderr(Stdio::piped()).spawn()?;
    ...
    results.insert(name.to_string(), Value::Number(Number::from(test_util::parse_max_mem(&out).unwrap())));
  }
...
fn main() -> Result<()> {
  ...
  new_data.insert("created_at".to_string(), Value::String(chrono::Utc::now().to_rfc3339_opts(chrono::SecondsFormat::Secs, true)));
  new_data.insert("sha1".to_string(), Value::String(test_util::run_collect(&["git", "rev-parse", "HEAD"], None, None, None, true).0.trim().to_string()));
  ...
  // changed too.
  new_data.insert("benchmark".to_string(), run_exec_time(&deno_exe, &target_dir)?);
  ...
  if cfg!(target_os = "linux") {
    ...
    new_data.insert("max_memory".to_string(), run_max_mem_benchmark(&deno_exe)?);
  }

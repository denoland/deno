...
pub(crate) fn benchmark(target_path: &PathBuf) -> Result<HashMap<String, HttpBenchmarkResult>> {
  ...
  res.insert("deno_proxy_tcp".to_string(), deno_tcp_proxy(deno_exe, hyper_hello_exe)?);
  // "core_http_bin_ops" was once called "deno_core_single"
  // "core_http_bin_ops" was once called "deno_core_http_bench"
  res.insert("core_http_bin_ops".to_string(), core_http_bin_ops(core_http_bin_ops_exe)?);
  res.insert("core_http_json_ops".to_string(), core_http_json_ops(core_http_json_ops_exe)?);
  // "node_http" was once called "node"
  res.insert("node_http".to_string(), node_http()?);
  res.insert("node_proxy".to_string(), node_http_proxy(hyper_hello_exe)?);
  res.insert("node_proxy_tcp".to_string(), node_tcp_proxy(hyper_hello_exe)?);
...
fn run(server_cmd: &[&str], port: u16, env: Option<Vec<(String, String)>>, origin_cmd: Option<&[&str]>) -> Result<HttpBenchmarkResult> {
  ...
  let wrk_cmd = &[wrk.to_str().unwrap(), "-d", DURATION, "--latency", &format!("http://127.0.0.1:{}/", port)];
  println!("{}", wrk_cmd.join(" "));
  let output = test_util::run_collect(wrk_cmd, None, None, None, true).0;

  println!("{}", output);
  assert!(server.try_wait()?.map_or(true, |s| s.success()), "server ended with error");
...
fn deno_tcp(deno_exe: &str) -> Result<HttpBenchmarkResult> {
  ...
  run(&[deno_exe, "run", "--allow-net", "cli/bench/deno_tcp.ts", &server_addr(port)], port, None, None)
}

fn deno_tcp_proxy(deno_exe: &str, hyper_exe: &str) -> Result<HttpBenchmarkResult> {
  let port = get_port();
  let origin_port = get_port();

  println!("http_proxy_benchmark testing DENO using net/tcp.");
  run(&[deno_exe, "run", "--allow-net", "--reload", "--unstable", "cli/bench/deno_tcp_proxy.ts", &server_addr(port), &server_addr(origin_port)], port, None, Some(&[hyper_exe, &origin_port.to_string()]))
}

fn deno_http(deno_exe: &str) -> Result<HttpBenchmarkResult> {
  let port = get_port();
  println!("http_benchmark testing DENO using net/http.");
  run(&[deno_exe, "run", "--allow-net", "--reload", "--unstable", "std/http/http_bench.ts", &server_addr(port)], port, None, None)
}

#[allow(dead_code)]
fn deno_http_proxy(deno_exe: &str, hyper_exe: &str) -> Result<HttpBenchmarkResult> {
  let port = get_port();
  let origin_port = get_port();

  println!("http_proxy_benchmark testing DENO using net/http.");
  run(&[deno_exe, "run", "--allow-net", "--reload", "--unstable", "cli/bench/deno_http_proxy.ts", &server_addr(port), &server_addr(origin_port)], port, None, Some(&[hyper_exe, &origin_port.to_string()]))
}
...
fn node_http() -> Result<HttpBenchmarkResult> {
  ...
  run(&["node", "cli/bench/node_http.js", &port.to_string()], port, None, None)
}
...
fn node_http_proxy(hyper_exe: &str) -> Result<HttpBenchmarkResult> {
  ...
  run(&["node", "cli/bench/node_http_proxy.js", &port.to_string(), &origin_port], port, None, Some(&[hyper_exe, &origin_port]))
}
...
fn node_tcp_proxy(exe: &str) -> Result<HttpBenchmarkResult> {
  ...
  run(&["node", "cli/bench/node_tcp_proxy.js", &port.to_string(), &origin_port], port, None, Some(&[exe, &origin_port]))
}

fn node_tcp() -> Result<HttpBenchmarkResult> {
  let port = get_port();
  println!("http_benchmark testing node_tcp.js");
  run(&["node", "cli/bench/node_tcp.js", &port.to_string()], port, None, None)
}

...
import { mappingTable } from "./mapping_table.ts";
import * as punycode from "./punycode.ts";
import * as regexes from "./regexes.ts";
import { STATUS_MAPPING } from "./status_mapping.ts";
  ...
  while (start <= end) {
    ...
    if (min <= val && max >= val) {
      if (
        ...
        return [STATUS_MAPPING.disallowed, ...target.slice(2)] as [
          STATUS_MAPPING,
          number
        ];
      } else if (target[1] === STATUS_MAPPING.disallowed_STD3_valid) {
        return [STATUS_MAPPING.valid, ...target.slice(2)] as [
          STATUS_MAPPING,
          number
        ];
      } else if (target[1] === STATUS_MAPPING.disallowed_STD3_mapped) {
        return [STATUS_MAPPING.mapped, ...target.slice(2)] as [
          STATUS_MAPPING,
          number
        ];
      }

      return target.slice(1) as [STATUS_MAPPING, number];
    } else if (min > val) {
      end = mid - 1;
    } else {
      start = mid + 1;
      ...
      if (ch === "\u200C" || ch === "\u200D") {
        if (i > 0) {
          ...
          if (ch === "\u200C") {
            ...
            const test =
              next < 0 ? codePoints.slice(last) : codePoints.slice(last, next);
...
function isBidiDomain(labels: string[]): boolean {
  const domain = labels
    .map((label) => {
...
function processing(
  ...
  for (const [i, origLabel] of labels.entries()) {
    ...
    const validation = validateLabel(
      label,
...
export function toUnicode(
  ...
  const result = processing(domainName, {
    processingOption: processingOption as "transitional" | "nontransitional",
    checkHyphens,
    checkBidi,
    checkJoiners,
    useSTD3ASCIIRules,

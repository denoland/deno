...
async function getProcessPermissions(): Promise<Permissions> {
  const [run, read, write, net, env, hrtime] = await Promise.all([
    ...
    Deno.permissions.query({ name: "hrtime" })
  ]);

  return {
    run: run.state === "granted",
    read: read.state === "granted",
    write: write.state === "granted",
    net: net.state === "granted",
    env: env.state === "granted",
    hrtime: hrtime.state === "granted"
  };
}

...
test(function urlSearchParamsInitIterable(): void {
  ...
  const searchParams = new URLSearchParams(init);
  assert.equals(searchParams.toString(), "a=54&b=true");
});
...
test(function urlSearchParamsDeleteSuccess(): void {
  ...
  assert.equals(searchParams.toString(), "a=54");
});

test(function urlSearchParamsGetAllSuccess(): void {
  const init = "a=54&b=true&a=true";
  const searchParams = new URLSearchParams(init);
  assert.equals(searchParams.getAll("a"), ["54", "true"]);
  assert.equals(searchParams.getAll("b"), ["true"]);
  assert.equals(searchParams.getAll("c"), []);
});

test(function urlSearchParamsGetSuccess(): void {
  const init = "a=54&b=true&a=true";
  const searchParams = new URLSearchParams(init);
  assert.equals(searchParams.get("a"), "54");
  assert.equals(searchParams.get("b"), "true");
  assert.equals(searchParams.get("c"), null);
});
...
test(function urlSearchParamsSetReplaceFirstAndRemoveOthers(): void {
  ...
  assert.equals(searchParams.toString(), "a=false&b=true");
});
...
test(function urlSearchParamsSetAppendNew(): void {
  ...
  assert.equals(searchParams.toString(), "a=54&b=true&a=true&c=foo");
});
...
test(function urlSearchParamsSortSuccess(): void {
  ...
  const searchParams = new URLSearchParams(init);
  searchParams.sort();
  assert.equals(searchParams.toString(), "a=2&a=1&b=3&c=4");
});
...
test(function urlSearchParamsForEachSuccess(): void {
  ...
  searchParams.forEach((value, key, parent): void => {
    assert.equals(searchParams, parent);
...
test(function urlSearchParamsMissingName(): void {
  const init = "=4";
  const searchParams = new URLSearchParams(init);
  assert.equals(searchParams.get(""), "4");
  assert.equals(searchParams.toString(), "=4");
});
...
test(function urlSearchParamsMissingPair(): void {
  const init = "c=4&&a=54&";
  const searchParams = new URLSearchParams(init);
  assert.equals(searchParams.toString(), "c=4&a=54");
});
...
test(function urlSearchParamsDeletingAppendedMultiple(): void {
  ...
  assert.equals(params.get("first"), "1");
  params.delete("first");
  assert.equals(params.has("first"), false);
  params.append("first", (1 as unknown) as string);

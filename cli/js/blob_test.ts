...
test(function blobString(): void {
  ...
  assert.equals(b2.size, b1.size + str.length);
});
...
test(function blobBuffer(): void {
  ...
  assert.equals(b1.size, 2 * u8.length);
  const b2 = new Blob([b1, f1]);
  assert.equals(b2.size, 3 * u8.length);
});
...
test(function blobSlice(): void {
  ...
  assert.equals(b1.size, 3);
  assert.equals(b1.type, "text/html");
  const b2 = blob.slice(-1, 3);
  assert.equals(b2.size, 0);
  const b3 = blob.slice(100, 3);
  assert.equals(b3.size, 0);
  const b4 = blob.slice(0, 10);
  assert.equals(b4.size, blob.size);
});
...
test(function nativeEndLine(): void {
  ...
  assert.equals(blob.size, Deno.build.os === "win" ? 12 : 11);
});

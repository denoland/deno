...
fn validate_name(exec_name: &str) -> Result<(), Error> {
  if EXEC_NAME_RE.is_match(exec_name) {
    ...
    Err(Error::new(ErrorKind::Other, format!("Invalid executable name: {}", exec_name)))
  }
}
...
fn generate_executable_file(file_path: PathBuf, args: Vec<String>) -> Result<(), Error> {
...
fn generate_config_file(file_path: PathBuf, config_file_name: String) -> Result<(), Error> {
...
fn get_installer_root() -> Result<PathBuf, Error> {
  ...
  let mut home_path = env::var_os(home_env_var).map(PathBuf::from).ok_or_else(|| Error::new(ErrorKind::NotFound, format!("${} is not defined", home_env_var)))?;
  home_path.push(".deno");
  Ok(home_path)
}
...
pub fn install(flags: Flags, module_url: &str, args: Vec<String>, name: Option<String>, root: Option<PathBuf>, force: bool) -> Result<(), Error> {
  let root = if let Some(root) = root { root.canonicalize()? } else { get_installer_root()? };
  let installation_dir = root.join("bin");

  // ensure directory exists
  if let Ok(metadata) = fs::metadata(&installation_dir) {
    if !metadata.is_dir() {
      return Err(Error::new(ErrorKind::Other, "Installation path is not a directory"));
    }
  } else {
  ...
  if file_path.exists() && !force {
    return Err(Error::new(ErrorKind::Other, "Existing installation found. Aborting (Use -f to overwrite)."));
  };
  ...
  if let Some(log_level) = flags.log_level {
    if log_level == Level::Error {
      ...
      let log_level = match log_level {
        Level::Debug => "debug",
        Level::Info => "info",
        _ => return Err(Error::new(ErrorKind::Other, format!("invalid log level {}", log_level))),
      };
...
mod tests {
  ...
  fn install_infer_name_from_url() {
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc/server.ts").unwrap()), Some("server".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc/main.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc/mod.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc/index.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc/cli.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/main.ts").unwrap()), Some("main".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com").unwrap()), None);
    assert_eq!(infer_name_from_url(&Url::parse("file:///abc/server.ts").unwrap()), Some("server".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("file:///abc/main.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("file:///main.ts").unwrap()), Some("main".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("file:///").unwrap()), None);
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc@0.1.0").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc@0.1.0/main.ts").unwrap()), Some("abc".to_string()));
    assert_eq!(infer_name_from_url(&Url::parse("https://example.com/abc@def@ghi").unwrap()), Some("abc".to_string()));
  }
  ...
  fn install_basic() {
    ...
    install(Flags::default(), "http://localhost:4545/cli/tests/echo_server.ts", vec![], Some("echo_test".to_string()), None, false).expect("Install failed");
  ...
  fn install_unstable() {
    ...
    install(Flags { unstable: true, ..Flags::default() }, "http://localhost:4545/cli/tests/echo_server.ts", vec![], Some("echo_test".to_string()), Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
    ...
    assert!(content.contains(r#""run" "--unstable" "http://localhost:4545/cli/tests/echo_server.ts"#));
  }
  ...
  fn install_inferred_name() {
    ...
    install(Flags::default(), "http://localhost:4545/cli/tests/echo_server.ts", vec![], None, Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
  ...
  fn install_inferred_name_from_parent() {
    ...
    install(Flags::default(), "http://localhost:4545/cli/tests/subdir/main.ts", vec![], None, Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
    ...
    assert!(file_path.exists());
    let content = fs::read_to_string(file_path).unwrap();
    assert!(content.contains(r#""run" "http://localhost:4545/cli/tests/subdir/main.ts""#));
  }
  ...
  fn install_custom_dir_option() {
    ...
    install(Flags::default(), "http://localhost:4545/cli/tests/echo_server.ts", vec![], Some("echo_test".to_string()), Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
  ...
  fn install_with_flags() {
    ...
    install(Flags { allow_net: true, allow_read: true, no_check: true, log_level: Some(Level::Error), ..Flags::default() }, "http://localhost:4545/cli/tests/echo_server.ts", vec!["--foobar".to_string()], Some("echo_test".to_string()), Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
  ...
  fn install_local_module() {
    ...
    install(Flags::default(), &local_module_str, vec![], Some("echo_test".to_string()), Some(temp_dir.path().to_path_buf()), false).expect("Install failed");
  ...
  fn install_with_config() {
    ...
    let result = install(Flags { config_path: Some(config_file_path.to_string_lossy().to_string()), ..Flags::default() }, "http://localhost:4545/cli/tests/cat.ts", vec![], Some("echo_test".to_string()), Some(temp_dir.path().to_path_buf()), true);

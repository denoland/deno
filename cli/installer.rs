...
// They MUST present on *1st* or *2nd* line of the file.
static TEMPLATE_HEADER: &str = "This executable is generated by Deno. Please don't modify it unless you know what it means.";
...
"#,
    TEMPLATE_HEADER, // Marker at 1st line
    commands[1..].join(" "),
    commands.join(" ")
  );
...
exit $ret
"#,
    TEMPLATE_HEADER, // Marker at 2nd line
    commands[1..].join(" "),
    commands.join(" ")
  );
  let mut file = File::create(&file_path)?;
  ...
  for exec_name in exec_names.iter() {
    ...
    if file_path.exists() {
      if !file_path.is_file() {
        failure_msg.replace(format!(
          "{} is not a regular file",
          file_path.to_string_lossy()
        ));
      } else if !is_deno_executable(&file_path) {
        failure_msg.replace(format!(
          "{} is not a Deno executable",
          file_path.to_string_lossy()
        ));
      ...
      failure_msg.replace(format!("{} not found", file_path.to_string_lossy()));
    }
...
fn is_deno_executable(filename: &PathBuf) -> bool {
  ...
  match lines.next() {
    Some(Ok(line)) => line.contains(TEMPLATE_HEADER),
    _ => false,
  }
}

            ...
            g.get(&uuid).map(|info| info.new_websocket_tx.clone()).map(
              |new_websocket_tx| {
                ws.on_upgrade(move |websocket| async move {
                  let (channel, pump) = create_websocket_proxy(websocket);
                  let _ = new_websocket_tx.unbounded_send(channel);
                  pump.await;
                })
              },
            )
...
/// 'Tokio's task budget, which causes issues wen DenoInspector::poll_sessions
...
/// is a LocalFuture that does the forwarding.
fn create_websocket_proxy(
  websocket: ws::WebSocket,
) -> (WebSocketProxy, Pin<Box<dyn Future<Output = ()> + Send>>) {
  ...
  let proxy_channel = WebSocketProxy {
    tx: outbound_tx,
    rx: inbound_rx,
  };
...
impl Drop for DenoInspectorSession {
  fn drop(&mut self) {
    self.websocket_tx.close_channel()
  }
}

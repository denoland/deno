...
mod tests {
  ...
  fn execute_mod_esm_imports_a() {
    let module_specifier =
      ModuleSpecifier::resolve_from_cwd("tests/esm_imports_a.js").unwrap();
  ...
  fn execute_mod_circular() {
    let module_specifier =
      ModuleSpecifier::resolve_from_cwd("tests/circular1.js").unwrap();
    let argv = vec![String::from("./deno"), module_specifier.to_string()];
    let state = ThreadSafeState::new(
      flags::DenoFlags::default(),
      argv,
  ...
  fn execute_006_url_imports() {
    let module_specifier =
      ModuleSpecifier::resolve_from_cwd("tests/006_url_imports.ts").unwrap();
    let argv = vec![String::from("deno"), module_specifier.to_string()];
  ...
  fn execute_mod_resolve_error() {
    tokio_util::init(|| {
      ...
      let mut worker = create_test_worker();
      let module_specifier =
        ModuleSpecifier::resolve_from_cwd("does-not-exist").unwrap();
  ...
  fn execute_mod_002_hello() {
    tokio_util::init(|| {
      ...
      let mut worker = create_test_worker();
      let module_specifier =
        ModuleSpecifier::resolve_from_cwd("./tests/002_hello.ts").unwrap();
      let result = worker.execute_mod_async(&module_specifier, false).wait();
      assert!(result.is_ok());
    })
  }

...
pub fn create_main_worker(
  ...
  let maybe_storage_key = ps.root_config.resolve_storage_key(&main_module);
...
async fn cache_command(
  ...
  let ps = ProcState::build(flags).await?;
  let lib = ps.root_config.ts_type_lib_window();
...
async fn create_graph_and_maybe_check(
  ...
  let maybe_locker = lockfile::as_maybe_locker(ps.lockfile.clone());
  let maybe_imports = ps.root_config.to_maybe_imports()?;
  let maybe_import_map_resolver =
    ps.maybe_import_map.clone().map(ImportMapResolver::new);
  let maybe_jsx_resolver = ps
    .root_config
  ...
  let check_js = ps.root_config.check_js();
  graph_valid(
    &graph,
  ...
  if ps.flags().type_check_mode != TypeCheckMode::None {
    ...
    let ts_config_result =
      ps.root_config.resolve_ts_config(TsConfigType::Check {
        tsc_emit: false,
        lib,
      })?;
    ...
    let maybe_config_specifier = ps.root_config.maybe_config_file_specifier();
...
fn bundle_module_graph(
  ...
  let ts_config_result =
    ps.root_config.resolve_ts_config(TsConfigType::Bundle)?;
  if ps.flags().type_check_mode == TypeCheckMode::None {
    if let Some(ignored_options) = ts_config_result.maybe_ignored_options {
      eprintln!("{}", ignored_options);
    }
...
async fn bundle_command(
  ...
  if root_config.flags.watch.is_some() {
    file_watcher::watch_func(
      resolver,
      operation,
      file_watcher::PrintConfig {
        job_name: "Bundle".to_string(),
        clear_screen: !root_config.flags.no_clear_screen,
      },
    )
    .await?;
  } else {
...
async fn format_command(
  ...
  let ps = ProcState::build(flags).await?;
  let maybe_fmt_config = ps.root_config.to_fmt_config()?;
  ...
  tools::fmt::format(ps.flags(), fmt_flags, maybe_fmt_config, &ps.dir).await?;
  Ok(0)
}

...
fn doc_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let source_file = matches.remove_one::<String>("source_file");
...
fn fmt_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  ...
  let line_width = matches.remove_one::<NonZeroU32>("options-line-width");
  let indent_width = matches.remove_one::<NonZeroU8>("options-indent-width");
  let single_quote = if matches.get_flag("options-single-quote") {
    Some(true)
  } else {
    None
  };
...
fn ca_file_arg_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  flags.ca_file = matches.remove_one::<String>("cert");
}
...
fn local_npm_args_parse(flags: &mut Flags, matches: &mut ArgMatches) {
  if matches.get_flag("node-modules-dir") {

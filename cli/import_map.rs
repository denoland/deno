...
impl ImportMapError {
  pub fn new(msg: &str) -> Self {
    ImportMapError { msg: msg.to_string() }
  }
}
...
impl ImportMap {
  pub fn load(file_path: &str) -> Result<Self, ErrBox> {
    ...
    let json_string = fs::read_to_string(&resolved_path).map_err(|err| io::Error::new(io::ErrorKind::InvalidInput, format!("Error retrieving import map file at \"{}\": {}", resolved_path.to_str().unwrap(), err.to_string()).as_str()))?;
  ...
  fn normalize_addresses(specifier_key: &str, base_url: &str, potential_addresses: Vec<String>) -> Vec<ModuleSpecifier> {
  ...
  fn parse_specifier_map(json_map: &Map<String, Value>, base_url: &str) -> SpecifierMap {
    ...
    for (specifier_key, value) in json_map.iter() {
      let normalized_specifier_key = match ImportMap::normalize_specifier_key(specifier_key, base_url) {
        Some(s) => s,
        None => continue,
      };
      ...
      let normalized_address_array = ImportMap::normalize_addresses(&normalized_specifier_key, base_url, potential_addresses);

      debug!("normalized specifier {:?}; {:?}", normalized_specifier_key, normalized_address_array);
  ...
  fn parse_scope_map(scope_map: &Map<String, Value>, base_url: &str) -> Result<ScopesMap, ImportMapError> {
    ...
    for (scope_prefix, potential_specifier_map) in scope_map.iter() {
      if !potential_specifier_map.is_object() {
        return Err(ImportMapError::new(&format!("The value for the {:?} scope prefix must be an object", scope_prefix)));
      }
      ...
      let scope_prefix_url = match Url::parse(base_url).unwrap().join(scope_prefix) {
        Ok(url) => {
          if !SUPPORTED_FETCH_SCHEMES.contains(&url.scheme()) {
            eprintln!("Invalid scope {:?}. Scope URLs must have a valid fetch scheme.", url.to_string());
            continue;
          }
          url.to_string()
        }
  ...
  pub fn resolve_scopes_match(scopes: &ScopesMap, normalized_specifier: &str, referrer: &str) -> Result<Option<ModuleSpecifier>, ImportMapError> {
    // exact-match
  ...
  pub fn resolve_imports_match(imports: &SpecifierMap, normalized_specifier: &str) -> Result<Option<ModuleSpecifier>, ImportMapError> {
    // exact-match
    if let Some(address_vec) = imports.get(normalized_specifier) {
      if address_vec.is_empty() {
        return Err(ImportMapError::new(&format!("Specifier {:?} was mapped to no addresses.", normalized_specifier)));
  ...
  pub fn resolve(&self, specifier: &str, referrer: &str) -> Result<Option<ModuleSpecifier>, ImportMapError> {
    ...
    let scopes_match = ImportMap::resolve_scopes_match(&self.scopes, &normalized_specifier, &referrer.to_string())?;
    ...
    Err(ImportMapError::new(&format!("Unmapped bare specifier {:?}", normalized_specifier)))
  }
}
...
mod tests {
  ...
  fn from_json_1() {
    ...
    for non_object in non_object_strings.to_vec() {
      assert!(ImportMap::from_json(base_url, &format!("{{\"imports\": {}}}", non_object),).is_err());
    }

    // invalid schema: 'scopes' is non-object
    for non_object in non_object_strings.to_vec() {
      assert!(ImportMap::from_json(base_url, &format!("{{\"scopes\": {}}}", non_object),).is_err());
    }
  }
  ...
  fn parse_specifier_keys_relative() {
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert_eq!(import_map.imports.get("https://base.example/path1/path2/foo").unwrap()[0], "https://base.example/dotslash".to_string());
    assert_eq!(import_map.imports.get("https://base.example/path1/foo").unwrap()[0], "https://base.example/dotdotslash".to_string());
    assert_eq!(import_map.imports.get("https://base.example/foo").unwrap()[0], "https://base.example/slash".to_string());
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert_eq!(import_map.imports.get("https://base.example/path1/path2/").unwrap()[0], "https://base.example/dotslash/".to_string());
    assert_eq!(import_map.imports.get("https://base.example/path1/").unwrap()[0], "https://base.example/dotdotslash/".to_string());
    assert_eq!(import_map.imports.get("https://base.example/").unwrap()[0], "https://base.example/slash/".to_string());
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert_eq!(import_map.imports.get("%2E/").unwrap()[0], "https://base.example/dotSlash1/".to_string());
    assert_eq!(import_map.imports.get("%2E%2E/").unwrap()[0], "https://base.example/dotDotSlash1/".to_string());
    assert_eq!(import_map.imports.get(".%2F").unwrap()[0], "https://base.example/dotSlash2".to_string());
    assert_eq!(import_map.imports.get("..%2F").unwrap()[0], "https://base.example/dotDotSlash2".to_string());
    assert_eq!(import_map.imports.get("%2F").unwrap()[0], "https://base.example/slash2".to_string());
    assert_eq!(import_map.imports.get("%2E%2F").unwrap()[0], "https://base.example/dotSlash3".to_string());
    assert_eq!(import_map.imports.get("%2E%2E%2F").unwrap()[0], "https://base.example/dotDotSlash3".to_string());
  }
  ...
  fn parse_specifier_keys_absolute() {
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert_eq!(import_map.imports.get("http://good/").unwrap()[0], "https://base.example/http/".to_string());
    assert_eq!(import_map.imports.get("https://good/").unwrap()[0], "https://base.example/https/".to_string());
    assert_eq!(import_map.imports.get("file:///good").unwrap()[0], "https://base.example/file".to_string());
    assert_eq!(import_map.imports.get("http://good/").unwrap()[0], "https://base.example/http/".to_string());
    assert_eq!(import_map.imports.get("import:bad").unwrap()[0], "https://base.example/import".to_string());
    assert_eq!(import_map.imports.get("mailto:bad").unwrap()[0], "https://base.example/mailto".to_string());
    assert_eq!(import_map.imports.get("javascript:bad").unwrap()[0], "https://base.example/javascript".to_string());
    assert_eq!(import_map.imports.get("wss:bad").unwrap()[0], "https://base.example/wss".to_string());
    assert_eq!(import_map.imports.get("about:bad").unwrap()[0], "https://base.example/about".to_string());
    assert_eq!(import_map.imports.get("blob:bad").unwrap()[0], "https://base.example/blob".to_string());
    assert_eq!(import_map.imports.get("data:bad").unwrap()[0], "https://base.example/data".to_string());
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert_eq!(import_map.imports.get("https://ex ample.org/").unwrap()[0], "https://base.example/unparseable1/".to_string());
    assert_eq!(import_map.imports.get("https://example.com:demo").unwrap()[0], "https://base.example/unparseable2".to_string());
    assert_eq!(import_map.imports.get("http://[www.example.com]/").unwrap()[0], "https://base.example/unparseable3/".to_string());
    assert_eq!(import_map.imports.get("https://example.org/").unwrap()[0], "https://base.example/invalidButParseable1/".to_string());
    assert_eq!(import_map.imports.get("https://example.com///").unwrap()[0], "https://base.example/invalidButParseable2/".to_string());
    assert_eq!(import_map.imports.get("https://example.net/").unwrap()[0], "https://base.example/prettyNormal/".to_string());
    assert_eq!(import_map.imports.get("https://example.com/").unwrap()[0], "https://base.example/percentDecoding/".to_string());
    assert_eq!(import_map.imports.get("https://example.com/%41").unwrap()[0], "https://base.example/noPercentDecoding".to_string());
  }
  ...
  fn parse_scope_keys_relative() {
    ...
    let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", json_map).unwrap();
    assert!(import_map.scopes.contains_key("https://base.example/path1/path2/foo/bar?baz#qux"));
  ...
  fn parse_addresses_relative_url_like() {
    ...
    assert_eq!(import_map.imports.get("dotSlash").unwrap(), &vec!["https://base.example/path1/path2/foo".to_string()]);
    ...
    assert_eq!(import_map.imports.get("dotSlash").unwrap(), &vec!["https://base.example/path1/path2/".to_string()]);
    assert_eq!(import_map.imports.get("dotDotSlash").unwrap(), &vec!["https://base.example/path1/".to_string()]);
    assert_eq!(import_map.imports.get("slash").unwrap(), &vec!["https://base.example/".to_string()]);
  ...
  fn parse_addresses_unparseable() {
    ...
    assert_eq!(import_map.imports.get("invalidButParseable1").unwrap(), &vec!["https://example.org/".to_string()]);
    assert_eq!(import_map.imports.get("invalidButParseable2").unwrap(), &vec!["https://example.com///".to_string()]);
    assert_eq!(import_map.imports.get("prettyNormal").unwrap(), &vec!["https://example.net/".to_string()]);
    assert_eq!(import_map.imports.get("percentDecoding").unwrap(), &vec!["https://example.com/".to_string()]);
    assert_eq!(import_map.imports.get("noPercentDecoding").unwrap(), &vec!["https://example.com/%41".to_string()]);
  ...
  fn parse_addresses_mismatched_trailing_slashes_with_nonmismatched_array() {
    ...
    assert_eq!(import_map.imports.get("trailer/").unwrap(), &vec!["https://base.example/atrailer/".to_string()]);
    // TODO: I'd be good to assert that warning was shown
  }
  ...
  fn parse_addresses_other_invalid() {
    ...
    for bad in &["bar", "\\bar", "~bar", "#bar", "?bar"] {
      ...
      let import_map = ImportMap::from_json("https://base.example/path1/path2/path3", &json_map.to_string()).unwrap();
  ...
  fn get_empty_import_map() -> ImportMap {
    ImportMap { base_url: "https://example.com/app/main.ts".to_string(), imports: IndexMap::new(), scopes: IndexMap::new() }
  }
  ...
  fn resolve_unmapped_relative_specifiers() {
    ...
    assert_resolve(import_map.resolve("./foo/bar", referrer_url), "https://example.com/js/foo/bar");
    assert_resolve(import_map.resolve("./foo/../bar", referrer_url), "https://example.com/js/bar");
    assert_resolve(import_map.resolve("./foo/../../bar", referrer_url), "https://example.com/bar");
  ...
  fn resolve_unmapped_absolute_specifiers() {
    ...
    assert_resolve(import_map.resolve("https://example.net", referrer_url), "https://example.net/");
    assert_resolve(import_map.resolve("https://ex%41mple.com/", referrer_url), "https://example.com/");
    assert_resolve(import_map.resolve("https:example.org", referrer_url), "https://example.org/");
    assert_resolve(import_map.resolve("https://///example.com///", referrer_url), "https://example.com///");
  }
  ...
  fn resolve_imports_package_like_modules() {
    ...
    assert_resolve(import_map.resolve("moment", referrer_url), "https://example.com/deps/moment/src/moment.js");
    assert_resolve(import_map.resolve("lodash-dot", referrer_url), "https://example.com/app/deps/lodash-es/lodash.js");
    assert_resolve(import_map.resolve("lodash-dotdot", referrer_url), "https://example.com/deps/lodash-es/lodash.js");

    // Should work for package submodules.
    assert_resolve(import_map.resolve("moment/foo", referrer_url), "https://example.com/deps/moment/src/foo");
    assert_resolve(import_map.resolve("lodash-dot/foo", referrer_url), "https://example.com/app/deps/lodash-es/foo");
    assert_resolve(import_map.resolve("lodash-dotdot/foo", referrer_url), "https://example.com/deps/lodash-es/foo");

    // Should work for package names that end in a slash.
    assert_resolve(import_map.resolve("moment/", referrer_url), "https://example.com/deps/moment/src/");
  ...
  fn resolve_imports_tricky_specifiers() {
    ...
    assert_resolve(import_map.resolve("package/withslash", referrer_url), "https://example.com/deps/package-with-slash/index.mjs");
    ...
    assert_resolve(import_map.resolve("..", referrer_url), "https://example.com/lib/dotdot.mjs");
    assert_resolve(import_map.resolve("..\\\\", referrer_url), "https://example.com/lib/dotdotbackslash.mjs");
    assert_resolve(import_map.resolve("%2E", referrer_url), "https://example.com/lib/percent2e.mjs");
    assert_resolve(import_map.resolve("%2F", referrer_url), "https://example.com/lib/percent2f.mjs");
  ...
  fn resolve_imports_url_like_specifier() {
    ...
    assert_resolve(import_map.resolve("https://example.com/lib/foo.mjs", referrer_url), "https://example.com/app/more/bar.mjs");
    assert_resolve(import_map.resolve("https://///example.com/lib/foo.mjs", referrer_url), "https://example.com/app/more/bar.mjs");
    assert_resolve(import_map.resolve("/lib/foo.mjs", referrer_url), "https://example.com/app/more/bar.mjs");
    assert_resolve(import_map.resolve("https://example.com/app/dotrelative/foo.mjs", referrer_url), "https://example.com/lib/dot.mjs");
    assert_resolve(import_map.resolve("../app/dotrelative/foo.mjs", referrer_url), "https://example.com/lib/dot.mjs");
    assert_resolve(import_map.resolve("https://example.com/dotdotrelative/foo.mjs", referrer_url), "https://example.com/lib/dotdot.mjs");
    assert_resolve(import_map.resolve("../dotdotrelative/foo.mjs", referrer_url), "https://example.com/lib/dotdot.mjs");
    ...
    assert!(import_map.resolve("../lib/no.mjs", referrer_url).is_err());
    assert!(import_map.resolve("https://example.com/app/dotrelative/no.mjs", referrer_url).is_err());
    assert!(import_map.resolve("/app/dotrelative/no.mjs", referrer_url).is_err());
    assert!(import_map.resolve("../app/dotrelative/no.mjs", referrer_url).is_err());

    // Should remap URLs that are just composed from / and ..
    assert_resolve(import_map.resolve("https://example.com/", referrer_url), "https://example.com/lib/slash-only/");
    assert_resolve(import_map.resolve("/", referrer_url), "https://example.com/lib/slash-only/");
    assert_resolve(import_map.resolve("../", referrer_url), "https://example.com/lib/slash-only/");
    assert_resolve(import_map.resolve("https://example.com/app/", referrer_url), "https://example.com/lib/dotslash-only/");
    assert_resolve(import_map.resolve("/app/", referrer_url), "https://example.com/lib/dotslash-only/");
    assert_resolve(import_map.resolve("../app/", referrer_url), "https://example.com/lib/dotslash-only/");

    // Should remap URLs that are prefix-matched by keys with trailing slashes.
    assert_resolve(import_map.resolve("/test/foo.mjs", referrer_url), "https://example.com/lib/url-trailing-slash/foo.mjs");
    assert_resolve(import_map.resolve("https://example.com/app/test/foo.mjs", referrer_url), "https://example.com/lib/url-trailing-slash-dot/foo.mjs");
    ...
    assert_resolve(import_map.resolve("/test", referrer_url), "https://example.com/lib/test2.mjs");
  }
  ...
  fn resolve_scopes_exact_vs_prefix_matching() {
    ...
    assert_resolve(import_map.resolve("moment", js_non_dir), "https://example.com/only-triggered-by-exact/moment");
    assert_resolve(import_map.resolve("moment/foo", js_non_dir), "https://example.com/only-triggered-by-exact/moment/foo");
    assert_resolve(import_map.resolve("moment", js_in_dir), "https://example.com/triggered-by-any-subpath/moment");
    assert_resolve(import_map.resolve("moment/foo", js_in_dir), "https://example.com/triggered-by-any-subpath/moment/foo");
  ...
  fn resolve_scopes_package_like() {
    ...
    assert_resolve(import_map.resolve("lodash-dot", js_in_dir), "https://example.com/app/node_modules_2/lodash-es/lodash.js");
    assert_resolve(import_map.resolve("lodash-dotdot", js_in_dir), "https://example.com/node_modules_2/lodash-es/lodash.js");
    assert_resolve(import_map.resolve("lodash-dot/foo", js_in_dir), "https://example.com/app/node_modules_2/lodash-es/foo");
    assert_resolve(import_map.resolve("lodash-dotdot/foo", js_in_dir), "https://example.com/node_modules_2/lodash-es/foo");

    // Should apply best scope match.
    assert_resolve(import_map.resolve("moment", top_level), "https://example.com/node_modules_3/moment/src/moment.js");
    assert_resolve(import_map.resolve("moment", js_in_dir), "https://example.com/node_modules_3/moment/src/moment.js");
    assert_resolve(import_map.resolve("vue", js_in_dir), "https://example.com/node_modules_3/vue/dist/vue.runtime.esm.js");

    // Should fallback to "imports".
    assert_resolve(import_map.resolve("moment/foo", top_level), "https://example.com/node_modules/moment/src/foo");
    assert_resolve(import_map.resolve("moment/foo", js_in_dir), "https://example.com/node_modules/moment/src/foo");
    assert_resolve(import_map.resolve("lodash-dot", top_level), "https://example.com/app/node_modules/lodash-es/lodash.js");
    assert_resolve(import_map.resolve("lodash-dotdot", top_level), "https://example.com/node_modules/lodash-es/lodash.js");
    assert_resolve(import_map.resolve("lodash-dot/foo", top_level), "https://example.com/app/node_modules/lodash-es/foo");
    assert_resolve(import_map.resolve("lodash-dotdot/foo", top_level), "https://example.com/node_modules/lodash-es/foo");
  ...
  fn resolve_scopes_relative_url_keys() {
    ...
    assert_resolve(import_map.resolve("b", in_same_dir_as_map), "https://example.com/b-dot-slash.mjs");

    // Should resolve a ../ scope using the import map URL's directory.
    assert_resolve(import_map.resolve("c", base_url), "https://example.com/c-dot-dot-slash.mjs");
    assert_resolve(import_map.resolve("c", in_same_dir_as_map), "https://example.com/c-dot-dot-slash.mjs");
    assert_resolve(import_map.resolve("c", in_dir_above_map), "https://example.com/c-dot-dot-slash.mjs");
  }
  ...
  fn resolve_builtins_remap() {
    ...
    assert_resolve(import_map.resolve("std:blank", base_url), "https://example.com/app/blank.mjs");
    assert_resolve(import_map.resolve("std:none", base_url), "https://example.com/app/none.mjs");
  }
}

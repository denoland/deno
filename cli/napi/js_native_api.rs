...
mod uv {
  ...
  impl std::fmt::Debug for uv_async_t {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
      f.debug_struct("uv_async_t")
  ...
  fn napi_none() -> napi_value<'static> {
    napi_value::from(None::<v8::Local<'static, v8::Value>>)
  }

  //TODO(nathanwhit): tests that the various uv types match up with us
  type uv_loop_t = Env;
  ...
  unsafe extern "C" fn uv_async_send(handle: *mut uv_async_t) -> c_int {
    unsafe { napi_queue_async_work((*handle).r#loop, (*handle).work) * -1 }
  }

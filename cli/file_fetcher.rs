...
impl TextDocument {
  pub fn new(bytes: Vec<u8>, charset: Option<impl Into<Cow<'static, str>>>) -> TextDocument {
    let charset = charset.map(|cs| cs.into()).unwrap_or_else(|| text_encoding::detect_charset(&bytes).into());
...
impl SourceFileFetcher {
  pub fn new(http_cache: HttpCache, use_disk_cache: bool, cache_blocklist: Vec<String>, no_remote: bool, cached_only: bool, ca_file: Option<&str>) -> Result<Self, ErrBox> {
    let file_fetcher = Self { http_cache, source_file_cache: SourceFileCache::default(), cache_blocklist, use_disk_cache, no_remote, cached_only, http_client: create_http_client(ca_file)? };
  ...
  pub fn check_if_supported_scheme(url: &Url) -> Result<(), ErrBox> {
    if !SUPPORTED_URL_SCHEMES.contains(&url.scheme()) {
      return Err(ErrBox::error(format!("Unsupported scheme \"{}\" for module \"{}\". Supported schemes: {:#?}", url.scheme(), url, SUPPORTED_URL_SCHEMES)));
    }
  ...
  pub fn fetch_cached_source_file(&self, specifier: &ModuleSpecifier, permissions: Permissions) -> Option<SourceFile> {
    ...
    // action in that path.
    if let Ok(maybe_source_file) = self.get_source_file_from_local_cache(specifier.as_url(), &permissions) {
      return maybe_source_file;
    }
  ...
  pub fn save_source_file_in_cache(&self, specifier: &ModuleSpecifier, file: SourceFile) {
    self.source_file_cache.set(specifier.to_string(), file);
  }

  pub async fn fetch_source_file(&self, specifier: &ModuleSpecifier, maybe_referrer: Option<ModuleSpecifier>, permissions: Permissions) -> Result<SourceFile, ErrBox> {
    let module_url = specifier.as_url().to_owned();
    debug!("fetch_source_file specifier: {} maybe_referrer: {:#?}", &module_url, maybe_referrer.as_ref());
    ...
    let result = self.get_source_file(&module_url, self.use_disk_cache, self.no_remote, self.cached_only, &permissions).await;

    match result {
      Ok(mut file) => {
        // TODO: move somewhere?
        if file.source_code.bytes.starts_with(b"#!") {
          file.source_code = filter_shebang(&file.source_code.to_str().unwrap()[..]).into();
        }
      ...
      Err(err) => {
        ...
        let referrer_suffix = if let Some(referrer) = maybe_referrer { format!(r#" from "{}""#, referrer) } else { "".to_owned() };
        ...
        let err = if err.to_string().contains("--cached-only") {
          let msg = format!(r#"Cannot find module "{}"{} in cache, --cached-only is specified"#, module_url, referrer_suffix);
          ErrBox::new("NotFound", msg)
        } else if is_not_found {
          let msg = format!(r#"Cannot resolve module "{}"{}"#, module_url, referrer_suffix);
          ErrBox::new("NotFound", msg)
        } else {
          err
        };
  ...
  fn get_source_file_from_local_cache(&self, module_url: &Url, permissions: &Permissions) -> Result<Option<SourceFile>, ErrBox> {
  ...
  /// not already cached.
  async fn get_source_file(&self, module_url: &Url, use_disk_cache: bool, no_remote: bool, cached_only: bool, permissions: &Permissions) -> Result<SourceFile, ErrBox> {
    let url_scheme = module_url.scheme();
    let is_local_file = url_scheme == "file";
    SourceFileFetcher::check_if_supported_scheme(&module_url)?;
    ...
    if no_remote {
      let e = std::io::Error::new(std::io::ErrorKind::NotFound, format!("Not allowed to get remote file '{}'", module_url.to_string()));
      return Err(e.into());
    }

    // Fetch remote file and cache on-disk for subsequent access
    self.fetch_remote_source(&module_url, use_disk_cache, cached_only, 10, permissions).await
  }

  /// Fetch local source file.
  fn fetch_local_file(&self, module_url: &Url, permissions: &Permissions) -> Result<SourceFile, ErrBox> {
    let filepath = module_url.to_file_path().map_err(|()| ErrBox::new("URIError", "File URL contains invalid path"))?;
    ...
    Ok(SourceFile { url: module_url.clone(), filename: filepath, media_type, source_code: TextDocument::new(source_code, charset), types_header: None })
  }
  ...
  fn fetch_cached_remote_source(&self, module_url: &Url, redirect_limit: i64) -> Result<Option<SourceFile>, ErrBox> {
    if redirect_limit < 0 {
    ...
    let (media_type, charset) = map_content_type(&fake_filepath, headers.get("content-type").map(|e| e.as_str()));
    let types_header = headers.get("x-typescript-types").map(|e| e.to_string());
    Ok(Some(SourceFile { url: module_url.clone(), filename: cache_filename, media_type, source_code: TextDocument::new(source_code, charset), types_header }))
  }
  ...
  /// Pin<Box<..>>.
  fn fetch_remote_source(&self, module_url: &Url, use_disk_cache: bool, cached_only: bool, redirect_limit: i64, permissions: &Permissions) -> Pin<Box<dyn Future<Output = Result<SourceFile, ErrBox>>>> {
    if redirect_limit < 0 {
    ...
    let is_blocked = check_cache_blocklist(module_url, self.cache_blocklist.as_ref());
    ...
    if cached_only {
      ...
      let message = format!("Cannot find remote file '{}' in cache, --cached-only is specified", module_url);
      return futures::future::err(ErrBox::new("NotFound", message)).boxed_local();
    }
    ...
    let f = async move {
      match http_util::fetch_once(http_client, &module_url, module_etag).await? {
        FetchOnceResult::NotModified => {
          let source_file = dir.fetch_cached_remote_source(&module_url, 10)?.unwrap();
        ...
        FetchOnceResult::Redirect(new_module_url, headers) => {
          ...
          // Recurse
          dir.fetch_remote_source(&new_module_url, use_disk_cache, cached_only, redirect_limit - 1, &permissions).await
        }
        FetchOnceResult::Code(source, headers) => {
          ...
          let fake_filepath = PathBuf::from(module_url.path());
          let (media_type, charset) = map_content_type(&fake_filepath, headers.get("content-type").map(String::as_str));

          let types_header = headers.get("x-typescript-types").map(String::to_string);

          let source_file = SourceFile { url: module_url.clone(), filename: cache_filepath, media_type, source_code: TextDocument::new(source, charset), types_header };
...
fn map_content_type(path: &Path, content_type: Option<&str>) -> (msg::MediaType, Option<String>) {
  match content_type {
    Some(content_type) => {
      ...
      let media_type = match ct.to_lowercase().as_ref() {
        "application/typescript" | "text/typescript" | "video/vnd.dlna.mpeg-tts" | "video/mp2t" | "application/x-typescript" => map_js_like_extension(path, msg::MediaType::TypeScript),
        "application/javascript" | "text/javascript" | "application/ecmascript" | "text/ecmascript" | "application/x-javascript" | "application/node" => map_js_like_extension(path, msg::MediaType::JavaScript),
      ...
      let charset = ct_iter.map(str::trim).find_map(|s| s.strip_prefix("charset=")).map(String::from);
...
fn map_js_like_extension(path: &Path, default: msg::MediaType) -> msg::MediaType {
  match path.extension() {
    None => default,
    Some(os_str) => match os_str.to_str() {
      None => default,
...
mod tests {
  ...
  fn setup_file_fetcher(dir_path: &Path) -> SourceFileFetcher {
    SourceFileFetcher::new(HttpCache::new(&dir_path.to_path_buf().join("deps")), true, vec![], false, false, None).expect("setup fail")
  }
  ...
  fn test_cache_blocklist() {
    let args = crate::flags::resolve_urls(vec![String::from("http://deno.land/std"), String::from("http://github.com/example/mod.ts"), String::from("http://fragment.com/mod.ts#fragment"), String::from("http://query.com/mod.ts?foo=bar"), String::from("http://queryandfragment.com/mod.ts?foo=bar#fragment")]);
    ...
    let u: Url = "http://queryandfragment.com/mod.ts?foo=bar".parse().unwrap();
    assert_eq!(check_cache_blocklist(&u, &args), true);

    let u: Url = "http://queryandfragment.com/mod.ts#fragment".parse().unwrap();
    assert_eq!(check_cache_blocklist(&u, &args), false);

    let u: Url = "http://query.com/mod.ts?foo=bar#fragment".parse().unwrap();
    assert_eq!(check_cache_blocklist(&u, &args), true);

    let u: Url = "http://fragment.com/mod.ts?foo=bar#fragment".parse().unwrap();
    assert_eq!(check_cache_blocklist(&u, &args), true);
  }
  ...
  fn test_fetch_local_file_no_panic() {
    ...
    if cfg!(windows) {
      ...
      fetcher.fetch_local_file(&u, &Permissions::allow_all()).unwrap_err();
    } else {
  ...
  async fn test_get_source_code_1() {
    ...
    let module_url = Url::parse("http://localhost:4545/cli/tests/subdir/mod2.ts").unwrap();
    ...
    let result = fetcher.get_source_file(&module_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    let r = result.unwrap();
    assert_eq!(r.source_code.bytes, &b"export { printHello } from \"./print_hello.ts\";\n"[..]);
    ...
    metadata.headers.insert("content-type".to_string(), "text/javascript".to_string());
    metadata.write(&cache_filename).unwrap();

    let result2 = fetcher_1.get_source_file(&module_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result2.is_ok());
    let r2 = result2.unwrap();
    assert_eq!(r2.source_code.bytes, &b"export { printHello } from \"./print_hello.ts\";\n"[..]);
    ...
    metadata.headers = HashMap::new();
    metadata.headers.insert("content-type".to_string(), "application/json".to_string());
    metadata.write(&cache_filename).unwrap();

    let result3 = fetcher_2.get_source_file(&module_url_1, true, false, false, &Permissions::allow_all()).await;
    assert!(result3.is_ok());
    let r3 = result3.unwrap();
    assert_eq!(r3.source_code.bytes, &b"export { printHello } from \"./print_hello.ts\";\n"[..]);
    // If get_source_file does not call remote, this should be JavaScript
    // as we modified before! (we do not overwrite .headers.json due to no http fetch)
    assert_eq!(&(r3.media_type), &msg::MediaType::Json);
    let metadata = crate::http_cache::Metadata::read(&cache_filename).unwrap();
    assert_eq!(metadata.headers.get("content-type").unwrap(), "application/json");

    // let's create fresh instance of DenoDir (simulating another freshh Deno process)
    // and don't use cache
    let fetcher = setup_file_fetcher(temp_dir.path());
    let result4 = fetcher.get_source_file(&module_url_2, false, false, false, &Permissions::allow_all()).await;
  ...
  async fn test_get_source_code_2() {
    let _http_server_guard = test_util::http_server();
    let (temp_dir, fetcher) = test_setup();
    let module_url = Url::parse("http://localhost:4545/cli/tests/subdir/mismatch_ext.ts").unwrap();
    let module_url_1 = module_url.clone();
    ...
    let mut metadata = crate::http_cache::Metadata::read(&cache_filename).unwrap();
    metadata.headers = HashMap::new();
    metadata.headers.insert("content-type".to_string(), "text/typescript".to_string());
    metadata.write(&cache_filename).unwrap();

    let result2 = fetcher.get_source_file(&module_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result2.is_ok());
    let r2 = result2.unwrap();
    ...
    let metadata = crate::http_cache::Metadata::read(&cache_filename).unwrap();
    assert_eq!(metadata.headers.get("content-type").unwrap(), "text/typescript");

    // let's create fresh instance of DenoDir (simulating another fresh Deno
    // process) and don't use cache
    let fetcher = setup_file_fetcher(temp_dir.path());
    let result3 = fetcher.get_source_file(&module_url_1, false, false, false, &Permissions::allow_all()).await;
    assert!(result3.is_ok());
    let r3 = result3.unwrap();
  ...
  async fn test_get_source_code_multiple_downloads_of_same_file() {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let specifier = ModuleSpecifier::resolve_url("http://localhost:4545/cli/tests/subdir/mismatch_ext.ts").unwrap();
    let cache_filename = fetcher.http_cache.get_cache_filename(&specifier.as_url());

    // first download
    let r = fetcher.fetch_source_file(&specifier, None, Permissions::allow_all()).await;
    assert!(r.is_ok());

    let headers_file_name = crate::http_cache::Metadata::filename(&cache_filename);
  ...
  async fn test_get_source_code_3() {
    ...
    let redirect_module_url = Url::parse("http://localhost:4546/cli/tests/subdir/redirects/redirect1.js").unwrap();
    let redirect_source_filepath = fetcher.http_cache.get_cache_filename(&redirect_module_url);
    let redirect_source_filename = redirect_source_filepath.to_str().unwrap().to_string();
    let target_module_url = Url::parse("http://localhost:4545/cli/tests/subdir/redirects/redirect1.js").unwrap();
    let redirect_target_filepath = fetcher.http_cache.get_cache_filename(&target_module_url);
    let redirect_target_filename = redirect_target_filepath.to_str().unwrap().to_string();

    // Test basic follow and headers recording
    let result = fetcher.get_source_file(&redirect_module_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    ...
    assert_eq!(headers.get("location").unwrap(), "http://localhost:4545/cli/tests/subdir/redirects/redirect1.js");
    // The target of redirection is downloaded instead.
    assert_eq!(fs::read_to_string(&redirect_target_filename).unwrap(), "export const redirect = 1;\n");
  ...
  async fn test_get_source_code_4() {
    ...
    let double_redirect_path = fetcher.http_cache.get_cache_filename(&double_redirect_url);
    ...
    let target_url = Url::parse("http://localhost:4545/cli/tests/subdir/redirects/redirect1.js").unwrap();
    let target_path = fetcher.http_cache.get_cache_filename(&target_url);

    // Test double redirects and headers recording
    let result = fetcher.get_source_file(&double_redirect_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    let mod_meta = result.unwrap();
  ...
  async fn test_get_source_code_5() {
    ...
    let result = fetcher.get_source_file(&redirect_url, true, false, false, &Permissions::allow_all()).await;
    assert!(result.is_ok());
  ...
  async fn test_get_source_code_6() {
    ...
    let result = fetcher.fetch_remote_source(&double_redirect_url, false, false, 2, &Permissions::allow_all()).await;
    assert!(result.is_ok());

    let result = fetcher.fetch_remote_source(&double_redirect_url, false, false, 1, &Permissions::allow_all()).await;
  ...
  async fn test_get_source_code_7() {
    ...
    let redirect_module_url = Url::parse("http://localhost:4550/REDIRECT/cli/tests/subdir/redirects/redirect1.js").unwrap();
    let redirect_source_filepath = fetcher.http_cache.get_cache_filename(&redirect_module_url);
    let redirect_source_filename = redirect_source_filepath.to_str().unwrap().to_string();
    let target_module_url = Url::parse("http://localhost:4550/cli/tests/subdir/redirects/redirect1.js").unwrap();
    let redirect_target_filepath = fetcher.http_cache.get_cache_filename(&target_module_url);
    let redirect_target_filename = redirect_target_filepath.to_str().unwrap().to_string();
    ...
    assert_eq!(fs::read_to_string(&redirect_source_filename).unwrap(), "");
    let (_, headers) = fetcher.http_cache.get(&redirect_module_url).unwrap();
    assert_eq!(headers.get("location").unwrap(), "/cli/tests/subdir/redirects/redirect1.js");
    // The target of redirection is downloaded instead.
    assert_eq!(fs::read_to_string(&redirect_target_filename).unwrap(), "export const redirect = 1;\n");
    let (_, headers) = fetcher.http_cache.get(&target_module_url).unwrap();
    assert!(headers.get("location").is_none());
  ...
  async fn test_get_source_no_remote() {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = Url::parse("http://localhost:4545/cli/tests/002_hello.ts").unwrap();
    // Remote modules are not allowed
    let result = fetcher.get_source_file(&module_url, true, true, false, &Permissions::allow_all()).await;
    assert!(result.is_err());
    // FIXME(bartlomieju):
  ...
  async fn test_get_source_cached_only() {
    ...
    let result = fetcher.get_source_file(&module_url, true, false, true, &Permissions::allow_all()).await;
    assert!(result.is_err());
    // FIXME(bartlomieju):
    // let err = result.err().unwrap();
    // assert_eq!(err.kind(), ErrorKind::NotFound);
    ...
    let result = fetcher_1.get_source_file(&module_url_1, true, false, false, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    // module is already cached, should be ok even with `cached_only`
    let result = fetcher_2.get_source_file(&module_url_2, true, false, true, &Permissions::allow_all()).await;
    assert!(result.is_ok());
  }
  ...
  async fn test_fetch_source_0() {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = Url::parse("http://127.0.0.1:4545/cli/tests/subdir/mt_video_mp2t.t3.ts").unwrap();
    let result = fetcher.fetch_remote_source(&module_url, false, false, 10, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    let r = result.unwrap();
  ...
  async fn fetch_remote_source_no_ext() {
    ...
    let (_temp_dir, fetcher) = test_setup();
    let module_url = &Url::parse("http://localhost:4545/cli/tests/subdir/no_ext").unwrap();
    let result = fetcher.fetch_remote_source(module_url, false, false, 10, &Permissions::allow_all()).await;
    assert!(result.is_ok());
    let r = result.unwrap();
    assert_eq!(r.source_code.bytes, b"export const loaded = true;\n");
  ...
  async fn fetch_remote_source_unknown_ext() {
    let _g = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = &Url::parse("http://localhost:4545/cli/tests/subdir/unknown_ext.deno").unwrap();
    let result = fetcher.fetch_remote_source(module_url, false, false, 10, &Permissions::allow_all()).await;
    assert!(result.is_ok());
  ...
  async fn test_fetch_source_file() {
    ...
    // Test failure case.
    let specifier = ModuleSpecifier::resolve_url(file_url!("/baddir/hello.ts")).unwrap();
    let r = fetcher.fetch_source_file(&specifier, None, Permissions::allow_all()).await;
    assert!(r.is_err());

    let p = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("rt/99_main.js");
  ...
  async fn test_fetch_source_file_2() {
    ...
    let p = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/001_hello.js");
    let specifier = ModuleSpecifier::resolve_url_or_path(p.to_str().unwrap()).unwrap();
    let r = fetcher.fetch_source_file(&specifier, None, Permissions::allow_all()).await;
    assert!(r.is_ok());
  }
  ...
  fn test_resolve_module_3() {
    // unsupported schemes
    let test_cases = ["ftp://localhost:4545/testdata/subdir/print_hello.ts", "blob:https://whatwg.org/d0360e2f-caee-469f-9a2f-87d5b0456f6f"];
  ...
  async fn test_fetch_source_file_from_disk_nonstandard_encoding(charset: &str, expected_content: String) {
    let (_temp_dir, fetcher) = test_setup();

    let p = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(format!("tests/encoding/{}.ts", charset));
    let specifier = ModuleSpecifier::resolve_url_or_path(p.to_str().unwrap()).unwrap();
    let r = fetcher.fetch_source_file(&specifier, None, Permissions::allow_all()).await;
    assert!(r.is_ok());
  ...
  async fn test_fetch_source_file_from_disk_utf_16_be() {
    test_fetch_source_file_from_disk_nonstandard_encoding("utf-16be", String::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A".to_vec()).unwrap()).await;
  }

  #[tokio::test]
  async fn test_fetch_source_file_from_disk_utf_16_le() {
    test_fetch_source_file_from_disk_nonstandard_encoding("utf-16le", String::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A".to_vec()).unwrap()).await;
  }

  #[tokio::test]
  async fn test_fetch_source_file_from_disk_utf_8_with_bom() {
    test_fetch_source_file_from_disk_nonstandard_encoding("utf-8", String::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A".to_vec()).unwrap()).await;
  }

  #[test]
  fn test_map_file_extension() {
    assert_eq!(map_file_extension(Path::new("foo/bar.ts")), msg::MediaType::TypeScript);
    assert_eq!(map_file_extension(Path::new("foo/bar.tsx")), msg::MediaType::TSX);
    assert_eq!(map_file_extension(Path::new("foo/bar.d.ts")), msg::MediaType::TypeScript);
    assert_eq!(map_file_extension(Path::new("foo/bar.js")), msg::MediaType::JavaScript);
    assert_eq!(map_file_extension(Path::new("foo/bar.jsx")), msg::MediaType::JSX);
    assert_eq!(map_file_extension(Path::new("foo/bar.json")), msg::MediaType::Json);
    assert_eq!(map_file_extension(Path::new("foo/bar.wasm")), msg::MediaType::Wasm);
    assert_eq!(map_file_extension(Path::new("foo/bar.cjs")), msg::MediaType::JavaScript);
    assert_eq!(map_file_extension(Path::new("foo/bar.txt")), msg::MediaType::Unknown);
    assert_eq!(map_file_extension(Path::new("foo/bar")), msg::MediaType::Unknown);
  }

  #[test]
  fn test_map_content_type_extension_only() {
    // Extension only
    assert_eq!(map_content_type(Path::new("foo/bar.ts"), None).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), None).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.d.ts"), None).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar.js"), None).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar.txt"), None).0, msg::MediaType::Unknown);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), None).0, msg::MediaType::JSX);
    assert_eq!(map_content_type(Path::new("foo/bar.json"), None).0, msg::MediaType::Json);
    assert_eq!(map_content_type(Path::new("foo/bar.wasm"), None).0, msg::MediaType::Wasm);
    assert_eq!(map_content_type(Path::new("foo/bar.cjs"), None).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), None).0, msg::MediaType::Unknown);
  }

  #[test]
  fn test_map_content_type_media_type_with_no_extension() {
    // Media Type
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/typescript")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("text/typescript")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("video/vnd.dlna.mpeg-tts")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("video/mp2t")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/x-typescript")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/javascript")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("text/javascript")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/ecmascript")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("text/ecmascript")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/x-javascript")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/json")).0, msg::MediaType::Json);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("application/node")).0, msg::MediaType::JavaScript);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("text/json")).0, msg::MediaType::Json);
    assert_eq!(map_content_type(Path::new("foo/bar"), Some("text/json; charset=utf-8 ")), (msg::MediaType::Json, Some("utf-8".to_owned())));
  }

  #[test]
  fn test_map_file_extension_media_type_with_extension() {
    assert_eq!(map_content_type(Path::new("foo/bar.ts"), Some("text/plain")).0, msg::MediaType::TypeScript);
    assert_eq!(map_content_type(Path::new("foo/bar.ts"), Some("foo/bar")).0, msg::MediaType::Unknown);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), Some("application/typescript"),).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), Some("application/javascript"),).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), Some("application/x-typescript"),).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), Some("video/vnd.dlna.mpeg-tts"),).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.tsx"), Some("video/mp2t")).0, msg::MediaType::TSX);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), Some("application/javascript"),).0, msg::MediaType::JSX);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), Some("application/x-typescript"),).0, msg::MediaType::JSX);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), Some("application/ecmascript"),).0, msg::MediaType::JSX);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), Some("text/ecmascript")).0, msg::MediaType::JSX);
    assert_eq!(map_content_type(Path::new("foo/bar.jsx"), Some("application/x-javascript"),).0, msg::MediaType::JSX);
  }
  ...
  async fn test_fetch_with_etag() {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = Url::parse("http://127.0.0.1:4545/etag_script.ts").unwrap();

    let source = fetcher.fetch_remote_source(&module_url, false, false, 1, &Permissions::allow_all()).await;
    assert!(source.is_ok());
    ...
    let cached_source = fetcher.fetch_remote_source(&module_url, false, false, 1, &Permissions::allow_all()).await.unwrap();
  ...
  async fn test_fetch_with_types_header() {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = Url::parse("http://127.0.0.1:4545/xTypeScriptTypes.js").unwrap();
    let source = fetcher.fetch_remote_source(&module_url, false, false, 1, &Permissions::allow_all()).await;
    assert!(source.is_ok());
    let source = source.unwrap();
    assert_eq!(source.source_code.bytes, b"export const foo = 'foo';");
    assert_eq!(&(source.media_type), &msg::MediaType::JavaScript);
    assert_eq!(source.types_header, Some("./xTypeScriptTypes.d.ts".to_string()));
  }

  #[tokio::test]
  async fn test_fetch_source_file_from_net_utf16_le() {
    let content = std::str::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A").unwrap();
    test_fetch_non_utf8_source_file_from_net("utf-16le", "utf-16le.ts", content).await;
  }

  #[tokio::test]
  async fn test_fetch_source_file_from_net_utf16_be() {
    let content = std::str::from_utf8(b"\xEF\xBB\xBFconsole.log(\"Hello World\");\x0A").unwrap();
    test_fetch_non_utf8_source_file_from_net("utf-16be", "utf-16be.ts", content).await;
  }
  ...
  async fn test_fetch_source_file_from_net_windows_1255() {
    ...
    test_fetch_non_utf8_source_file_from_net("windows-1255", "windows-1255", content).await;
  }

  async fn test_fetch_non_utf8_source_file_from_net(charset: &str, file_name: &str, expected_content: &str) {
    let _http_server_guard = test_util::http_server();
    let (_temp_dir, fetcher) = test_setup();
    let module_url = Url::parse(&format!("http://127.0.0.1:4545/cli/tests/encoding/{}", file_name)).unwrap();
    ...
    let (_, headers) = fetcher.http_cache.get(&module_url).unwrap();
    assert_eq!(headers.get("content-type").unwrap(), &format!("application/typescript;charset={}", charset));
  }
}

...
fn write_deno_types(
  ...
  // types. Without these, stock tsc reports TS7016 ("implicitly has an 'any'
  // type") for a no-types dependency; Deno instead treats it as `any`. A
  // shorthand `declare module "x";` makes every import form (default/named/
  // namespace) resolve to `any`. The corresponding `paths` entry is omitted (see
  // generate_npm_paths) so tsc falls through to this ambient instead of loading
  // the package's untyped `.js`.
...
/// with no `types`/`exports.types` entry are shimmed; their `paths` entries are
/// skipped (see `generate_npm_paths`) so tsc uses these ambients.
fn no_types_npm_shims(
  ...
  for (alias, target) in imports {
    ...
    if !pkg_dir.exists()
      || resolve_package_types_entry_path(&pkg_dir, ".").is_some()
    {
      continue;
    }
    // No types: shim every import form (scheme, exact specifier, alias) and
    // their subpaths as `any`.
    ...
    for (alias, target) in imports {
      ...
      match npm_ref.sub_path() {
        ...
        None => {
          ...
          if resolve_package_types_entry_path(&pkg_dir, ".").is_none() {
            continue;
          }

...
// TODO: this is not working yet with "TypeError: expected typed ArrayBufferView" from
// somewhere in the Rust code
Deno.test("zlib compression with dataview", { sanitizeResources: false }, () => {
  const buf = Buffer.from("hello world");
...
Deno.test("zlib compression with an encoded string", { sanitizeResources: false }, () => {
  const encoder = new TextEncoder();
  const buffer = encoder.encode("hello world");

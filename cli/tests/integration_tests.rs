...
fn repl_test() {
  ...
  let repl_process = |lines: &Vec<&str>,
  ...
  let test_cases = vec![
		...
		(vec!["Deno.writeFileSync"], None, "[Function: writeFileSync]\n", "", 0),
		(vec!["(\n1 + 2\n)"], None, "3\n", "", 0),
		(vec!["eval('{')"], None, "", "[WILDCARD]Unexpected end of input[WILDCARD]", 0),
		(vec!["not_a_variable"], None, "", "[WILDCARD]not_a_variable is not defined[WILDCARD]", 0),
		(vec!["syntax error"], None, "", "[WILDCARD]Unexpected identifier[WILDCARD]", 0),
		(vec!["console()"], None, "", "[WILDCARD]console is not a function[WILDCARD]", 0),
		(vec!["console()"], None, "", "[WILDCARD]console is not a function[WILDCARD]", 0),
		(vec!["var a = 123;", "a"], None, "undefined\n123\n", "", 0),
		(vec!["let a = 123;", "a"], None, "undefined\n123\n", "", 0),
		(vec!["1"], Some(vec![("DENO_DIR".to_owned(), "nonexistent".to_owned())]), "1\n", "", 0),
		(vec!["1", "_"], None, "1\n1\n", "", 0),
		(vec!["throw 1", "_error"], None, "1\n", "Thrown: 1\n", 0),

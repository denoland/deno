...
fn task_subprocess_kill() {
  ...
  // give task some time to start up and get its pid
  let stdout = child.stdout.take().unwrap();
  let mut buf_reader = BufReader::new(stdout);
  let mut pid_string = String::new();
  buf_reader.read_line(&mut pid_string).unwrap();
  let pid = pid_string.trim().parse::<u32>().unwrap();
...
#[cfg(windows)]
fn kill_processes(pid: u32) -> bool {

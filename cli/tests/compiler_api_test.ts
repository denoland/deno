...
Deno.test({
  ...
  async fn() {
    ...
    assert(emitMap);
    assertEquals(Object.keys(emitMap), [
      "/bar.js.map",
      "/bar.js",
      "/foo.js.map",
      "/foo.js",
...
Deno.test({
  ...
  async fn() {
    const { diagnostics, emitMap } = await Deno.compile("./subdir/mod1.ts");
    assert(diagnostics == null);
    assert(emitMap);
    const keys = Object.keys(emitMap);
...
Deno.test({
  ...
  async fn() {
    ...
    assert(diagnostics == null);
    assert(emitMap);
    assertEquals(Object.keys(emitMap), ["/foo.js"]);
    assert(emitMap["/foo.js"].startsWith("define("));
  },
});
...
Deno.test({
  ...
  async fn() {
    ...
    assert(diagnostics == null);
    assert(emitMap);
    assertEquals(Object.keys(emitMap), ["/foo.js.map", "/foo.js"]);
  },
});
...
Deno.test({
  ...
  async fn() {
    ...
    assert(diagnostics == null);
    assert(emitMap);
    assertEquals(Object.keys(emitMap), ["lib/foo.js.map", "lib/foo.js"]);
  },
});
...
Deno.test({
  ...
  async fn() {
    const { diagnostics, output } = await Deno.bundle("/foo.ts", {
      "/foo.ts": `export * from "./bar.ts";\n`,
      "/bar.ts": `export const bar = "bar";\n`,
    });
    assert(diagnostics == null);
    assert(output.includes(`__instantiate("foo", false)`));
    assert(output.includes(`__exp["bar"]`));
  },
});
...
Deno.test({
  ...
  async fn() {
    const { diagnostics, output } = await Deno.bundle("./subdir/mod1.ts");
    assert(diagnostics == null);
    assert(output.includes(`__instantiate("mod1", false)`));
    assert(output.includes(`__exp["printHello3"]`));
  },
});
...
Deno.test({
  ...
  async fn() {
    const { diagnostics, output } = await Deno.bundle(
      "/foo.ts",
      {
...
Deno.test({
  ...
  async fn() {
    const { diagnostics, output } = await Deno.bundle("/foo.js", {
      "/foo.js": `export * from "./bar.js";\n`,
      "/bar.js": `export const bar = "bar";\n`,
    });
    assert(diagnostics == null);
    assert(output.includes(`System.register("bar",`));
  },
});
...
Deno.test({
  ...
  async fn() {
    ...
    assert(diagnostics == null);
    assert(output.includes(`var __awaiter = `));
  },
});

...
unitTest(
    ...
    async function flockFileSync() {
      ...
      `;
      const run = (e: boolean, w: number) => Deno.run({ cmd: [Deno.execPath(), "eval", "--unstable", script(e, w)] });
      const firstBlocksSecond = async (first: boolean, second: boolean): Promise<boolean> => {
      ...
      assertEquals(await firstBlocksSecond(true, false), true, "exclusive blocks shared");
      assertEquals(await firstBlocksSecond(false, true), true, "shared blocks exclusive");

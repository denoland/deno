...
pub fn apply_source_map<G: SourceMapGetter>(js_error: &deno_core::JSError, getter: &G) -> deno_core::JSError {
  ...
  let source_line = match line_number {
    Some(ln) if js_error.source_line.is_some() && script_resource_name.is_some() => {
  ...
  deno_core::JSError { message: js_error.message.clone(), source_line, script_resource_name, line_number, start_column, end_column, frames: js_error.frames.clone(), formatted_frames: js_error.formatted_frames.clone() }
}

fn get_maybe_orig_position<G: SourceMapGetter>(file_name: Option<String>, line_number: Option<i64>, column_number: Option<i64>, mappings_map: &mut CachedMaps, getter: &G) -> (Option<String>, Option<i64>, Option<i64>) {
  match (file_name, line_number, column_number) {
    (Some(file_name_v), Some(line_v), Some(column_v)) => {
      let (file_name, line_number, column_number) = get_orig_position(file_name_v, line_v, column_v, mappings_map, getter);
...
pub fn get_orig_position<G: SourceMapGetter>(file_name: String, line_number: i64, column_number: i64, mappings_map: &mut CachedMaps, getter: &G) -> (String, i64, i64) {
  ...
  match maybe_source_map {
    None => default_pos,
    Some(source_map) => match source_map.lookup_token(line_number as u32, column_number as u32) {
      None => default_pos,
      Some(token) => match token.get_source() {
        None => default_pos,
        Some(original) => (original.to_string(), i64::from(token.get_src_line()) + 1, i64::from(token.get_src_col()) + 1),
      },
    },
  }
}

fn get_mappings<'a, G: SourceMapGetter>(file_name: &str, mappings_map: &'a mut CachedMaps, getter: &G) -> &'a Option<SourceMap> {
  mappings_map.entry(file_name.to_string()).or_insert_with(|| parse_map_string(file_name, getter))
}

// TODO(kitsonk) parsed source maps should probably be cached in state in
// the module meta data.
fn parse_map_string<G: SourceMapGetter>(file_name: &str, getter: &G) -> Option<SourceMap> {
  getter.get_source_map(file_name).and_then(|raw_source_map| SourceMap::from_slice(&raw_source_map).ok())
}
...
mod tests {
  ...
  impl SourceMapGetter for MockSourceMapGetter {
    ...
    fn get_source_line(&self, file_name: &str, line_number: usize) -> Option<String> {
      let s = match file_name {
        "foo_bar.ts" => vec!["console.log('foo');", "console.log('foo');", "console.log('foo');", "console.log('foo');", "console.log('foo');"],
        _ => return None,
      };
  ...
  fn apply_source_map_line() {
    let e = deno_core::JSError { message: "TypeError: baz".to_string(), source_line: Some("foo".to_string()), script_resource_name: Some("foo_bar.ts".to_string()), line_number: Some(4), start_column: Some(16), end_column: None, frames: vec![], formatted_frames: vec![] };

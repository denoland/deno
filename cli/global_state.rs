...
impl GlobalState {
  pub fn new(flags: flags::Flags) -> Result<Arc<Self>, ErrBox> {
    ...
    let file_fetcher = SourceFileFetcher::new(http_cache, !flags.reload, flags.cache_blocklist.clone(), flags.no_remote, flags.cached_only, ca_file.as_deref())?;

    let ts_compiler = TsCompiler::new(file_fetcher.clone(), flags.clone(), dir.gen_cache.clone())?;
    ...
    let maybe_import_map: Option<ImportMap> = match flags.import_map_path.as_ref() {
      None => None,
      Some(file_path) => {
        if !flags.unstable {
    ...
    let global_state = GlobalState { dir, permissions: Permissions::from_flags(&flags), flags, file_fetcher, ts_compiler, lockfile, maybe_import_map, compiler_starts: AtomicUsize::new(0), compile_lock: AsyncMutex::new(()) };
  ...
  /// and traspilation.
  pub async fn prepare_module_load(self: &Arc<Self>, module_specifier: ModuleSpecifier, maybe_referrer: Option<ModuleSpecifier>, target_lib: TargetLib, permissions: Permissions, is_dyn_import: bool, maybe_import_map: Option<ImportMap>) -> Result<(), ErrBox> {
    ...
    let mut module_graph_loader = ModuleGraphLoader::new(self.file_fetcher.clone(), maybe_import_map, permissions.clone(), is_dyn_import, false);
    module_graph_loader.add_to_graph(&module_specifier, maybe_referrer).await?;
    let module_graph = module_graph_loader.get_graph();

    let out = self.file_fetcher.fetch_cached_source_file(&module_specifier, permissions.clone()).expect("Source file not found");
    ...
    let should_compile = needs_compilation(self.ts_compiler.compile_js, out.media_type, &module_graph_files);
    ...
    if should_compile {
      if self.flags.no_check {
        ...
        self.ts_compiler.compile(self, &out, target_lib, permissions, module_graph, allow_js).await?;
      }
    }
  ...
  pub async fn fetch_compiled_module(&self, module_specifier: ModuleSpecifier, _maybe_referrer: Option<ModuleSpecifier>) -> Result<CompiledModule, ErrBox> {
    let module_specifier = module_specifier.clone();

    let out = self.file_fetcher.fetch_cached_source_file(&module_specifier, Permissions::allow_all()).expect("Cached source file doesn't exist");
    ...
    let was_compiled = match out.media_type {
      msg::MediaType::TypeScript | msg::MediaType::TSX | msg::MediaType::JSX => true,
    ...
    let compiled_module = if was_compiled {
      match self.ts_compiler.get_compiled_module(&out.url) {
        ...
        Err(e) => {
          ...
          CompiledModule { code: "".to_string(), name: out.url.to_string() }
        }
      }
    } else {
      CompiledModule { code: out.source_code.to_string()?, name: out.url.to_string() }
    };
  ...
  #[cfg(test)]
  pub fn mock(argv: Vec<String>, maybe_flags: Option<flags::Flags>) -> Arc<GlobalState> {
    GlobalState::new(flags::Flags { argv, ..maybe_flags.unwrap_or_default() }).unwrap()
  }
}
  ...
  module_graph_files.iter().any(|module_file| {
    if module_file.media_type == MediaType::JSX {
      ...
      module_file.imports.iter().any(|import_desc| {
        let import_file = module_graph_files.iter().find(|f| f.specifier == import_desc.resolved_specifier.to_string().as_str()).expect("Failed to find imported file");
        let media_type = import_file.media_type;
        media_type == MediaType::TypeScript || media_type == MediaType::TSX || media_type == MediaType::JSX
      })
    } else {
      false
    }
...
fn needs_compilation(compile_js: bool, media_type: MediaType, module_graph_files: &[&ModuleGraphFile]) -> bool {
...
fn test_needs_compilation() {
  ...
  assert!(needs_compilation(false, MediaType::JavaScript, &[&ModuleGraphFile { specifier: "file:///some/file.ts".to_string(), url: "file:///some/file.ts".to_string(), redirect: None, filename: "some/file.ts".to_string(), imports: vec![], referenced_files: vec![], lib_directives: vec![], types_directives: vec![], type_headers: vec![], media_type: MediaType::TypeScript, version_hash: "1".to_string(), source_code: "function foo() {}".to_string() }, &ModuleGraphFile { specifier: "file:///some/file1.js".to_string(), url: "file:///some/file1.js".to_string(), redirect: None, filename: "some/file1.js".to_string(), imports: vec![], referenced_files: vec![], lib_directives: vec![], types_directives: vec![], type_headers: vec![], version_hash: "1".to_string(), media_type: MediaType::JavaScript, source_code: "function foo() {}".to_string() },],));
}

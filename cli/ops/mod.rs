...
impl DenoOpDispatcher for JsonOp {
  fn dispatch(
    &self,
    ...
    let json_args: JsonOpArgs = serde_json::from_slice(control).unwrap();
    let op_id = json_args.op_id;

    match op_id {
      OP_EXIT => {
        dispatch_json::dispatch(os::op_exit, state, control, zero_copy)
      }
      OP_IS_TTY => {
      ...
      OP_REPL_READLINE => dispatch_json::dispatch(
      ...
      OP_DIAL => {
        dispatch_json::dispatch(net::op_dial, state, control, zero_copy)
      }
      OP_SHUTDOWN => {
      ...
      OP_RUN_STATUS => dispatch_json::dispatch(
        process::op_run_status,
        state,
        control,
        zero_copy,
      ),
      ...
      OP_STAT => {
        dispatch_json::dispatch(fs::op_stat, state, control, zero_copy)
      }
      OP_READ_DIR => {
      ...
      OP_LINK => {
        dispatch_json::dispatch(fs::op_link, state, control, zero_copy)
      }
      OP_SYMLINK => {
...
pub fn setup_dispatcher_registry(state: ThreadSafeState) -> Arc<OpDisReg> {
  ...
  registry.register_op(OP_NAMESPACE, state.wrap_op(JsonOp));

...
pub fn check_unstable(state: &OpState, api_name: &str) {
  state.borrow::<Arc<CliState>>().check_unstable(api_name)
}
...
pub fn global_state(state: &OpState) -> Arc<CliState> {
  state.borrow::<Arc<CliState>>().clone()
}

/// Helper for extracting the commonly used state. Used for async ops.
pub fn global_state2(state: &Rc<RefCell<OpState>>) -> Arc<CliState> {
  let state = state.borrow();
  state.borrow::<Arc<CliState>>().clone()
}

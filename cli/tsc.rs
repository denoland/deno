...
impl CompilerWorker {
  ...
  pub fn get_response(&mut self) -> String {
    ...
    assert!(maybe_response.is_some(), "Unexpected missing response from TS compiler");
...
fn warn_ignored_options(maybe_ignored_options: Option<tsc_config::IgnoredCompilerOptions>, config_path: &Path) {
  if let Some(ignored_options) = maybe_ignored_options {
    eprintln!("Unsupported compiler options in \"{}\"\n  The following options were ignored:\n    {}", config_path.to_string_lossy(), ignored_options);
  }
}

/// Create a new worker with snapshot of TS compiler and setup compiler's
/// runtime.
fn create_compiler_worker(global_state: &Arc<GlobalState>, permissions: Permissions) -> CompilerWorker {
  // TODO(bartlomieju): these $deno$ specifiers should be unified for all subcommands
  // like 'eval', 'repl'
  let entry_point = ModuleSpecifier::resolve_url_or_path("./__$deno$ts_compiler.ts").unwrap();
  let worker_state = State::new(&global_state, Some(permissions), entry_point, None, true).expect("Unable to create worker state");

  // TODO(bartlomieju): this metric is never used anywhere
  // Count how many times we start the compiler worker.
  global_state.compiler_starts.fetch_add(1, Ordering::SeqCst);

  let mut worker = CompilerWorker::new("TS".to_string(), startup_data::compiler_isolate_init(), &worker_state);
  worker.execute("globalThis.bootstrapCompilerRuntime()").unwrap();
  worker
}
...
impl CompilerConfig {
  ...
  pub fn load(maybe_config_path: Option<String>) -> Result<Self, ErrBox> {
    if maybe_config_path.is_none() {
      return Ok(Self { path: Some(PathBuf::new()), options: json!({}), maybe_ignored_options: None, hash: "".to_string(), compile_js: false });
    }
    ...
    let config_path = config_file.canonicalize().map_err(|_| io::Error::new(io::ErrorKind::InvalidInput, format!("Could not find the config file: {}", config_file.to_string_lossy())))?;
    ...
    let (options, maybe_ignored_options) = if config_str.is_empty() { (json!({}), None) } else { tsc_config::parse_config(&config_str)? };
    ...
    Ok(Self { path: Some(config_path), options, maybe_ignored_options, hash: config_hash, compile_js })
  }
}
...
impl CompiledFileMetadata {
  pub fn from_json_string(metadata_string: String) -> Result<Self, serde_json::Error> {
...
fn source_code_version_hash(source_code: &[u8], version: &str, config_hash: &[u8]) -> String {
...
impl TsCompiler {
  pub fn new(file_fetcher: SourceFileFetcher, flags: Flags, disk_cache: DiskCache) -> Result<Self, ErrBox> {
    let config = CompilerConfig::load(flags.config_path.clone())?;
    let use_disk_cache = !flags.reload;

    Ok(TsCompiler(Arc::new(TsCompilerInner { file_fetcher, flags, disk_cache, compile_js: config.compile_js, config, compiled: Mutex::new(HashSet::new()), use_disk_cache })))
  }
  ...
  fn has_compiled_source(&self, url: &Url) -> bool {
    ...
    if let Some(source_file) = self.file_fetcher.fetch_cached_source_file(&specifier, Permissions::allow_all()) {
      if let Some(metadata) = self.get_metadata(&url) {
        // Compare version hashes
        let version_hash_to_validate = source_code_version_hash(&source_file.source_code.as_bytes(), version::DENO, &self.config.hash.as_bytes());
  ...
  fn has_valid_cache(&self, url: &Url, build_info: &Option<String>) -> Result<bool, ErrBox> {
    if let Some(build_info_str) = build_info.as_ref() {
      ...
      for (filename, file_info) in file_infos.iter() {
        ...
        if let Some(source_file) = self.file_fetcher.fetch_cached_source_file(&specifier, Permissions::allow_all()) {
          let existing_hash = crate::checksum::gen(&[&source_file.source_code.as_bytes(), version::DENO.as_bytes()]);
          let expected_hash = file_info["version"].as_str().unwrap().to_string();
  ...
  /// compiler.
  pub async fn compile(&self, global_state: &Arc<GlobalState>, source_file: &SourceFile, target: TargetLib, permissions: Permissions, module_graph: ModuleGraph, allow_js: bool) -> Result<(), ErrBox> {
    let module_url = source_file.url.clone();
    let build_info_key = self.disk_cache.get_cache_filename_with_extension(&module_url, "buildinfo");
    ...
    if (self.use_disk_cache || self.has_compiled(&source_file.url)) && self.has_valid_cache(&source_file.url, &build_info)? {
      return Ok(());
    }

    let module_graph_json = serde_json::to_value(module_graph).expect("Failed to serialize data");
    ...
    let mut lib = if target == "main" { vec!["deno.window"] } else { vec!["deno.worker"] };
    ...
    warn_ignored_options(compiler_config.maybe_ignored_options, compiler_config.path.as_ref().unwrap());
    ...
    let json_str = execute_in_same_thread(global_state, permissions, req_msg).await?;
  ...
  pub async fn bundle(&self, global_state: &Arc<GlobalState>, module_specifier: ModuleSpecifier) -> Result<String, ErrBox> {
    debug!("Invoking the compiler to bundle. module_name: {}", module_specifier.to_string());

    let permissions = Permissions::allow_all();
    let mut module_graph_loader = ModuleGraphLoader::new(self.file_fetcher.clone(), global_state.maybe_import_map.clone(), permissions.clone(), false, true);
    module_graph_loader.add_to_graph(&module_specifier, None).await?;
    ...
    if let Some(ref lockfile) = global_state.lockfile {
      ...
      for graph_file in &module_graph_files {
        let check_passed = g.check_or_insert(&graph_file.url, &graph_file.source_code);

        if !check_passed {
          eprintln!("Subresource integrity check failed --lock={}\n{}", g.filename, graph_file.url);
          std::process::exit(10);
        }
      }
    }
    ...
    let target = "main";
    let performance = matches!(global_state.flags.log_level, Some(Level::Debug));
    let unstable = self.flags.unstable;
  ...
  pub async fn transpile(&self, module_graph: ModuleGraph) -> Result<(), ErrBox> {
    ...
    for (_, value) in module_graph.iter() {
      ...
      if !value.url.ends_with(".d.ts") && (!self.use_disk_cache || !self.has_compiled_source(&url)) {
        source_files.push(TranspileSourceFile { source_code: value.source_code.clone(), file_name: value.url.clone() });
      }
    }
    ...
    let compiler_options: TranspileTsOptions = serde_json::from_value(compiler_options)?;

    let transpile_options = swc_util::EmitTranspileOptions { emit_metadata: compiler_options.emit_decorator_metadata, inline_source_map: true, jsx_factory: compiler_options.jsx_factory, jsx_fragment_factory: compiler_options.jsx_fragment_factory, transform_jsx: compiler_options.jsx == "react" };
    for source_file in source_files {
      let (stripped_source, _maybe_source_map) = swc_util::transpile(&source_file.file_name, MediaType::TypeScript, &source_file.source_code, &transpile_options)?;

      // TODO(bartlomieju): this is superfluous, just to make caching function happy
      let emitted_filename = PathBuf::from(&source_file.file_name).with_extension("js").to_string_lossy().to_string();
      let emitted_source = EmittedSource { filename: source_file.file_name.to_string(), contents: stripped_source };
  ...
  fn get_metadata(&self, url: &Url) -> Option<CompiledFileMetadata> {
    ...
    if let Ok(metadata_bytes) = self.disk_cache.get(&cache_key) {
      if let Ok(metadata) = std::str::from_utf8(&metadata_bytes) {
        if let Ok(read_metadata) = CompiledFileMetadata::from_json_string(metadata.to_string()) {
  ...
  fn cache_build_info(&self, url: &Url, build_info: String) -> std::io::Result<()> {
    let js_key = self.disk_cache.get_cache_filename_with_extension(url, "buildinfo");
    self.disk_cache.set(&js_key, build_info.as_bytes())?;

    Ok(())
  }

  fn cache_emitted_files(&self, emit_map: HashMap<String, EmittedSource>) -> std::io::Result<()> {
    for (emitted_name, source) in emit_map.iter() {
      let specifier = ModuleSpecifier::resolve_url(&source.filename).expect("Should be a valid module specifier");

      let source_file = self.file_fetcher.fetch_cached_source_file(&specifier, Permissions::allow_all()).expect("Source file not found");
  ...
  pub fn get_compiled_module(&self, module_url: &Url) -> Result<CompiledModule, ErrBox> {
    let compiled_source_file = self.get_compiled_source_file(module_url)?;

    let compiled_module = CompiledModule { code: compiled_source_file.source_code.to_string()?, name: module_url.to_string() };
  ...
  pub fn get_compiled_source_file(&self, module_url: &Url) -> Result<SourceFile, ErrBox> {
    ...
    let compiled_module = SourceFile { url: module_url.clone(), filename: compiled_code_filename, media_type: msg::MediaType::JavaScript, source_code: compiled_code.into(), types_header: None };
  ...
  fn cache_compiled_file(&self, module_specifier: &ModuleSpecifier, source_file: SourceFile, contents: &str) -> std::io::Result<()> {
    let js_key = self.disk_cache.get_cache_filename_with_extension(module_specifier.as_url(), "js");
    self.disk_cache.set(&js_key, contents.as_bytes())?;
    self.mark_compiled(module_specifier.as_url());

    let version_hash = source_code_version_hash(&source_file.source_code.as_bytes(), version::DENO, &self.config.hash.as_bytes());

    let compiled_file_metadata = CompiledFileMetadata { version_hash };
    let meta_key = self.disk_cache.get_cache_filename_with_extension(module_specifier.as_url(), "meta");
    self.disk_cache.set(&meta_key, compiled_file_metadata.to_json_string()?.as_bytes())
  }
  ...
  // SourceFileFetcher
  pub fn get_source_map_file(&self, module_specifier: &ModuleSpecifier) -> Result<SourceFile, ErrBox> {
    let cache_key = self.disk_cache.get_cache_filename_with_extension(module_specifier.as_url(), "js.map");
    ...
    let source_map_file = SourceFile { url: module_specifier.as_url().to_owned(), filename: source_map_filename, media_type: msg::MediaType::JavaScript, source_code: source_code.into(), types_header: None };

    Ok(source_map_file)
  }

  /// Save source map file for given TS module to on-disk cache.
  fn cache_source_map(&self, module_specifier: &ModuleSpecifier, contents: &str) -> std::io::Result<()> {
    let js_key = self.disk_cache.get_cache_filename_with_extension(module_specifier.as_url(), "js");
    let js_path = self.disk_cache.location.join(js_key);
    let js_file_url = Url::from_file_path(js_path).expect("Bad file URL for file");

    let source_map_key = self.disk_cache.get_cache_filename_with_extension(module_specifier.as_url(), "js.map");

    let mut sm = SourceMap::from_slice(contents.as_bytes()).expect("Invalid source map content");
    sm.set_file(Some(&js_file_url.to_string()));
    sm.set_source(0, &module_specifier.to_string());

    let mut output: Vec<u8> = vec![];
    sm.to_writer(&mut output).expect("Failed to write source map");
...
impl TsCompiler {
  ...
  fn try_resolve_and_get_source_file(&self, script_name: &str) -> Option<SourceFile> {
    if let Some(module_specifier) = self.try_to_resolve(script_name) {
      return self.file_fetcher.fetch_cached_source_file(&module_specifier, Permissions::allow_all());
    }

    None
  }

  fn try_to_resolve_and_get_source_map(&self, script_name: &str) -> Option<Vec<u8>> {
    if let Some(module_specifier) = self.try_to_resolve(script_name) {
      return match self.get_source_map_file(&module_specifier) {
        Ok(out) => Some(out.source_code.into_bytes()),
        Err(_) => {
          // Check if map is inlined
          if let Ok(compiled_source) = self.get_compiled_module(module_specifier.as_url()) {
            let mut content_lines = compiled_source.code.split('\n').map(|s| s.to_string()).collect::<Vec<String>>();

            if !content_lines.is_empty() {
              let last_line = content_lines.pop().unwrap();
              if last_line.starts_with("//# sourceMappingURL=data:application/json;base64,") {
                let encoded = last_line.trim_start_matches("//# sourceMappingURL=data:application/json;base64,");
                let decoded_map = base64::decode(encoded).expect("failed to parse source map");
...
async fn execute_in_same_thread(global_state: &Arc<GlobalState>, permissions: Permissions, req: String) -> Result<String, ErrBox> {
...
async fn create_runtime_module_graph(global_state: &Arc<GlobalState>, permissions: Permissions, root_name: &str, sources: &Option<HashMap<String, String>>, type_files: Vec<String>) -> Result<(Vec<String>, ModuleGraph), ErrBox> {
  let mut root_names = vec![];
  let mut module_graph_loader = ModuleGraphLoader::new(global_state.file_fetcher.clone(), None, permissions, false, false);
  ...
  for type_file in type_files {
    ...
    module_graph_loader.add_to_graph(&type_specifier, None).await?;
...
pub async fn runtime_compile(global_state: &Arc<GlobalState>, permissions: Permissions, root_name: &str, sources: &Option<HashMap<String, String>>, maybe_options: &Option<String>) -> Result<Value, ErrBox> {
  let mut user_options = if let Some(options) = maybe_options { tsc_config::parse_raw_config(options)? } else { json!({}) };

  // Intentionally calling "take()" to replace value with `null` - otherwise TSC will try to load that file
  // using `fileExists` API
  let type_files = if let Some(types) = user_options["types"].take().as_array() { types.iter().map(|type_value| type_value.as_str().unwrap_or("").to_string()).filter(|type_str| !type_str.is_empty()).collect() } else { vec![] };

  let unstable = global_state.flags.unstable;

  let mut lib = vec![];
  if let Some(user_libs) = user_options["lib"].take().as_array() {
    let libs = user_libs.iter().map(|type_value| type_value.as_str().unwrap_or("").to_string()).filter(|type_str| !type_str.is_empty()).collect::<Vec<String>>();
    lib.extend(libs);
  } else {
  ...
  let (root_names, module_graph) = create_runtime_module_graph(&global_state, permissions.clone(), root_name, sources, type_files).await?;
  let module_graph_json = serde_json::to_value(module_graph).expect("Failed to serialize data");
  ...
  let json_str = execute_in_same_thread(global_state, permissions, req_msg).await.map_err(js_error_to_errbox)?;
...
pub async fn runtime_bundle(global_state: &Arc<GlobalState>, permissions: Permissions, root_name: &str, sources: &Option<HashMap<String, String>>, maybe_options: &Option<String>) -> Result<Value, ErrBox> {
  let mut user_options = if let Some(options) = maybe_options { tsc_config::parse_raw_config(options)? } else { json!({}) };
...
pub async fn runtime_transpile(global_state: &Arc<GlobalState>, permissions: Permissions, sources: &HashMap<String, String>, maybe_options: &Option<String>) -> Result<Value, ErrBox> {
  let user_options = if let Some(options) = maybe_options { tsc_config::parse_raw_config(options)? } else { json!({}) };
  ...
  let json_str = execute_in_same_thread(global_state, permissions, req_msg).await.map_err(js_error_to_errbox)?;
  let v = serde_json::from_str::<serde_json::Value>(&json_str).expect("Error decoding JSON string.");
  Ok(v)
}
...
impl Visit for DependencyVisitor {
  fn visit_import_decl(&mut self, import_decl: &swc_ecmascript::ast::ImportDecl, _parent: &dyn Node) {
    let src_str = import_decl.src.value.to_string();
    self.dependencies.push(DependencyDescriptor { specifier: src_str, kind: DependencyKind::Import, span: import_decl.span });
  }

  fn visit_named_export(&mut self, named_export: &swc_ecmascript::ast::NamedExport, _parent: &dyn Node) {
    if let Some(src) = &named_export.src {
      let src_str = src.value.to_string();
      self.dependencies.push(DependencyDescriptor { specifier: src_str, kind: DependencyKind::Export, span: named_export.span });
    }
  }

  fn visit_export_all(&mut self, export_all: &swc_ecmascript::ast::ExportAll, _parent: &dyn Node) {
    let src_str = export_all.src.value.to_string();
    self.dependencies.push(DependencyDescriptor { specifier: src_str, kind: DependencyKind::Export, span: export_all.span });
  }

  fn visit_ts_import_type(&mut self, ts_import_type: &swc_ecmascript::ast::TsImportType, _parent: &dyn Node) {
    // TODO(bartlomieju): possibly add separate DependencyKind
    let src_str = ts_import_type.arg.value.to_string();
    self.dependencies.push(DependencyDescriptor { specifier: src_str, kind: DependencyKind::Import, span: ts_import_type.arg.span });
  }

  fn visit_call_expr(&mut self, call_expr: &swc_ecmascript::ast::CallExpr, parent: &dyn Node) {
      ...
      match &*arg.expr {
        Lit(lit) => {
          if let swc_ecmascript::ast::Lit::Str(str_) = lit {
            ...
            self.dependencies.push(DependencyDescriptor { specifier: src_str, kind: DependencyKind::DynamicImport, span: call_expr.span });
          }
        }
        _ => return,
      }
...
pub fn pre_process_file(file_name: &str, media_type: MediaType, source_code: &str, analyze_dynamic_imports: bool) -> Result<(Vec<ImportDesc>, Vec<TsReferenceDesc>), SwcDiagnosticBuffer> {
  let parser = AstParser::default();
  let parse_result = parser.parse_module(file_name, media_type, source_code);
  let module = parse_result?;
  let mut collector = DependencyVisitor { dependencies: vec![] };
  ...
  let imports = dependency_descriptors
    ...
    .map(|desc| {
      ...
      ImportDesc { specifier: desc.specifier.to_string(), deno_types, location: location.into() }
    })
    .collect();

  // analyze comment from beginning of the file and find TS directives
  let comments = parser.comments.with_leading(module_span.lo(), |cmts| cmts.to_vec());
  ...
  for comment in comments {
    ...
    if let Some((kind, specifier)) = parse_ts_reference(text.trim()) {
      ...
      references.push(TsReferenceDesc { kind, specifier, location: location.into() });
    }
  }
...
fn parse_ts_reference(comment: &str) -> Option<(TsReferenceKind, String)> {
  ...
  let (kind, specifier) = if let Some(capture_groups) = PATH_REFERENCE_RE.captures(comment) {
...
fn parse_deno_types(comment: &str) -> Option<String> {
  if let Some(capture_groups) = DENO_TYPES_RE.captures(comment) {
    if let Some(specifier) = capture_groups.get(1) {
      let s = specifier.as_str().trim_start_matches('\"').trim_start_matches('\'').trim_end_matches('\"').trim_end_matches('\'').to_string();
      return Some(s);
    }
  }
...
mod tests {
  ...
  fn test_parse_deno_types() {
    assert_eq!(parse_deno_types("@deno-types=./a/b/c.d.ts"), Some("./a/b/c.d.ts".to_string()));
    assert_eq!(parse_deno_types("@deno-types=\"./a/b/c.d.ts\""), Some("./a/b/c.d.ts".to_string()));
    assert_eq!(parse_deno_types("@deno-types = https://dneo.land/x/some/package/a.d.ts"), Some("https://dneo.land/x/some/package/a.d.ts".to_string()));
    assert_eq!(parse_deno_types("@deno-types = ./a/b/c.d.ts"), Some("./a/b/c.d.ts".to_string()));
    assert!(parse_deno_types("asdf").is_none());
    assert!(parse_deno_types("// deno-types = fooo").is_none());
    assert_eq!(parse_deno_types("@deno-types=./a/b/c.d.ts some comment"), Some("./a/b/c.d.ts".to_string()));
    assert_eq!(parse_deno_types("@deno-types=./a/b/c.d.ts // some comment after slashes"), Some("./a/b/c.d.ts".to_string()));
  }

  #[test]
  fn test_parse_ts_reference() {
    assert_eq!(parse_ts_reference(r#"/ <reference lib="deno.shared_globals" />"#), Some((TsReferenceKind::Lib, "deno.shared_globals".to_string())));
    assert_eq!(parse_ts_reference(r#"/ <reference path="./type/reference/dep.ts" />"#), Some((TsReferenceKind::Path, "./type/reference/dep.ts".to_string())));
    assert_eq!(parse_ts_reference(r#"/ <reference types="./type/reference.d.ts" />"#), Some((TsReferenceKind::Types, "./type/reference.d.ts".to_string())));
    assert!(parse_ts_reference("asdf").is_none());
    assert!(parse_ts_reference(r#"/ <reference unknown="unknown" />"#).is_none());
  }
  ...
  async fn test_compile() {
    ...
    let out = SourceFile { url: specifier.as_url().clone(), filename: PathBuf::from(p.to_str().unwrap().to_string()), media_type: msg::MediaType::TypeScript, source_code: include_bytes!("./tests/002_hello.ts").to_vec().into(), types_header: None };
    let dir = deno_dir::DenoDir::new(Some(test_util::new_deno_dir().path().to_owned())).unwrap();
    let http_cache = http_cache::HttpCache::new(&dir.root.join("deps"));
    let mock_state = GlobalState::mock(vec![String::from("deno"), String::from("hello.ts")], None);
    let file_fetcher = SourceFileFetcher::new(http_cache, true, mock_state.flags.cache_blocklist.clone(), false, false, None).unwrap();

    let mut module_graph_loader = ModuleGraphLoader::new(file_fetcher.clone(), None, Permissions::allow_all(), false, false);
    module_graph_loader.add_to_graph(&specifier, None).await.expect("Failed to create graph");
    let module_graph = module_graph_loader.get_graph();

    let ts_compiler = TsCompiler::new(file_fetcher, mock_state.flags.clone(), dir.gen_cache.clone()).unwrap();

    let result = ts_compiler.compile(&mock_state, &out, TargetLib::Main, Permissions::allow_all(), module_graph, false).await;
    assert!(result.is_ok());
    let compiled_file = ts_compiler.get_compiled_module(&out.url).unwrap();
    let source_code = compiled_file.code;
    assert!(source_code.as_bytes().starts_with(b"\"use strict\";\nconsole.log(\"Hello World\");"));
    let mut lines: Vec<String> = source_code.split('\n').map(|s| s.to_string()).collect();
    let last_line = lines.pop().unwrap();
    assert!(last_line.starts_with("//# sourceMappingURL=data:application/json;base64"));
  }
  ...
  async fn test_transpile() {
    ...
    let compiled_file = ts_compiler.get_compiled_module(&out.url).unwrap();
    let source_code = compiled_file.code;
    assert!(source_code.as_bytes().starts_with(b"console.log(\"Hello World\");"));
    let mut lines: Vec<String> = source_code.split('\n').map(|s| s.to_string()).collect();
    let last_line = lines.pop().unwrap();
    assert!(last_line.starts_with("//# sourceMappingURL=data:application/json;base64"));
  }
  ...
  async fn test_bundle() {
    let p = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).parent().unwrap().join("cli/tests/002_hello.ts");
    use deno_core::ModuleSpecifier;
    let module_name = ModuleSpecifier::resolve_url_or_path(p.to_str().unwrap()).unwrap();

    let mock_state = GlobalState::mock(vec![String::from("deno"), p.to_string_lossy().into(), String::from("$deno$/bundle.js")], None);

    let result = mock_state.ts_compiler.bundle(&mock_state, module_name).await;
  ...
  fn test_source_code_version_hash() {
    assert_eq!("0185b42de0686b4c93c314daaa8dee159f768a9e9a336c2a5e3d5b8ca6c4208c", source_code_version_hash(b"1+2", "0.4.0", b"{}"));
    // Different source_code should result in different hash.
    assert_eq!("e58631f1b6b6ce2b300b133ec2ad16a8a5ba6b7ecf812a8c06e59056638571ac", source_code_version_hash(b"1", "0.4.0", b"{}"));
    // Different version should result in different hash.
    assert_eq!("307e6200347a88dbbada453102deb91c12939c65494e987d2d8978f6609b5633", source_code_version_hash(b"1", "0.1.0", b"{}"));
    // Different config should result in different hash.
    assert_eq!("195eaf104a591d1d7f69fc169c60a41959c2b7a21373cd23a8f675f877ec385f", source_code_version_hash(b"1", "0.4.0", b"{\"compilerOptions\": {}}"));
  }

...
pub async fn vendor(
  ...
  if cli_options.start_dir.workspace.config_folders().len() > 1 {
  ...
  let root_folder = cli_options.start_dir.workspace.root_folder_configs();
...
fn validate_options(
  ...
  let import_map_specifier = options
    ...
    .or_else(|| {
      let config_file = options.start_dir.workspace.root_deno_json()?;
      config_file

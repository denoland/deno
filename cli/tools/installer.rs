...
pub fn uninstall(uninstall_flags: UninstallFlags) -> Result<(), AnyError> {
  ...
  if let Ok(metadata) = fs::metadata(&installation_dir) {
    if !metadata.is_dir() {
      return Err(JsNativeError::generic("Installation path is not a directory").into());
    }
  }
...
async fn resolve_shim_data(
  ...
  let name = match name {
    Some(name) => name,
    None => return Err(JsNativeError::generic(
  ...
  if let Some(log_level) = flags.log_level {
    if log_level == Level::Error {
      ...
      let log_level = match log_level {
        ...
        Level::Info => "info",
        _ => {
          return Err(JsNativeError::generic(format!("invalid log level {log_level}")).into())
        }
      };

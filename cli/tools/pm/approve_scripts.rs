...
struct LifecycleScriptsState {
  #[serde(rename = "ignored-scripts")]
  ignored_scripts: Vec<String>,
}
...
fn read_state_toml(node_modules_dir: &Path) -> Result<LifecycleScriptsState, AnyError> {
  let state_file_path = node_modules_dir.join(".deno").join(".state.toml");
  ...
  // Parse TOML content manually since we're avoiding the toml crate
...
/// Simple TOML parser for our specific .state.toml format
fn parse_state_toml(content: &str) -> Result<LifecycleScriptsState, AnyError> {
  let mut ignored_scripts = Vec::new();
  let mut in_ignored_scripts_array = false;
  ...
  for line in content.lines() {
    ...
    if line == "ignored-scripts = [" {
      in_ignored_scripts_array = true;
      continue;
    }
    
    if in_ignored_scripts_array {
      if line == "]" {
        in_ignored_scripts_array = false;
        continue;
      }
      
      // Parse array items like: "  \"package-name\","
      if let Some(package) = line.strip_prefix("  \"").and_then(|s| s.strip_suffix("\",")) {
        ignored_scripts.push(package.to_string());
      }
    }
  }
  
  Ok(LifecycleScriptsState {
    ignored_scripts,
  })
}
...
pub async fn approve_scripts(
  ...
  println!("Found node_modules directory at: {}", node_modules_dir.display());
  
  // Read the existing .state.toml file
  let mut state = read_state_toml(&node_modules_dir)
    .context("Failed to read .state.toml file")?;
  
  if state.ignored_scripts.is_empty() {
    println!("No packages with lifecycle scripts found in .state.toml");
  } else {

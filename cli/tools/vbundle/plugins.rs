...
impl PluginHost {
    ...
    while let Some(req) = self.rx.recv().await {
      ...
      match req {
        ...
        PluginHostRequest::Resolve {
          ...
          let r = self.resolve_id(&source, importer.as_deref(), &options);
  ...
  fn load(&mut self, id: &str) -> Result<Option<LoadResult>, AnyError> {
    deno_core::scope!(scope, &mut self.worker.js_runtime);
    ...
    if let Some(result) = result {
      ...
      let load_result: LoadResult = deno_core::serde_v8::from_v8(scope, result)?;
...
async fn create_plugin_runner_inner(
  ...
  let flags = Flags {
    subcommand: DenoSubcommand::Run(crate::args::RunFlags {
      script: "$deno$vbundle.mts".to_string(),
      ..Default::default()
    }),
  ...
  let (install_plugins_fn, resolve_id_fn, load_fn, transform_fn) = {

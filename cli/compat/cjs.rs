...
impl Module {
    ...
    fn _resolve_lookup_paths(request: &str, parent: Option<&Module>) -> Vec<String> {
        // if NativeModule.can_be_required_by_users(request) {
        //     return vec![];
        // }

        // Check for node modules paths.
        let request_chars = request.chars();
        if let Some(char_) = request_chars.nth(0) {
            // if char_ != '.'
        }
        ...
        let filename = parent.filename.unwrap();

        let parent_dir = vec![
            parent.filename.unwrap().parent().to_string_lossy()
        ];
        return parent_dir;
    }

    fn _resolve_filename(request: &str, parent: Option<&Module>, is_main: bool, options: Option<()>) -> String {
        if request.starts_with("node:") /*|| NativeModule.can_be_required_by_users(request)*/ {
        ...
        //     if let Some(prent_filename) = parent.filename {
        //         if request[0] == '#' {

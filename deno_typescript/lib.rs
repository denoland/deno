...
fn compiler_op<D>(
  ...
  dispatcher: D,
) -> impl Fn(&mut CoreIsolateState, &[u8], Box<[ZeroCopyBuf]>) -> Op
where
  D: Fn(&mut TSState, &[u8]) -> Op,
{
  move |_state: &mut CoreIsolateState,
        ...
        -> Op {
    assert!(zero_copy_bufs.len() == 0); // zero_copy_bufs unused in compiler.
...
pub fn op_fetch_asset<S: ::std::hash::BuildHasher>(
  ...
  move |_state: &mut CoreIsolateState,
        ...
        zero_copy_bufs: Box<[ZeroCopyBuf]>|
        -> Op {
    assert!(zero_copy_bufs.len() == 0); // zero_copy_bufs unused in this op.

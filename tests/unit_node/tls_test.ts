...
const v1ViaNonCaKey = Deno.readTextFileSync(
  join(tlsTestdataDir, "localhost_v1_via_non_ca.key"),
);
const v1ViaNonCaCert = Deno.readTextFileSync(
  join(tlsTestdataDir, "localhost_v1_via_non_ca.crt"),
);
...
Deno.test("tls.connect rejects X.509v1 chain through a non-CA cert", async () => {
  await withV1Server(
    // `cert` carries the v1 leaf followed by `localhost.crt`, which really
    // signed it but has basicConstraints CA:FALSE.
    { key: v1ViaNonCaKey, cert: v1ViaNonCaCert + cert },
    async (port) => {
      const error = await new Promise<Error>((resolve, reject) => {
        const client = tls.connect({
          host: "localhost",
        ...
        client.on("secureConnect", () => {
          client.destroy();
          reject(new Error("unexpectedly accepted a non-CA issuer"));
        });
        client.on("error", resolve);
      });
...
Deno.test("tls.connect rejectUnauthorized:false proceeds for forged X.509v1 server cert", async () => {
  await withV1Server(
    { key: v1ForgedKey, cert: v1ForgedCert },
    async (port) => {
      const socket = await new Promise<tls.TLSSocket>((resolve, reject) => {

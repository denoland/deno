...
fn run_test(
    ...
    && util::pty::Pty::is_supported()
  {
    ...
    if !v8_flags.is_empty() {
      // Note: v8_flags are handled separately for PTY
    }
    args.push(&test_path);

    let mut env_vars = std::collections::HashMap::new();
    env_vars.insert(
      "NODE_TEST_KNOWN_GLOBALS".to_string(),
      "0".to_string(),
    );

// Test that fs.write() works on a raw OS fd not registered in Deno's resource table.
// Uses a socketpair via FFI to create a raw fd, then writes to it with fs.write().
...
const message = "hello from fs.write\n";
...
await new Promise<void>((resolve, reject) => {
  fs.write(writeFd, buf, 0, buf.length, null, (err, nwritten) => {
    if (err) {
      reject(err);
    } else if (nwritten !== buf.length) {
      reject(
        new Error(`expected ${buf.length} bytes written, got ${nwritten}`),
      );
    } else {
      resolve();
    }
...
console.log("PASS: fs.write() succeeded on raw fd");

...
#[test]
fn run_workspace_with_jsr_json_fallback() {
  let context = TestContextBuilder::new().use_temp_cwd().build();
  let temp_dir = context.temp_dir().path();
  ...
  // Main file that imports from both packages
  temp_dir.join("main.ts").write(
    r#"
import { add } from "@scope/pkg1";
import { subtract } from "@scope/pkg2";
console.log(add(1, 2));
console.log(subtract(5, 3));
"#,
  );

  let output = context.new_command().args("run main.ts").run();
  output.assert_exit_code(0);
  output.assert_matches_text("[WILDCARD]3[WILDCARD]2[WILDCARD]");
}

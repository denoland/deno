...
pub fn create_pty(command: &str, environment_variables: Option<HashMap<String, String>>) -> Box<dyn Pty> {
...
impl WinPseudoConsole {
  pub fn new(command_text: &str, maybe_environment_variables: Option<HashMap<String, String>>) -> Self {
    ...
    unsafe {
      ...
      let mut environment_vars = maybe_environment_variables.map(|v| get_env_vars(v));
      ...
      assert_win_success!(CreateProcessW(
        ...
        environment_vars.as_mut().map(|v| v.as_mut_ptr() as LPVOID).unwrap_or(ptr::null_mut()),
        ptr::null(),
      ...
      let owned_stdout_read_handle = OwnedHandle::new_duplicate(stdout_read_handle);
...
impl Pty for WinPseudoConsole {
  fn reader(&self) -> Box<dyn std::io::Read + Send> {
    Box::new(ConsoleReader { read_handle: OwnedHandle::new_duplicate(self.stdout_read_handle) })
  }
}
...
fn get_env_vars(env_vars: HashMap<String, String>) -> Vec<u16> {
  ...
  let text = format!("{}\0", env_vars.into_iter()

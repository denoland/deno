...
impl CommandDef {
    ...
    pub fn find_subcommand(&self, name: &str) -> Option<&CommandDef> {
        self.subcommands.iter().find(|cmd| {
    ...
    pub fn find_arg_long(&self, long: &str) -> Option<&ArgDef> {
        self.all_args().find(|a| {
            a.long == Some(long)
                || a.long_aliases.iter().any(|alias| *alias == long)
        })
    }
    ...
    pub fn find_arg_short(&self, short: char) -> Option<&ArgDef> {
        self.all_args().find(|a| {
            a.short == Some(short)
                || a.short_aliases.iter().any(|c| *c == short)
        })
    }
}
...
impl ArgDef {
    ...
    pub const fn long_aliases(mut self, aliases: &'static [&'static str]) -> Self {

...
fn uninstall_with_help_flag() { let r = flags_from_vec(svec!["deno", "uninstall", "--help"]); assert!(r.is_ok()); assert!(matches!(r.unwrap().subcommand, DenoSubcommand::Help(_))); }
...
fn completions() {
    ...
    assert!(matches!(r.subcommand, DenoSubcommand::Completions));
}
...
fn allow_scripts() {
    ...
    let r = flags_from_vec(svec!["deno", "cache", "--allow-scripts", "script.ts"]);
    assert_eq!(
        r.unwrap(),
        Flags {
            subcommand: DenoSubcommand::Cache(CacheFlags {
    ...
    let r = flags_from_vec(svec!["deno", "cache", "--allow-scripts=npm:foo", "script.ts"]);
    assert_eq!(
        r.unwrap(),
        Flags {
            subcommand: DenoSubcommand::Cache(CacheFlags {
    ...
    let r = flags_from_vec(svec!["deno", "cache", "--allow-scripts=npm:foo,npm:bar@2", "script.ts"]);
    assert_eq!(
        r.unwrap(),
        Flags {
            subcommand: DenoSubcommand::Cache(CacheFlags {

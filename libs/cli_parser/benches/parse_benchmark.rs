...
const UNSTABLE_FLAG_NAMES: &[(&str, &str, bool)] = &[
  ...
  (
    "unstable-bundle",
    "Enable unstable bundle runtime API",
    true,
  ),
  ...
  (
    "unstable-detect-cjs",
    "Treats ambiguous files as CommonJS in more cases",
    true,
  ),
  ...
  (
    ...
    "Lazily loads statically analyzable dynamic imports",
    true,
  ),
  ...
  (
    ...
    "Enable abort signal without legacy behavior",
    true,
  ),
  (
    "unstable-node-globals",
    "Prefer Node.js globals over Deno globals",
    true,
  ),
  (
    "unstable-npm-lazy-caching",
    "Enable unstable lazy caching of npm dependencies",
    true,
  ),
  (
    "unstable-otel",
    "Enable unstable OpenTelemetry features",
    false,
  ),
  ("unstable-process", "Enable unstable process APIs", false),
  (
    "unstable-raw-imports",
    "Enable unstable bytes and text imports",
    true,
  ),
  (
  ...
  (
    ...
    "Enable subdomain wildcards for --allow-net",
    false,
  ),
  ("unstable-temporal", "Enable unstable Temporal API", false),
  (
    "unstable-tsgo",
    "Enable unstable TypeScript Go integration",
    true,
  ),
  (
    "unstable-unsafe-proto",
    "Enable unsafe __proto__ support",
    true,
  ),
...
fn check_arg(checks_local_by_default: bool) -> Arg {
  ...
  if checks_local_by_default {
    arg
      .help("Set type-checking behavior (type-checks local modules by default)")
  } else {
...
fn add_subcommand() -> Command {
  with_unstable_args(
    Command::new("add").about("Add dependencies to your configuration file"),
  )
  .defer(|cmd| {
    cmd
      .arg(
...
fn approve_scripts_subcommand() -> Command {
  with_unstable_args(
    Command::new("approve-scripts")
      .about("Approve npm lifecycle scripts for installed dependencies"),
  )
      ...
      .arg(
        Arg::new("ext")
          ...
          .value_parser([
            ...
            "jsonc", "css", "scss", "sass", "less", "html", "svelte", "vue",
...
fn uninstall_subcommand() -> Command {
  with_unstable_args(
    Command::new("uninstall")
      .about("Uninstalls a dependency or an executable script"),
  )
  .defer(|cmd| {
    cmd
...
fn bench_parse(c: &mut Criterion) {
  ...
  group.bench_function("custom_complex", |b| {
    let args: Vec<String> =
      args_complex.iter().map(|s| s.to_string()).collect();
    b.iter(|| {
      let _ = deno_cli_parser::parse(
        &deno_cli_parser::defs::DENO_ROOT,
        black_box(&args),

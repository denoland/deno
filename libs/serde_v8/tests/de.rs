...
macro_rules! decheck {
  ($fn_name:ident, $t:ty, $src:expr_2021, $x:ident, $check:expr_2021) => {
    #[test]
    fn $fn_name() {
      #[allow(clippy::bool_assert_comparison, reason = "explicit comparison improves clarity in tests")]
      dedo($src, |scope, v| {
...
macro_rules! defail {
  ($fn_name:ident, $t:ty, $src:expr_2021, $failcase:expr_2021) => {
    #[test]
    fn $fn_name() {
      #[allow(clippy::bool_assert_comparison, clippy::redundant_closure_call, reason = "macro-generated test code")]
      dedo($src, |scope, v| {

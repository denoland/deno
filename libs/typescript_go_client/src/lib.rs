...
pub trait CallbackHandler {
    ...
    fn handle_callback(&self, name: &str, payload: String) -> Result<String, Error>;
}
...
impl<T: CallbackHandler> SyncRpcChannel<T> {
    ...
    pub fn new<I, S>(exe: impl AsRef<OsStr>, args: I, callback_handler: T) -> Result<Self, Error>
    where
    ...
    pub fn request_sync(&mut self, method: &str, payload: String) -> Result<String, Error> {
        self.request_bytes_sync(method, payload.as_bytes())
            .and_then(|arr| {
    ...
    pub fn request_bytes_sync(&mut self, method: &str, payload: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        self.conn
            ...
            .map_err(Error::RpcConnection)?;
        loop {
            let (ty, name, payload) = self.conn.read().map_err(Error::RpcConnection)?;

...
pub struct TsConfigData {
  ...
  /// True when the tsconfig was written by `deno install` for stock TS
  /// tooling. Identified by the `_deno_generated: true` sentinel at the top
  /// level of the JSON object — robust across platforms (no path-string
  /// comparisons) and disambiguates from a hand-written `.deno/tsconfig.json`.
  ...
  fn read_ts_config(
    ...
    let extends_targets = object
      ...
      .filter_map(|s| {
        ...
        // Skip tsconfigs generated by `deno install` for stock TS tooling
        // (tsc, tsgo). Identified by the `_deno_generated` sentinel rather
        // than a path-string match, so it works on Windows (backslashes)
        // and doesn't trample a user file that legitimately lives at
        // .deno/tsconfig.json.
    ...
    let is_deno_generated = object
      .and_then(|o| o.get("_deno_generated")?.as_bool())
      .unwrap_or(false);
    Ok(TsConfigData {

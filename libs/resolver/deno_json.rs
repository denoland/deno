      ...
      let types = self
        ...
        .filter_map(|s| {
          let types = s
            ...
            .filter_map(|v| {
              let s = v.as_str()?;
              // Skip "deno" types entry -- Deno provides its own types
              // natively. This entry is used by stock tsc/tsgo (via
              // @types/deno in node_modules) but should not be loaded
              // by Deno's own type checker.
              if s == "deno" {
                return None;
              }
              Some(s.to_string())
...
> TsConfigCollector<'a, 'b, TSys, NSys, TNpfr>
{
  ...
  fn read_ts_config(
    ...
    let extends_targets = object
      ...
      // Skip extends targets that point to .deno/tsconfig.json -- those
      // are generated by `deno install` for stock TypeScript tooling
      // (tsc, tsgo) and should not be loaded by Deno's own type checker.
      .filter(|s| !s.contains(".deno/tsconfig.json"))
      .filter_map(|s| {

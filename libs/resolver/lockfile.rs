  ...
  pub async fn discover(
    ...
    fn pkg_json_deps(
      ...
      deps
        ...
        .filter(|name| !name.is_empty()) // Skip empty package names
        .filter_map(|name| {
          let dep = deps.dependencies.get(name).and_then(|d| d.as_ref().ok())
            .or_else(|| deps.dev_dependencies.get(name).and_then(|d| d.as_ref().ok()))?;
          match dep {

  ...
  fn create_setup_future(
    ...
    async move {
      ...
      let maybe_registry_config =
        tarball_cache.npmrc.tarball_config(&tarball_uri).cloned();
      let maybe_auth_header = maybe_registry_config.as_deref().and_then(|c| maybe_auth_header_value_for_npm_registry(c).ok()?);
      ...
      let result = tarball_cache.http_client
        .download_with_retries_on_any_tokio_runtime(tarball_uri, maybe_auth_header, None, maybe_registry_config.as_deref())
        .await;
      if let Some(reporter) = &reporter {

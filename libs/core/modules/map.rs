...
impl ModuleMap {
  ...
  fn resolve_callback<'s, 'i>(
    ...
    // imports such as `with { type: "yaml" }` (or JSON5/JSONC imported as
    // `with { type: "json" }`), which deno_graph rewrites into a JavaScript
    // module that parses the file. Such a module is registered under its
    // concrete (`None`) type, so if the lookup by the requested type misses,
    // fall back to the plain module. A genuine module of the requested type
    // (e.g. a real `.json` or `.css`) is registered under that type and is
    ...
    let resolved_id = self
      ...
      .or_else(|| {
        if matches!(
          module_type,
          RequestedModuleType::Other(_) | RequestedModuleType::Json
        ) {

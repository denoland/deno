...
impl ModuleMap {
  ...
  pub(crate) fn load_dynamic_import(
    ...
    // Convert resolve response to a Result for the prepare future.
    // For sync resolution, we check right away; for async, prepare() will
    // resolve again internally.
    let resolve_result = match resolve_response {
      ModuleResolveResponse::Sync(result) => {
        Some(result.map_err(CoreError::from))
      }
      ModuleResolveResponse::Async(_) => {
        // Async resolution will be handled by prepare() -> resolve_root_async()
        None
      }
    };
    ...
    let fut = match resolve_result {
      ...
      Some(Err(error)) => async move { (load.id, Err(error)) }.boxed_local(),
    };

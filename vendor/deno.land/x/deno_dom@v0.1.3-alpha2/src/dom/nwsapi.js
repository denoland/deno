...
function Factory(global, Export) {
  var version = "nwsapi-2.2.0",
    ...
    GROUPS = {
      ...
      logicalsel:
        "(matches|not)\\x28\\s?([^()]*|[^\\x28]*\\x28[^\\x29]*\\x29)\\s?(?:\\x29|$)",
      treestruct:
      ...
      inputstate:
        "(enabled|disabled|read-only|read-write|placeholder-shown|default)\\b",
      inputvalue:
    ...
    hasMixedCaseTagNames = function (context) {
      ...
      ns = context.documentElement.namespaceURI ||
    ...
    byTag = function (tag, context) {
      ...
      if (api in context) {
        ...
        if ((e = context.firstElementChild)) {
          tag = tag.toLowerCase();
          if (
            !(e.nextElementSibling || tag == "*" ||
              e.nodeName.toLowerCase() == tag)
          ) {
            return slice.call(e[api]("*", tag));
          } else {
            nodes = [];
            do {
              if (tag == "*" || e.nodeName.toLowerCase() == tag) {
    ...
    nthOfType = (function () {
      ...
      return function (element, dir) {
        ...
        if (
          nodes[set] && nodes[set][name] && parent === element.parentElement
        ) {
          i = set;
          j = idx;
          l = len;
        ...
        if (
          element !== nodes[i][name][j] && element !== nodes[i][name][j = 0]
        ) {
    ...
    compile = function (selector, mode, callback) {
      ...
      return mode || mode === null
        ? (selectLambdas[selector] = factory)
        : (matchLambdas[selector] = factory);
    },
        ...
        switch (symbol) {
          ...
          case "|":
            ...
            if (match[1] == "*") {
              ...
              source = "if(" + N + '(e.namespaceURI=="' + NAMESPACE + '")){' +
                source + "}";
            } else {
          ...
          case "[":
            ...
            if (match[4] === "") {
              ...
              match[4] = convertEscapes(match[4]).replace(
                REX.RegExpChar,
                "\\$&",
              );
            }
            ...
            source = "if(" + N + "(" +
              (!match[2]
                ...
                ? 'e.getAttribute("' + name + '")==""'
                : "(/" + test.p1 + match[4] + test.p2 + "/" + type +
                  ').test(e.getAttribute("' + name + '"))==' + test.p3) +
              ")){" + source + "}";
            break;
          ...
          case "~":
            ...
            source = "n=e;while((e=e.previousElementSibling)){" + source +
              "}e=n;";
            break;
          ...
          case ":":
            if ((match = selector.match(Patterns.structural))) {
              ...
              switch (match[1]) {
                case "root":
                  ...
                  source = "if(" + N + "(e===s.root)){" + source + (mode
                    ? "break main;"
                    : "") +
                    "}";
                  break;
                ...
                case "last-child":
                  ...
                  break;
                case "first-child":
                  source = "if(" + N + "(!e.previousElementSibling)){" +
                    source + "}";
                  break;
                ...
                case "only-of-type":
                  source = "o=e.nodeName;" +
                    ...
                    "n=e;while((n=n.previousElementSibling)&&n.nodeName!=o);}if(" +
                    D + "n){" + source + "}";
                  break;
                case "last-of-type":
                  source =
                    "n=e;o=e.nodeName;while((n=n.nextElementSibling)&&n.nodeName!=o);if(" +
                    D + "n){" + source + "}";
                  break;
                case "first-of-type":
                  source =
                    "n=e;o=e.nodeName;while((n=n.previousElementSibling)&&n.nodeName!=o);if(" +
                    D + "n){" + source + "}";
                  break;
                default:
                  emit("'" + selector_string + "'" + qsInvalid);
                  ...
                  if (match[1] && match[2]) {
                    ...
                    if (match[2] == "n") {
                      ...
                      source = expr
                        ? "n=e;o=e.nodeName;" +
                          "while((n=n." + test +
                          "ElementSibling)&&n.nodeName!=o);if(" + D + "n){" +
                          source + "}"
                        : "if(" + N + "!e." + test + "ElementSibling){" +
                          source + "}";
                      break;
                    } else if (
                      match[2] == "even" || match[2] == "2n0" ||
                      match[2] == "2n+0" || match[2] == "2n"
                    ) {
                      test = "n%2==0";
                    } else if (
                      match[2] == "odd" || match[2] == "2n1" ||
                      match[2] == "2n+1"
                    ) {
                      test = "n%2==1";
                    } else {
                    ...
                    source = "n=s.nth" + expr + "(e," + type + ");if(" + N +
                      "(" + test + ")){" + source + "}";
                  } else {
                    emit("'" + selector_string + "'" + qsInvalid);
                  }
            ...
            // :matches( s1, [ s2, ... ]), :not( s1, [ s2, ... ])
              ...
              switch (match[1]) {
                case "matches":
                  ...
                  nested = true;
                  expr = match[2].replace(REX.CommaGroup, ",").replace(
                    REX.TrimSpaces,
                    "",
                  );
                  ...
                  for (i = 0, l = expr.length; l > i; ++i) {
                    ...
                    source = 'if(s.match("' + expr[i].replace(/\x22/g, '\\"') +
                      '",e)){' + source + "}";
                  }
                  break;
                case "not":
            ...
            // :link, :visited, :target
              ...
              switch (match[1]) {
                case "link":
                  source = "if(" + N +
                    '(/^a|area|link$/i.test(e.nodeName)&&e.hasAttribute("href"))){' +
                    source + "}";
                  break;
                case "visited":
                  source = "if(" + N +
                    '(/^a|area|link$/i.test(e.nodeName)&&e.hasAttribute("href")&&e.visited)){' +
                    source + "}";
                  break;
                case "target":
                  source = "if(" + N +
                    "((s.doc.compareDocumentPosition(e)&16)&&s.doc.location.hash&&e.id==s.doc.location.hash.slice(1))){" +
                    source + "}";
                  break;
                default:
                  emit("'" + selector_string + "'" + qsInvalid);
              ...
              switch (match[1]) {
                ...
                case "focus":
                  source = "hasFocus" in doc
                    ? "if(" + N +
                      '(e===s.doc.activeElement&&s.doc.hasFocus()&&(e.type||e.href||typeof e.tabIndex=="number"))){' +
                      source + "}"
                    : "if(" + N +
                      "(e===s.doc.activeElement&&(e.type||e.href))){" + source +
                      "}";
                  break;
                case "focus-within":
                  source = "hasFocus" in doc
                    ? "n=s.doc.activeElement;while(e){if(e===n||e.parentNode===n)break;}" +
                      "if(" + N +
                      '(e===n&&s.doc.hasFocus()&&(e.type||e.href||typeof e.tabIndex=="number"))){' +
                      source + "}"
                    : source;
                  break;
                default:
            ...
            // :enabled, :disabled, :read-only, :read-write, :placeholder-shown, :default
              ...
              switch (match[1]) {
                case "enabled":
                  source = "if(" + N +
                    '(("form" in e||/^optgroup$/i.test(e.nodeName))&&"disabled" in e &&e.disabled===false' +
                    ")){" + source + "}";
                  break;
                case "disabled":
                  // https://www.w3.org/TR/html5/forms.html#enabling-and-disabling-form-controls:-the-disabled-attribute
                  source = "if(" + N +
                    '(("form" in e||/^optgroup$/i.test(e.nodeName))&&"disabled" in e&&' +
                ...
                case "default":
                  source = "if(" + N + '("form" in e && e.form)){' +
                    ...
                    "}" +
                    "if(" + N +
                    '(e.form&&(e===n[x]&&"|image|submit|".includes("|"+e.type+"|"))||' +
            ...
            // :checked, :indeterminate, :valid, :invalid, :in-range, :out-of-range, :required, :optional
              ...
              switch (match[1]) {
                ...
                case "indeterminate":
                  source = "if(" + N +
                    '(/^progress$/i.test(e.nodeName)&&!e.hasAttribute("value"))||' +
    ...
    match = function _matches(selectors, element, callback) {
      ...
      if (element && matchResolvers[selectors]) {
        return match_assert(
          matchResolvers[selectors].factory,
          element,
          callback,
        );
      }
    ...
    install = function (all) {
      ...
      Element.prototype.closest = function closest() {
        var ctor =
          Object.getPrototypeOf(this).__proto__.__proto__.constructor.name;
        if (!("nodeType" in this)) {
          emit(
            "'closest' called on an object that does not implement interface " +
              ctor + ".",
            TypeError,
          );
        }
      ...
      Element.prototype.matches = function matches() {
        ...
        if (!("nodeType" in this)) {
          emit(
            "'matches' called on an object that does not implement interface " +
              ctor + ".",
            TypeError,
          );
        }
      ...
      Element.prototype.querySelector = Document.prototype.querySelector =
        DocumentFragment.prototype.querySelector = function querySelector() {
          ...
          if (!("nodeType" in this)) {
            emit(
              "'querySelector' called on an object that does not implement interface " +
                ctor + ".",
              TypeError,
            );
          }
      ...
      Element.prototype.querySelectorAll = Document.prototype.querySelectorAll =
        DocumentFragment.prototype.querySelectorAll =
          function querySelectorAll() {
            ...
            if (!("nodeType" in this)) {
              emit(
                "'querySelectorAll' called on an object that does not implement interface " +
                  ctor + ".",
                TypeError,
              );
            }

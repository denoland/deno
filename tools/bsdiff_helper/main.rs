...
fn main() {
  ...
  if args.len() != 4 {
    eprintln!("Usage: {} <old_file> <new_file> <patch_file>", args[0]);
    std::process::exit(1);
  }

  let old = std::fs::read(&args[1]).unwrap_or_else(|e| {
    eprintln!("Failed to read old file '{}': {}", args[1], e);
    std::process::exit(1);
  });
  let new = std::fs::read(&args[2]).unwrap_or_else(|e| {
    eprintln!("Failed to read new file '{}': {}", args[2], e);
    std::process::exit(1);
  });

  let mut patch = Vec::new();
  bsdiff::diff(&old, &new, &mut patch).unwrap_or_else(|e| {
    eprintln!("Failed to generate bsdiff patch: {}", e);
    std::process::exit(1);
  });

  std::fs::write(&args[3], &patch).unwrap_or_else(|e| {
    eprintln!("Failed to write patch file '{}': {}", args[3], e);
    std::process::exit(1);
  });

...
import sys
from os import path
from util import add_env_path, find_exts, make_env, remove_and_symlink, rmtree, root_path, run
...
def run_pip():
    # Upgrade pip itself. The version that is bundled with python is too old
    # to support the next steps.
    run([
        sys.executable, "-m", "pip", "install", "--upgrade", "--target",
        python_packages_path, "pip"
    ],
        cwd=third_party_path,
        env=python_env())

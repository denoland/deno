...
async function checkoutUpstream() {
  ...
  const cmd = `curl -L https://api.github.com/repos/${REPO}/tarball/${COMMIT} | tar -C '${TARGET_DIR}' -xzvf - --strip=2 '${tarPrefix}'`;
  // console.log(cmd);
...
async function patchCargo() {
  ...
  const patched = data
    ...
    .replace(/^deno_core \= .*$/gm, `deno_core = { version = "${V_DENO_CORE}", path = "../../core" }`)

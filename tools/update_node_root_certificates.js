...
const url = `https://raw.githubusercontent.com/nodejs/node/${nodeVersion}/src/node_root_certs.h`
...
function transformCppToJs(cppCode) {
  ...
  const certRegex = /(\/\*[\s\S]*?\*\/)\s*"-----BEGIN CERTIFICATE-----\s*([\s\S]*?)\s*-----END CERTIFICATE-----"/g;
  const certificates = [];
  let match;

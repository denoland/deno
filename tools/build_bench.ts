...
if (Deno.args.length === 0) {
  console.log("Usage: build_bench [-v] [--profile release|debug] commit1 [commit2 [comment3...]]");
  Deno.exit(1);
}
...
async function buildGitCommit(commit) {
  ...
  const gitInfo = await $`git log --pretty=oneline --abbrev-commit -n1 ${commit}`.stdout("piped").stderr("piped");
  if (gitInfo.code != 0) {
  ...
  await runCommand(`Failed to clone commit ${commit}`, $`git clone ${gitDir} ${tempDir}`);
  progress.completed++;

  progress.text = `reset ${hash}`;
  await runCommand(`Failed to reset commit ${commit}`, $`git reset --hard ${hash}`.cwd(tempDir));
  progress.completed++;

  progress.text = `build ${hash}`;
  if (profile === 'debug') {
    await runCommand(`Failed to build commit ${commit}`, $`cargo build`.cwd(tempDir));
  } else {
    await runCommand(`Failed to build commit ${commit}`, $`cargo build --profile ${profile}`.cwd(tempDir));
  }
  progress.completed++;

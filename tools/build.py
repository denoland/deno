...
print("DENO_BUILD_PATH: %s" % build_path())
...
def maybe_add_default_target(args):
    lines = run_output(
        ...
        quiet=True).split(b"\n")
    targets = [l.rsplit(b":", 1)[0] for l in lines]
    deno_targets = [target for target in targets if target.startswith(b":")]
    deno_targets += [target.lstrip(b":") for target in deno_targets]

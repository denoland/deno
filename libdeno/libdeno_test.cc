...
TEST(LibDenoTest, InitializesCorrectly) {
  ...
  Deno* d = deno_new(snapshot, deno_config{0, empty, nullptr});
  EXPECT_TRUE(deno_execute(d, nullptr, "a.js", "1 + 2"));
  deno_delete(d);
}

TEST(LibDenoTest, InitializesCorrectlyWithoutSnapshot) {
  Deno* d = deno_new(empty, deno_config{0, empty, nullptr});
  EXPECT_TRUE(deno_execute(d, nullptr, "a.js", "1 + 2"));
  deno_delete(d);
}

TEST(LibDenoTest, SnapshotterInitializesCorrectly) {
  Deno* d = deno_new(empty, deno_config{1, empty, nullptr});
  deno_delete(d);
}

TEST(LibDenoTest, Snapshotter) {
  Deno* d1 = deno_new(empty, deno_config{1, empty, nullptr});
  EXPECT_TRUE(deno_execute(d1, nullptr, "a.js", "a = 1 + 2"));
  deno_buf test_snapshot = deno_get_snapshot(d1);
  deno_delete(d1);

  Deno* d2 = deno_new(test_snapshot, deno_config{0, empty, nullptr});
  EXPECT_TRUE(
...
TEST(LibDenoTest, RecvReturnEmpty) {
  ...
  Deno* d = deno_new(snapshot, deno_config{0, empty, recv_cb});
...
TEST(LibDenoTest, Shared) {
  ...
  Deno* d = deno_new(snapshot, deno_config{0, shared, nullptr});

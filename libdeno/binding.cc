...
void Recv(const v8::FunctionCallbackInfo<v8::Value>& args) {
  ...
  if (!d->recv_.IsEmpty()) {
    isolate->ThrowException(v8_str("Deno.core._recv already called."));
    return;
  }
...
void InitializeContext(v8::Isolate* isolate, v8::Local<v8::Context> context) {
  ...
  CHECK(core_val->Set(context, deno::v8_str("_print"), print_val).FromJust());

  auto recv_tmpl = v8::FunctionTemplate::New(isolate, Recv);
  auto recv_val = recv_tmpl->GetFunction(context).ToLocalChecked();
  CHECK(core_val->Set(context, deno::v8_str("_recv"), recv_val).FromJust());

  auto send_tmpl = v8::FunctionTemplate::New(isolate, Send);
  auto send_val = send_tmpl->GetFunction(context).ToLocalChecked();
  CHECK(core_val->Set(context, deno::v8_str("_send"), send_val).FromJust());
  ...
  CHECK(core_val->Set(context, deno::v8_str("_evalContext"), eval_context_val)
            .FromJust());
  ...
  CHECK(core_val->Set(context, deno::v8_str("_errorToJSON"), error_to_json_val)
            .FromJust());

  CHECK(core_val->SetAccessor(context, deno::v8_str("_shared"), Shared)
            .FromJust());
}

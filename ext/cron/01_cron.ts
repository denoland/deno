...
let RAN_SETUP = false;
let USING_NET_HANDLER = false;
function setupNetHandler(): boolean {
  if (RAN_SETUP) return USING_NET_HANDLER;
  RAN_SETUP = true;
  const rid = op_cron_take_net_handler();
  if (rid) {
    USING_NET_HANDLER = true;
    runOnSock(rid);
  }
  return USING_NET_HANDLER;
}
...
function runOnSock(rid: number) {
  ...
  const onRequest = async (req: Request) => {
    ...
    if (req.method === "GET" && url.pathname === "/crons") {
    ...
    if (req.method === "POST" && url.pathname === "/crons") {
      const { name, id } = await req.json();
      const cron = CRONS.get(name);
      if (cron) {
        PromisePrototypeThen(
          executeCron(name, cron.schedule, cron.handler),
          (success) => {
  ...
  return serveHttpOnListener(
    ...
    onRequest,
    (error) => {
      import.meta.log("error", "Error in cron sock", error);

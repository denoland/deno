...
ObjectAssign(SubtleCrypto.prototype, {
   ...
   * @returns {Promise<any>}
   */
  // `SubtleCrypto.prototype.importKey` is implemented natively on the
  // cppgc impl block in `ext/crypto/subtle_crypto.rs`; see
  // `subtle_import_key.rs` for the per-algorithm/format dispatch.
  ...
  // `SubtleCrypto.prototype.generateKey` is implemented natively on
  // the cppgc impl block in `ext/crypto/subtle_crypto.rs`; see
  // `subtle_generate_key.rs`.
   ...
   */
  // `SubtleCrypto.prototype.encapsulateKey` is implemented natively on
  // the cppgc impl block in `ext/crypto/subtle_crypto.rs`; see
  // `subtle_encapsulate_key.rs`.
   ...
   * @returns {Promise<CryptoKey>}
   */
  // `SubtleCrypto.prototype.decapsulateKey` is implemented natively on
  // the cppgc impl block in `ext/crypto/subtle_crypto.rs`; see
  // `subtle_encapsulate_key.rs`.
   ...
   * @returns {Promise<CryptoKey>}
   */
  // `SubtleCrypto.prototype.getPublicKey` is implemented natively on
  // the cppgc impl block in `ext/crypto/subtle_crypto.rs`; see
  // `subtle_get_public_key.rs` for the per-algorithm public-key
  // derivation.
...
function exportKeyOkp(name, format, key, innerKey) {
  ...
  switch (format) {
    ...
    case "spki": {
      ...
      return TypedArrayPrototypeGetBuffer(ops.spki(innerKey));
    }
    case "pkcs8": {
      if (key.type !== "private") {
        throw new DOMException(
      ...
      const pkcs8Der = ops.pkcs8(
        new Uint8Array([...ops.prefix, ...new SafeArrayIterator(innerKey)]),
      );
      pkcs8Der[15] = ops.lenByte;

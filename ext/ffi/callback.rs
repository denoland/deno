...
unsafe fn do_ffi_callback(
  ...
  let cif_types = std::slice::from_raw_parts(cif.arg_types, cif.nargs as usize);
    ...
    info.parameters.iter().enumerate().zip(vals)
  {
    let value: v8::Local<v8::Value> = match native_type {
      ...
      NativeType::Struct(_) => {
        let size = cif_types[index].as_ref().unwrap().size;
        ...
        let local_value: v8::Local<v8::Value> =
          v8::Uint8Array::new(scope, ab, 0, size).unwrap().into();
        local_value
      }
  ...
  match info.result {
    ...
    NativeType::Struct(_) => {
      // Get the libffi given size and use it as an upper bound for memory copying.
      let out_size = cif.rtype.as_ref().unwrap().size;
      // Get the JS ArrayBuffer(View) size and use it as the other upper bound for copying.
      let size;
      let pointer = if let Ok(value) =
        v8::Local::<v8::ArrayBufferView>::try_from(value)
      {
        let byte_offset = value.byte_offset();
        let array_buffer = value
          .buffer(scope)
          .expect("Unable to deserialize result parameter.");

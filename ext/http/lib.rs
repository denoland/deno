...
struct HttpConnResource {
  ...
  close_result_fut: Shared<RemoteHandle<Result<(), Arc<hyper::Error>>>>,
  cancel_handle: Rc<CancelHandle>, // Cancels accept ops.
}
...
impl HttpConnResource {
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
  {
    ...
    let service = HttpListenerService::new(acceptors_rx);
    ...
    let task_fut = async move {
      ...
      swallow_not_connected_error(result).map_err(Arc::from)
    };
    let (task_fut, close_result_fut) = task_fut.remote_handle();
    let output_fut = close_result_fut.shared();
  ...
  async fn accept(
    ...
    async {
      match fut.await {
        ...
        None => self.get_closed().map_ok(|_| None).await,
      }
    }
  ...
  async fn get_closed(&self) -> Result<(), AnyError> {
    self.close_result_fut.clone().map_err(AnyError::from).await
  }
...
fn swallow_not_connected_error(
...
where
  S: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
  let listener = HttpConnResource::new(io, scheme, addr);
...
struct HttpListenerService {
  acceptor_rx: Peekable<mpsc::UnboundedReceiver<HttpAcceptor>>,
}

impl HttpListenerService {
  fn new(acceptor_rx: mpsc::UnboundedReceiver<HttpAcceptor>) -> Self {
    let acceptor_rx = acceptor_rx.peekable();
    Self { acceptor_rx }
  }
}

impl Service<Request<Body>> for HttpListenerService {
  ...
  fn poll_ready(
    ...
    let acceptor_rx = Pin::new(&mut self.acceptor_rx);
    let result = ready!(acceptor_rx.poll_peek(cx))
      .map(|_| ())
  ...
  fn call(&mut self, request: Request<Body>) -> Self::Future {
    let acceptor = self.acceptor_rx.next().now_or_never().flatten().unwrap();
...
async fn op_http_response(
  ...
  match body_buf {
    Some(body_buf) => {
      ...
      // TODO(piscisaureus): get rid of this buffer copy.
      let body_bytes = Bytes::copy_from_slice(&body_buf[..]);
      body = builder.body(Body::from(body_bytes))?;
  ...
  let response_tx = match replace(&mut *old_wr, new_wr) {
    ...
    _ => return Err(custom_error("Http", "response headers already sent")),
  };

  match response_tx.send(body) {
    Ok(_) => Ok(()),
    Err(_) => {
      stream.conn.get_closed().await?;
      unreachable!();
    }
  }
}
  ...
  loop {
    let body_tx = match &mut *wr {
      ...
      HttpResponseWriter::Headers(_) => {
        break Err(custom_error("Http", "no response headers"))
      }
      HttpResponseWriter::Closed => {
        break Err(custom_error("Http", "response already completed"))
      }
    };

    // TODO(piscisaureus): pass zero-copy buf directly to `send_data()` below.
    let bytes = Bytes::copy_from_slice(&buf[..]);
    match body_tx.send_data(bytes).await {
      Ok(_) => break Ok(()),
      Err(err) => {
        // Don't return "channel closed", that's an implementation detail.
        // Pull up the failure associated with the connection instead.
...
// Gracefully close the write half of the HTTP stream. Note that this does not

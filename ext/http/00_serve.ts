...
(function () {
    ...
    function finishOrReturnNative(
      ...
      if (
        trySetServeFastStaticResponse(
          ...
          innerRequest,
          fromPromise
            ? closeInnerRequestImmediately
            : closeInnerRequestForNative,
        )
      ) {
        return undefined;
      }
  ...
  function serve(arg1, arg2) {
    ...
    if (overrideKind) {
      ...
      switch (overrideKind) {
        ...
        case 5: {
          // Memory (in-process byte channel)
          envOptions = {
            ...envOptions,
            memory: overrideHost,
          };
      ...
      if (duplicateListener) {
        ...
        return {
          ...
          finished: SafePromiseAll([
            envListener.finished,
            userListener.finished,
          ]),
          shutdown() {
  ...
  function serveInner(options, handler) {
    ...
    const wantsMemory = ObjectHasOwn(options, "memory");
  ...
  function serveHttpOnListener(listener, signal, handler, onError, onListen) {
    ...
    nativeCallback = mapToNativeResponseCallback(
      serverContext,
      handler,
      onError,
    );
   ...
   */
  function serveHttpOnConnection(
    connection,
    signal,
    handler,
    onError,

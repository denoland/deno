...
use crate::canvas::OffscreenRenderingContextId;
...
impl UnsafeWindowSurface {
  ...
  fn new(
    ...
    // SAFETY: window/display handles are platform pointers. FFI permission
    // is checked above; `*const c_void` only deserializes null or External.
...
/// wgpu_core device/queue for Canvas2D present (`COPY_DST` uploads).

...
fn response_expires_at(
  ...
  if let Some(expires) = get_header("expires", response_headers) {
    ...
    // a time in the past
    let Ok(expires) =
      httpdate::parse_http_date(String::from_utf8_lossy(&expires).trim())
    else {
      return Some(0);
    };
    let Ok(expires) = expires.duration_since(UNIX_EPOCH) else {
      return Some(0);
    };
    return Some(expires.as_secs());
  }
  None
}
...
mod tests {
  ...
  fn test_response_expires_at() {
    ...
    assert_eq!(
      response_expires_at(&[h("expires", "Sunday, 06-Nov-94 08:49:37 GMT")], 0),
      Some(784111777)
    );
    assert_eq!(
      response_expires_at(&[h("expires", "Sun Nov  6 08:49:37 1994")], 0),
      Some(784111777)
    );
    assert_eq!(

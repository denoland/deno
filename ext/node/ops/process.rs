...
/// Names of bootstrap events Node emits when `--trace-event-categories`
/// includes `node` or `node.bootstrap`. Mirrors Node's TRACE_EVENT calls in
/// `src/node.cc` and `src/api/environment.cc`.
const NODE_BOOTSTRAP_TRACE_EVENT_NAMES: &[&str] = &[
  "nodeStart",
  "v8Start",
  "environment",
  "loopStart",
...
/// Emit a `node_trace.${rotation}.log` JSON file in the current working
/// directory when the child process was launched with
/// `--trace-event-categories` (forwarded via `NODE_OPTIONS` by `cp.fork`).
///
/// Deno does not have V8/Node trace-event instrumentation, so this only emits
/// the bootstrap-phase events in the requested categories. Other categories
/// (e.g. `v8`, `node.async_hooks`) result in an empty `traceEvents` array.
#[op2(fast)]
pub fn op_node_maybe_write_trace_events_bootstrap_log() {
  let Ok(node_options) = std::env::var("NODE_OPTIONS") else {
    return;
  };
  ...
  let mut file_pattern = String::from("node_trace.${rotation}.log");
  let tokens: Vec<&str> = node_options.split_whitespace().collect();
  let mut i = 0;
  while i < tokens.len() {
    let t = tokens[i];
    if t == "--trace-event-categories" && i + 1 < tokens.len() {
      categories = tokens[i + 1].to_string();
      i += 2;
      continue;
    } else if let Some(v) = t.strip_prefix("--trace-event-categories=") {
      categories = v.to_string();
    } else if t == "--trace-event-file-pattern" && i + 1 < tokens.len() {
      file_pattern = tokens[i + 1].to_string();
      i += 2;
      continue;
    } else if let Some(v) = t.strip_prefix("--trace-event-file-pattern=") {
      file_pattern = v.to_string();
    }
    i += 1;
  }
  ...
  let cats: Vec<&str> = categories
    ...
    .collect();
  let wants_bootstrap =
    cats.contains(&"node") || cats.contains(&"node.bootstrap");

  let pid = std::process::id();
  let mut events = String::from("[");
  if wants_bootstrap {
    for (idx, name) in NODE_BOOTSTRAP_TRACE_EVENT_NAMES.iter().enumerate() {
      if idx > 0 {
        events.push(',');
      }
      events.push_str(&format!(
        "{{\"pid\":{},\"tid\":1,\"ts\":{},\"ph\":\"X\",\"cat\":\"node,node.bootstrap\",\"name\":\"{}\",\"dur\":1,\"tdur\":1,\"args\":{{}}}}",
        pid, idx, name
      ));
    }
  }
  ...
  let content = format!("{{\"traceEvents\":{}}}", events);

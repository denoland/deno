...
use crate::ops::tls_wrap::TLSWrap;
...
/// The consumed transport stream for native I/O.
/// Mirrors Node's StreamBase consumption but uses enum dispatch since
/// Deno's CppGC doesn't have C++ virtual dispatch in Rust.
pub enum ConsumedStream {
  /// Direct TCP/Pipe: read via libuv callbacks, write via uv_write.
  Tcp {
    stream: *mut deno_core::uv_compat::UvStream,
  },
  /// TLS: cleartext consumer on TLSWrapInner. Reads come from TLSWrap's
  /// clear_out (decrypted), writes go through TLSWrap's rustls encryption.
  Tls {
    /// Opaque pointer to TLSWrapInner, used for h2_write_cleartext.
    tls_inner: *mut c_void,
    /// Alive flag cloned from TLSWrapInner, checked before deref.
    alive: Rc<Cell<bool>>,
    /// CppGC ref to TLSWrap to prevent GC while H2 holds a pointer.
    tls_wrap: cppgc::Ref<TLSWrap>,
  },
}
...
pub struct Session {
  ...
  pub stream: Option<ConsumedStream>,
...
impl Session {
  ...
  /// Release the consumed stream, cleaning up side tables and stopping reads.
  fn release_stream(stream: &mut Option<ConsumedStream>) {
    if let Some(consumed) = stream.take() {
      match consumed {
        ConsumedStream::Tcp { stream: tcp } => {
          H2_SESSIONS
            .with(|s| s.borrow_mut().remove(&(tcp as usize)));
          // SAFETY: tcp is a valid libuv handle
          unsafe {
            deno_core::uv_compat::uv_read_stop(tcp);
          }
        }
        ConsumedStream::Tls { .. } => {
          // TLS write-only path: just drop the reference.
          // Reads are handled by JS, TLS still owns TCP reads.
        }
      }
    }
  }
  ...
  pub fn send_pending_data(&mut self) {
    ...
    let consumed = match self.stream {
      Some(ref consumed) => consumed,
      None => {
    ...
    match consumed {
      ConsumedStream::Tcp { stream } => {
        let stream = *stream;
        self.send_pending_data_tcp(stream);
      }
      ConsumedStream::Tls {
        tls_inner, alive, ..
      } => {
        let tls_inner = *tls_inner;
        let alive = alive.clone();
        self.send_pending_data_tls(tls_inner, &alive);
      }
    }
  }

  /// Write pending H2 frames directly to a libuv TCP/Pipe stream.
  fn send_pending_data_tcp(
    &mut self,
    stream: *mut deno_core::uv_compat::UvStream,
  ) {
  ...
  /// Write pending H2 frames through TLS encryption via TLSWrapInner.
  fn send_pending_data_tls(
    &mut self,
    tls_inner: *mut c_void,
    alive: &Rc<Cell<bool>>,
  ) {
    ...
    loop {
      ...
      if src_len > 0 {
        // SAFETY: src and src_len are valid per nghttp2_session_mem_send contract
        let data = unsafe { std::slice::from_raw_parts(src, src_len as usize) };
        // SAFETY: tls_inner is valid (alive check above), points to TLSWrapInner
        unsafe {
          crate::ops::tls_wrap::h2_write_cleartext(tls_inner, data);
        }
      } else {
        break;
      }
  ...
  pub fn receive_data(&mut self, data: &[u8]) {
    ...
    // Complete deferred destroy: release the stream now that
    // send_pending_data has had a chance to send GOAWAY etc.
    if self.pending_destroy {
      self.pending_destroy = false;
      Self::release_stream(&mut self.stream);
    }
  }
...
impl Http2Session {
  ...
  /// Consume a stream handle for native H2 I/O.
  /// Accepts any LibUvStreamWrap-derived object (TCPWrap, PipeWrap, TLSWrap).
  /// For TCP/Pipe: installs libuv read callbacks and writes via uv_write.
  /// For TLS: registers as a cleartext consumer on TLSWrap.
  /// Mirrors Node's Http2Session::Consume(StreamBase*).
  #[nofast]
  fn consume_stream(
    &self,
    ...
    // Try TLSWrap first — it also inherits LibUvStreamWrap, so we must
    // check for TLS before falling back to the generic raw-stream path.
    if let Some(tls_wrap_ref) =
      deno_core::cppgc::try_unwrap_cppgc_persistent_object::<TLSWrap>(
        scope,
        handle_value,
      )
    {
      self.consume_tls_stream(session, tls_wrap_ref);
      return;
    }

    // Fall back to generic LibUvStreamWrap (TCP, Pipe, etc.)
    if let Some(stream_wrap) = deno_core::cppgc::try_unwrap_cppgc_object::<
      crate::ops::stream_wrap::LibUvStreamWrap,
    >(scope, handle_value)
    {
      self.consume_raw_stream(session, &stream_wrap);
    }
  }
  ...
  fn destroy(
    ...
    if session.is_sending {
      // We're inside receive_data's mem_recv. Defer the stream
      // release so that send_pending_data (called after mem_recv) can
      // still send pending frames (e.g. GOAWAY) before the socket
      // closes.
      session.pending_destroy = true;
    } else {
      // Release the consumed stream (TCP or TLS).
      Session::release_stream(&mut session.stream);
    }
...
/// Helper methods for consume_stream — not ops, just internal helpers.
impl Http2Session {
  fn consume_raw_stream(
    &self,
    session: &mut Session,
    stream_wrap: &crate::ops::stream_wrap::LibUvStreamWrap,
  ) {
    let stream = stream_wrap.stream_ptr();

    // Stop the existing reads (LibUvStreamWrap's reads)
    ...
    session.stream = Some(ConsumedStream::Tcp { stream });
  ...
  fn consume_tls_stream(
    &self,
    session: &mut Session,
    tls_wrap_ref: cppgc::Ref<TLSWrap>,
  ) {
    // For TLS, we only set up the WRITE path natively.
    // Reads still flow through JS: socket.on("data") → handle.receive().
    // This avoids disrupting TLSWrap's encrypted read pipeline while
    // eliminating the fragile JS sendPending override for writes.
    // SAFETY: tls_wrap_ref is valid; we get the inner pointer and alive flag.
    let (tls_inner, alive) =
      unsafe { tls_wrap_ref.get_write_handle() };

    session.stream = Some(ConsumedStream::Tls {
      tls_inner,
      alive,
      tls_wrap: tls_wrap_ref,
    });
  }
}

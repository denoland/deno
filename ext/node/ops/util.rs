...
/// Check if a file descriptor or resource ID is a terminal/TTY.
/// This is used by Node.js `tty.isatty(fd)`.
///
/// For fd 0, 1, 2 (stdin, stdout, stderr), we check the actual OS file descriptors.
/// For other values, we treat them as Deno resource IDs and look up the underlying
/// OS handle from the resource table.
#[op2(fast)]
pub fn op_node_is_terminal(state: &mut OpState, fd: i32) -> bool {
  if fd < 0 {
    return false;
  }

  // For stdin (0), stdout (1), stderr (2), check the actual OS file descriptors
  // These are guaranteed to be real OS fds, not Deno resource IDs
  if fd <= 2 {
    return is_terminal_raw_fd(fd);
  }

  // For other values, treat as Deno resource ID and look up in resource table
  let handle = match state.resource_table.get_handle(fd as u32) {
    Ok(handle) => handle,
    Err(_) => return false,
  };

  match handle {
    ResourceHandle::Fd(raw_fd) => is_terminal_handle(raw_fd),
    _ => false,
  }
}

#[cfg(unix)]
fn is_terminal_raw_fd(fd: i32) -> bool {
  use std::io::IsTerminal;
  // SAFETY: We're borrowing the fd briefly to check if it's a terminal.
  unsafe { std::os::fd::BorrowedFd::borrow_raw(fd).is_terminal() }
}

#[cfg(unix)]
fn is_terminal_handle(handle: ResourceHandleFd) -> bool {
  use std::io::IsTerminal;
  // SAFETY: The handle is obtained from the resource table.
  unsafe { std::os::fd::BorrowedFd::borrow_raw(handle).is_terminal() }
}

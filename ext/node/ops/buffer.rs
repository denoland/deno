...
use crate::ops::internal_binding_utils::ascii_to_bytes;
use crate::ops::internal_binding_utils::base64_to_bytes;
use crate::ops::internal_binding_utils::base64_url_to_bytes;
use crate::ops::internal_binding_utils::hex_to_bytes;
use crate::ops::internal_binding_utils::utf16le_to_bytes;
...
fn parse_fill_encoding(
  scope: &mut v8::PinScope,
  value: v8::Local<v8::Value>,
) -> FillEncoding {
  if !value.is_string() {
    return FillEncoding::Utf8;
  }
  ...
  match value
    ...
    .as_str()
  {
    "hex" => FillEncoding::Hex,
    "base64" => FillEncoding::Base64,
    "base64url" => FillEncoding::Base64Url,
    "ucs2" | "ucs-2" | "utf16le" | "utf-16le" => FillEncoding::Ucs2,
    "latin1" | "binary" | "ascii" => FillEncoding::Latin1,
    _ => FillEncoding::Utf8,
  }
}
...
fn string_fill_pattern(
  scope: &mut v8::PinScope,
  value: v8::Local<v8::Value>,
  encoding: FillEncoding,
) -> Option<Vec<u8>> {
  let str = value.to_string(scope)?.to_rust_string_lossy(scope);
  match encoding {
    FillEncoding::Utf8 => Some(str.into_bytes()),
    FillEncoding::Latin1 => Some(ascii_to_bytes(&str)),
    FillEncoding::Ucs2 => Some(utf16le_to_bytes(&str, None)),
    FillEncoding::Hex => Some(hex_to_bytes(&str)),
    FillEncoding::Base64 => base64_to_bytes(&str),
    FillEncoding::Base64Url => base64_url_to_bytes(&str),
  }
}
...
fn fill_callback(
  ...
  let pattern = if value.is_array_buffer_view() {
    ...
    let encoding = parse_fill_encoding(scope, args.get(4));
    string_fill_pattern(scope, value, encoding).unwrap_or_default()
  } else {

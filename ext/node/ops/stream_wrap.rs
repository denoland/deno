...
/// callback with a 65536-byte suggested size on every read. In Node,
/// the equivalent path allocates via `v8::ArrayBuffer::NewBackingStore`
/// which goes through V8's pointer-compression sandbox / memory cage
/// — a reserved virtual address region that V8 manages internally.
/// Backing-store alloc/free inside the cage do not go through the OS
/// allocator, so Node's sustained-HTTP profile shows zero
/// `mach_vm_reclaim_*` traps.
///
/// We allocate read buffers with `std::alloc::alloc` which goes
/// through the macOS xzone allocator. At ~70k reads/sec of a fixed
/// 64KB size, every free triggers a `mach_vm_reclaim_*` kernel trap
/// (~6% of CPU in the pre-pool profile). Pooling gives us the same
/// effect as V8's cage: the 64KB slab stays owned by the runtime
/// and is reused across reads instead of handed back to the kernel.
///
/// The pool is capped so long-lived idle processes don't retain
/// excess memory. Non-65536 sizes skip the pool entirely.
const POOLED_BUF_SIZE: usize = 65536;
const POOLED_BUF_MAX: usize = 8;

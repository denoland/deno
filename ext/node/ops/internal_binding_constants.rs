...
fn parse_json_obj<'s>(
  ...
  let value = v8::json::parse(scope, source).unwrap();
...
fn set_infinity(
  ...
  let key = v8::String::new(scope, name).unwrap();
  let value = v8::Number::new(scope, f64::INFINITY);
  obj.set(scope, key.into(), value.into());
}

#[op2]
pub fn op_node_internal_binding_constants<'s>(
  scope: &mut v8::PinScope<'s, '_>,
) -> v8::Local<'s, v8::Object> {
  let obj = v8::Object::new(scope);

  let os_json = if cfg!(target_os = "macos") {
    OS_DARWIN_JSON
  ...
  let os = parse_json_obj(scope, os_json);
  freeze_property_object(scope, os, "signals");
  set_value(scope, obj, "os", os.into());

  let fs =
    serde_v8::to_v8(scope, crate::ops::constant::node_fs_constants()).unwrap();
  set_value(scope, obj, "fs", fs);

  let crypto = parse_json_obj(scope, CRYPTO_JSON);
  set_value(scope, obj, "crypto", crypto.into());

  let zlib = parse_json_obj(scope, ZLIB_JSON);
  set_infinity(scope, zlib, "Z_MAX_CHUNK");
  set_value(scope, obj, "zlib", zlib.into());

  let trace = parse_json_obj(scope, TRACE_JSON);
  set_value(scope, obj, "trace", trace.into());

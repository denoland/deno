...
#[cfg(unix)]
pub type RawPipeHandle = std::os::fd::RawFd;

#[cfg(windows)]
pub type RawPipeHandle = std::os::windows::raw::Handle;
...
impl BiPipe {
  pub fn from_raw(raw: RawPipeHandle) -> Result<Self, std::io::Error> {
    let (read_end, write_end) = pipe(raw)?;
    Ok(Self {
      read_end,
      write_end,
    })
...
impl deno_core::Resource for BiPipeResource {
  ...
  //   async move {
  //     let mut read_half = RcRef::map(self, |r| &r.read_half).borrow_mut().await;
  ...
  //       })
  //     } else {
  //       Ok(deno_core::WriteOutcome::Full { nwritten: written })
  //     }
  //   }
  //   .boxed_local()
  // }
...
pub struct BiPipeResource {
  ...
  cancel: CancelHandle,
  raw_handle: RawPipeHandle,
}

impl BiPipeResource {
  pub fn from_raw_handle(raw: RawPipeHandle) -> Result<Self, std::io::Error> {
...
impl tokio::io::AsyncWrite for BiPipe {
  fn poll_write_vectored(
    self: std::pin::Pin<&mut Self>,
    cx: &mut std::task::Context<'_>,
    bufs: &[std::io::IoSlice<'_>],
  ) -> std::task::Poll<Result<usize, std::io::Error>> {
    self.project().write_end.poll_write_vectored(cx, bufs)
  }

  fn is_write_vectored(&self) -> bool {
    self.write_end.is_write_vectored()
  }
  ...
  fn poll_write(
    ...
    self.project().write_end.poll_write(cx, buf)
  }
  ...
  fn poll_flush(
    ...
    self.project().write_end.poll_flush(cx)
  }
  ...
  fn poll_shutdown(
    ...
    self.project().write_end.poll_shutdown(cx)
  }
}
...
impl tokio::io::AsyncWrite for BiPipeWrite {
  fn poll_write_vectored(
    self: std::pin::Pin<&mut Self>,
    cx: &mut std::task::Context<'_>,
    bufs: &[std::io::IoSlice<'_>],

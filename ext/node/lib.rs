...
deno_core::extension!(deno_node,
  ...
  customizer = |ext: &mut deno_core::ExtensionBuilder| {
    ...
    let mut esm_sources = include_js_files!(
      deno_node
      dir "polyfills",
      "00_globals.js",
      "02_init.js",
    ...
    for (specifier, path) in built_in_modules {
      esm_sources.push(ExtensionFileSource {
        specifier,
        code: ExtensionFileSourceCode::LoadedFromFsDuringSnapshot(
          PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("polyfills").join(path)
        ),
      });
    }

    ext.esm(esm_sources);
    ext.esm_entry_point("ext:deno_node/02_init.js");
  }
);

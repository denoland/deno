...
/** https://nodejs.org/api/process.html#processexecveexecpath-args-env */
export function execve(
  execPath: string,
  args: string[] = [],
  envArg: Record<string, string> = process.env,
): never {
  if (internals.__isWorkerThread) {
  ...
  validateObject(envArg, "env");
  for (const key of Object.keys(envArg)) {
    const value = envArg[key];
    if (
      typeof key !== "string" ||
      typeof value !== "string" ||
      key.includes("\u0000") ||
  ...
  // Deno does not implement the execve(2) syscall, so the call always fails.

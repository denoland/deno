...
const { isError, lazyDOMException } = core.loadExtScript(
  ...
  #resolvePendingWrites() {
    while (
      ...
      for (let i = 0; i < pending.chunks.length; i++) {
        this.#bytesWritten += TypedArrayPrototypeGetByteLength(
          pending.chunks[i],
        );
      }
      pending.resolve();
...
class PushWriter {
  ...
  writeSync(chunk) {
    ...
    if (
      !result && this.#queue.backpressurePolicy === "block" &&

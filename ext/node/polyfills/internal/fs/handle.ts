  ...
  #close(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        reject(denoErrorToNodeError(err, { syscall: "close" }));
      }
      this.#rid = -1;
      resolve();
    });
...
function readPromise(
  ...
  if (ObjectPrototypeIsPrototypeOf(Uint8ArrayPrototype, bufferOrOpt)) {
    ...
    return new Promise((resolve, reject) => {
      readAsync(rid, bufferOrOpt, (err, bytesRead, buffer) => {
        if (err) reject(err);
        else resolve({ buffer, bytesRead });
      });
    });
...
function writePromise(
  ...
  if (ObjectPrototypeIsPrototypeOf(Uint8ArrayPrototype, bufferOrStr)) {
    ...
    return new Promise((resolve, reject) => {
      writeAsync(rid, str, position, encoding, (err, bytesWritten, buffer) => {
        if (err) reject(err);
        else resolve({ buffer, bytesWritten });
      });
    });
...
async function fsCall<P, R, T extends (...args: P[]) => Promise<R>>(
  fn: T,
  fnName: string,
  handle: FileHandle,
  ...args: P[]

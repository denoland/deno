...
// This file is a port of Node.js `lib/internal/zip/archive.js` (nodejs/node#64339).
...
const { Readable } = core.createLazyLoader("node:stream")();
...
function createZipArchive(entries, options) {
  return Readable.from(generateZipArchive(entries, options), {
    objectMode: false,
  });
}
...
async function* fileEntries(files, followSymlinks) {
  for await (const pair of files) {
    ...
    const stats = followSymlinks
      ? await fsStatAsync(sourcePath)
      : await fsLstatAsync(sourcePath);
    const options = {
      __proto__: null,
    ...
    if (stats.isSymbolicLink()) {
      yield ZipEntry.createSymlink(
        name,
        await fsReadlinkAsync(sourcePath),
        options,
      );
      ...
      const flags = followSymlinks
        ? lazyFs().constants.O_RDONLY
        : lazyFs().constants.O_RDONLY | (lazyFs().constants.O_NOFOLLOW || 0);
      const handle = await lazyFs().promises.open(sourcePath, flags);
      let ownsHandle = true;
      try {
        ...
        const stream = lazyFs().createReadStream(undefined, { fd: handle });
        ownsHandle = false;
        let handedOff = false;
        try {
...
async function disposeQueuedEntries(iterator, isAsync) {
  ...
  for (let next = iterator.next(); !next.done; next = iterator.next()) {
    ...
    if (typeof entry?.[SymbolAsyncDispose] === "function") {
      await entry[SymbolAsyncDispose]();
    }
  }
}
...
async function* generateZipArchive(entries, options) {
  ...
  const iterator = isAsync
    ? entries[SymbolAsyncIterator]()
    : entries[SymbolIterator]();
  let completed = false;
  try {
    while (true) {
    ...
    const trailer = buildArchiveTrailer(
      count,
...
function* createZipArchiveSync(entries, options) {
  ...
  try {
    ...
    if (!completed) {
      if (typeof current?.[SymbolDispose] === "function") {
        current[SymbolDispose]();
      }
      for (let next = iterator.next(); !next.done; next = iterator.next()) {
        const entry = next.value;
        if (typeof entry?.[SymbolDispose] === "function") {
          entry[SymbolDispose]();
        }
      }
    }

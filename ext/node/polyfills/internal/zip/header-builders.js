...
// This file is a port of Node.js `lib/internal/zip/header-builders.js` (nodejs/node#64339).
...
const { encodeDosDateTime } = core.loadExtScript(
  "ext:deno_node/internal/zip/dos.js",
);
const { writeSafeUint64 } = core.loadExtScript(
  "ext:deno_node/internal/zip/binary.js",
);
const { buildExtTimestampExtra } = core.loadExtScript(
...
function buildLocalHeader(meta) {
  ...
  const zip64 = streaming ||
    meta.compressedSize >= SENTINEL32 ||
    meta.uncompressedSize >= SENTINEL32;
  const ts = meta.extendedMtime !== null
    ? buildExtTimestampExtra(meta.extendedMtime)
    : EMPTY_BUFFER;
...
function buildCentralHeader(meta, localOffset) {
  ...
  const ts = meta.extendedMtime !== null
    ? buildExtTimestampExtra(meta.extendedMtime)
    : EMPTY_BUFFER;
  const extraLength = (zip64Fields ? 4 + 8 * zip64Fields : 0) + ts.length +
    meta.extra.length;
  checkExtraLength(extraLength);
...
function buildArchiveTrailer(
  ...
  if (zip64) {
    ...
    ArrayPrototypePush(
      chunks,
      buildZip64EndRecord(count, centralDirectorySize, centralDirectoryOffset),
    );

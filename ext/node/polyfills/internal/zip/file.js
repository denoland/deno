...
// This file is a port of Node.js `lib/internal/zip/file.js` (nodejs/node#64339).
...
function buildCentralDirectoryChunks(records, centralDirectoryOffset, comment) {
  ...
  for (let i = 0; i < centralHeaders.length; i++) {
    ArrayPrototypePush(chunks, centralHeaders[i]);
  }
  ...
  for (let i = 0; i < trailer.length; i++) {
    ArrayPrototypePush(chunks, trailer[i]);
  }
...
function checkArchiveEnd(end, size) {
  if (end.centralDirectorySize > kMaxLength) {
    throw new ERR_ZIP_ARCHIVE_TOO_LARGE(
      "the central directory is too large to buffer",
    );
  }
...
// looser lower bound.
async function validateMemberBounds(
  fd,
  headers,
  prefix,
  size,
...
function validateMemberBoundsSync(
  fd,
  headers,
  prefix,
  size,
...
function checkMemberOverlap(members, centralDirectoryOffset) {
  ...
  for (let i = 0; i < members.length; i++) {
    const bound = i + 1 < members.length
      ? members[i + 1].offset
      : centralDirectoryOffset;
...
class ZipFile {
  ...
  // #assertNotBusy(). Failures do not break the chain (both arms continue it).
    ...
    this.#queue = PromisePrototypeThen(
      result,
      () => undefined,
      () => undefined,
    );
  ...
  async #doAdd(entry) {
    ...
    MapPrototypeSet(this.#entries, entry.name, {
      central: null,
      entry,
      localOffset,
    });
  ...
  #rewriteCentralDirectorySync() {
    ...
    lazyFs().ftruncateSync(this.#handle.fd, pos);
    this.#adoptRewrittenCentralDirectory(names, centralHeaders, records);
  }
  ...
  compactSync(comment) {
    ...
    const chunks = [];
    for (
      const chunk of createZipArchiveSync(entries, {
  ...
  *entriesSync() {
  ...
  closeSync() {
    ...
    this.#handle.closed = true;
    MapPrototypeClear(this.#entries);
    lazyFs().closeSync(this.#handle.fd);
  }
  ...
  static openSync(filename, options) {
    ...
    const writable = options?.writable ?? false;
    validateBoolean(writable, "options.writable");
    const fd = lazyFs().openSync(filename, writable ? "r+" : "r");
    try {
      const size = lazyFs().fstatSync(fd).size;
      const tailLength = MathMin(size, TAIL_LENGTH);
      const tail = Buffer.allocUnsafe(tailLength);

...
// This file is a port of Node.js `lib/internal/zip/compression.js` (nodejs/node#64339).
...
const { compose } = core.createLazyLoader("node:stream")();
...
let zlib;
function lazyZlib() {
  zlib ??= core.createLazyLoader("node:zlib")();
  return zlib;
}
...
function assertDecodable(info, options) {
  ...
  if (
    info.method !== METHOD_STORE && info.method !== METHOD_DEFLATE &&
    info.method !== METHOD_ZSTD
  ) {
    throw new ERR_ZIP_UNSUPPORTED_FEATURE(
      `entry ${
        JSONStringify(info.name)
      } uses compression method ${info.method}`,
    );
  }
  if (
    options?.maxSize !== undefined && info.uncompressedSize > options.maxSize
  ) {
    throw new ERR_ZIP_ENTRY_TOO_LARGE(
      `entry ${
        JSONStringify(info.name)
      } declares ${info.uncompressedSize} bytes, ` +
  ...
  if (err?.code === "ERR_BUFFER_TOO_LARGE") {
    throw new ERR_ZIP_ENTRY_CORRUPT(
      `entry ${JSONStringify(info.name)} ` +
        `${
          method === METHOD_DEFLATE ? "inflates" : "decompresses"
        } beyond its ` +
...
function checkDecoded(data, info, verify) {
  if (data.length !== info.uncompressedSize) {
    throw new ERR_ZIP_ENTRY_CORRUPT(
      `entry ${
        JSONStringify(info.name)
      } produced ${data.length} bytes, expected ` +
...
async function* decodeMemberStream(source, info, options) {
  ...
  let produced = 0;
  let state = 0;
  const decoded = info.method === METHOD_DEFLATE
    ? inflateRawStream(source)
    : info.method === METHOD_ZSTD
    ? zstdDecompressStream(source)
    : source;
  for await (const chunk of decoded) {
    ...
    if (produced > info.uncompressedSize) {
      throw new ERR_ZIP_ENTRY_CORRUPT(
        `entry ${
          JSONStringify(info.name)
        } inflates beyond its declared size of ` +
...
function decodeMemberSync(compressed, info, options) {
  ...
  if (info.method === METHOD_DEFLATE) {
    ...
    try {
      data = lazyZlib().zstdDecompressSync(compressed, {
        maxOutputLength: cap,
      });
    } catch (err) {
      rethrowDecodeFailure(err, info, METHOD_ZSTD);

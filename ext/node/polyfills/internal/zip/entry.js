...
// This file is a port of Node.js `lib/internal/zip/entry.js` (nodejs/node#64339).
...
const { crc32: crc32Native } = core.loadExtScript(
  "ext:deno_node/_zlib_binding.mjs",
);
const {
  EMPTY_BUFFER,
...
const {
  extraFieldMtime,
  stripZip64Extra,
} = core.loadExtScript("ext:deno_node/internal/zip/extra-fields.js");
const {
  decodeZipName,
  decodeZipText,
...
const {
  ...
  LocalFileHeader,
  findArchiveEnd,
} = core.loadExtScript("ext:deno_node/internal/zip/headers.js");
const {
  buildLocalHeader,
  buildCentralHeader,
  buildDataDescriptor64,
} = core.loadExtScript("ext:deno_node/internal/zip/header-builders.js");
const {
  deflateRawAsync,
  deflateRawSync,
  zstdCompressAsync,
  ...
  decodeMemberAsync,
  decodeMemberSync,
} = core.loadExtScript("ext:deno_node/internal/zip/compression.js");
const {
  readFdFully,
  readFdFullySync,
} = core.loadExtScript("ext:deno_node/internal/zip/fs-util.js");
const { getMaxZipContentSize } = core.loadExtScript(
  "ext:deno_node/internal/zip/content-size.js",
);
...
function extendedMtimeSeconds(mtimeMs) {
  ...
  return (mtimeMs % 2000 !== 0 && seconds >= -2147483648 &&
      seconds <= 2147483647)
    ? seconds
    : null;
}
...
function createEntryMeta(filename, options) {
  ...
  const mode = options?.mode ??
    (isSymlink ? 0o777 : isDirectory ? 0o755 : 0o644);
  ...
  const modified = options?.modified ??
    new Date(MathFloor(DateNow() / 2000) * 2000);
  if (!isDate(modified)) {
    throw new ERR_INVALID_ARG_TYPE("options.modified", "Date", modified);
  }
  if (
    options?.method !== undefined &&
    options.method !== "deflate" && options.method !== "store" &&
    options.method !== "zstd"
  ) {
    throw new ERR_INVALID_ARG_VALUE(
      "options.method",
      options.method,
      "must be 'deflate', 'store', or 'zstd'",
    );
  }
...
class ZipEntry {
  ...
  // buffer, only a descriptor *handle* ({ fd, closed }) shared with the
  // owning ZipFile plus the local-header offset, and reads its compressed
  // bytes from disk on demand (see #compressedBytes/#rawChunks). The shared
  // handle lets close() invalidate every outstanding entry at once, so a
  // read never falls through to a bare (possibly reused) fd number.
  ...
  get compressed() {
    return this.method !== METHOD_STORE;
  }
  get rawContent() {
    return this.#content;
  }
  ...
  get isDirectory() {
    return StringPrototypeEndsWith(this.name, "/");
  }
  ...
  #finalizeMeta() {
    ...
    const extendedMtime = extraFieldMtime(extra) === null
      ? extendedMtimeSeconds(modified.getTime())
      : null;
    const meta = {
  ...
  // The live numeric descriptor for a file-backed entry, or a clean state
  // error if the ZipFile that owns it has since been closed - never a raw
  // (possibly OS-reused) fd number. Returns null for an in-memory entry.
  // A file-backed entry holds a descriptor *handle* ({ fd, closed }) shared
  // with its ZipFile, not a bare fd, so close() is observable here.
  #liveDescriptor() {
  ...
  async #resolveLocalHeader() {
    ...
    const fd = this.#liveDescriptor();
    const fixed = Buffer.allocUnsafe(30);
    await readFdFully(fd, fixed, this.#localOffset);
  ...
  #resolveLocalHeaderSync() {
    ...
    const fd = this.#liveDescriptor();
    const fixed = Buffer.allocUnsafe(30);
    readFdFullySync(fd, fixed, this.#localOffset);
    if (fixed.readUInt32LE(0) !== SIG_LOCAL_FILE_HEADER) {
      throw new ERR_ZIP_INVALID_ARCHIVE(
        `entry ${JSONStringify(this.name)} has an invalid local file header`,
      );
  ...
  async #compressedBytes() {
    ...
    await readFdFully(this.#liveDescriptor(), compressed, start);
    return compressed;
  }
  ...
  #compressedBytesSync() {
    ...
    const compressed = Buffer.allocUnsafe(size);
    readFdFullySync(this.#liveDescriptor(), compressed, start);
    return compressed;
  }
  ...
  *#rawChunksSync() {
    ...
    while (remaining > 0) {
      ...
      const chunk = Buffer.allocUnsafe(take);
      // Re-check per chunk: the ZipFile may be closed mid-stream.
      readFdFullySync(this.#liveDescriptor(), chunk, pos);
      pos += take;
      remaining -= take;
      yield chunk;
    }
  ...
  contentIterator(options) {
    // No default maxSize here, unlike content(): streaming is the
    // bounded-memory path for arbitrarily large members, so imposing content()'s
    // buffer-oriented ceiling would defeat its purpose (and cap legitimate
    // multi-gigabyte reads). Output is still bounded per chunk to the declared
    // uncompressed size; a caller that wants an explicit cap passes
    // options.maxSize.
  ...
  *[SymbolIterator]() {
    if (this.#source) {
      throw new ERR_INVALID_STATE(
        "a streaming entry cannot be serialized synchronously",
      );
    }
  ...
  async *[SymbolAsyncIterator]() {
    ...
    if (this.#serialized) {
      throw new ERR_INVALID_STATE(
        "a streaming entry can only be serialized once",
      );
    }
    ...
    const output = meta.method === METHOD_DEFLATE
      ? deflateRawStream(counted)
      : meta.method === METHOD_ZSTD
      ? zstdCompressStream(counted)
      : counted;
  ...
  async [SymbolAsyncDispose]() {
    const source = this.#releaseSource();
    if (source === null) return;
    if (typeof source[SymbolAsyncDispose] === "function") {
      await source[SymbolAsyncDispose]();
  ...
  [kFinalize](localOffset, preserveDescriptorFlag = false) {
    ...
    const meta = this.#finalizeMeta();
    if (
      preserveDescriptorFlag && this.#central !== null &&
      (this.#central.flags & FLAG_DATA_DESCRIPTOR) !== 0
    ) {
      return buildCentralHeader(
        { ...meta, flags: meta.flags | FLAG_DATA_DESCRIPTOR },
        localOffset,
      );
    }
  ...
  static #prepareCreate(filename, data, options) {
    ...
    if (isDirectory && content.length) {
      throw new ERR_INVALID_ARG_VALUE(
        "data",
        data,
        "must be empty for a directory entry",
      );
    }
  ...
  static async create(filename, data, options) {
    const { meta, content, method } = ZipEntry.#prepareCreate(
      filename,
      data,
      options,
    );
    const compressed = method === METHOD_DEFLATE
      ? await deflateRawAsync(content)
      : method === METHOD_ZSTD
      ? await zstdCompressAsync(content)
      : null;
  ...
  static createStream(filename, source, options) {
    ...
    if (StringPrototypeEndsWith(filename, "/")) {
      throw new ERR_INVALID_ARG_VALUE(
        "filename",
        filename,
        "a directory entry cannot be streamed",
      );
    }
    meta.flags |= FLAG_DATA_DESCRIPTOR;
    meta.method = options?.method === "store"
      ? METHOD_STORE
      : options?.method === "zstd"
      ? METHOD_ZSTD
      : METHOD_DEFLATE;
...
function* readArchiveEntries(buf, end) {
  ...
  for (let index = 0; index < end.totalRecords; index++) {
    ...
    if (pos + central.byteLength > cdEnd) {
      throw new ERR_ZIP_INVALID_ARCHIVE(
        "central directory header is out of bounds",
      );
    }
    ...
    const content = length
      ? buf.subarray(dataStart, dataStart + length)
      : EMPTY_BUFFER;
  ...
  // directory order.
  const ranges = ArrayPrototypeSort(
    ArrayPrototypeSlice(parsed),
    (a, b) => a.start - b.start,
  );
  for (let i = 0; i < ranges.length; i++) {
    const bound = i + 1 < ranges.length
      ? ranges[i + 1].start

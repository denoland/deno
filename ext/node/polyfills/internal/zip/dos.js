...
// This file is a port of Node.js `lib/internal/zip/dos.js` (nodejs/node#64339).
...
const { isUtf8 } = core.loadExtScript(
...
const { forEachExtraField } = core.loadExtScript(
...
function encodeDosDateTime(value) {
  ...
  const date = ((year - 1980) << 9) | ((value.getMonth() + 1) << 5) |
    value.getDate();
...
function unicodePathName(extra, standardNameBuffer) {
  ...
  forEachExtraField(extra, (id, body) => {
    if (result !== null || id !== EXTRA_ID_UNICODE_PATH || body.length < 5) {
      return;
    }
    if (body[0] !== 1) return;

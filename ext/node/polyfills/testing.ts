...
// ---------------------------------------------------------------------------
// Snapshot testing: `t.assert.snapshot()` / `t.assert.fileSnapshot()` and the
// `snapshot` module export.
//
...
function getNodeFs() {
  if (_nodeFs === null) {
    _nodeFs = core.createLazyLoader("node:fs")();
  }
  return _nodeFs;
}
let _nodePath = null;
function getNodePath() {
  if (_nodePath === null) {
    _nodePath = core.createLazyLoader("node:path")();
  }
  return _nodePath;
}
let _nodeVm = null;
...
// Detecting `--update-snapshots` relies on a CLI op that is only present when
// running under `deno test`. Outside the test runner (e.g. `deno run`) the op
// is absent, so update mode is simply off.
let _snapshotUpdateMode;
function isSnapshotUpdateMode() {
  if (_snapshotUpdateMode === undefined) {
    const op = getSnapshotOps().op_test_snapshot_in_update_mode;
    if (typeof op === "function") {
      try {
        _snapshotUpdateMode = op();
      } catch {
...
function snapshotThrowReadError(err, filename) {
...
function snapshotThrowWriteError(err, filename) {
  const error = new ERR_INVALID_STATE(
    `Cannot write snapshot file '${filename}.'`,
  );
  error.cause = err;
  error.filename = filename;
  throw error;
...
class SnapshotManager {
  ...
  serializeWithoutEscape(input, serializers) {
    try {
      return snapshotSerializeValue(input, serializers);
    } catch (err) {
      snapshotThrowSerializationError(input, err);
    }
  }
  ...
  assertFileSnapshot(actual, path, options = { __proto__: null }) {
    validateString(path, "path");
    validateObject(options, "options");
    const { serializers = snapshotSerializerFns } = options;
    validateSnapshotSerializers(serializers, "options.serializers");
    const value = this.serializeWithoutEscape(actual, serializers);
    // `fileSnapshot` writes to an arbitrary user-supplied path, so it is not a
    // runner-managed location and goes through normal `node:fs` (subject to
    // read/write permissions).
    const fs = getNodeFs();
    if (isSnapshotUpdateMode()) {
      try {
        fs.mkdirSync(getNodePath().dirname(path), { recursive: true });
        fs.writeFileSync(path, value, "utf8");
      } catch (err) {
        snapshotThrowWriteError(err, path);
      }
    } else {
      let expected;
      try {
        ...
        snapshotThrowReadError(err, path);
      }
...
// Build the `{ snapshot, fileSnapshot }` methods bound to a single test's
// origin and full name. Shared by both context implementations.
function makeSnapshotAssertMethods(getOriginUrl, getFullName) {
  return {
    ...
    fileSnapshot(actual, path, options) {
      return snapshotManager.assertFileSnapshot(actual, path, options);
    },
  };
}
...
class TapContext {
  ...
  get assert() {
    if (this.#assertObject === undefined) {
      ...
      obj.fileSnapshot = methods.fileSnapshot;
...
class NodeTestContext {
  ...
  get assert() {
    if (this.#plan) {
      if (!this.#planAssert) {
        ...
        wrapped.fileSnapshot = function (...args) {
          plan.increment();
          return ReflectApply(snap.fileSnapshot, this, args);
        };
    ...
    if (this.#assertObject === undefined) {
      ...
      obj.fileSnapshot = snap.fileSnapshot;
      this.#assertObject = obj;
    }
    return this.#assertObject;
  }

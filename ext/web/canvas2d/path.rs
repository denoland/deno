...
use crate::canvas2d::v8_util::to_number_guarded;
...
pub(super) fn build_round_rect_path(
  ...
  // Clockwise when `w` and `h` share a sign, else counterclockwise (nonzero
  // fill cancels opposite windings). Corners are 90°; 0 = +x toward +y.
  if (w < 0.0) == (h < 0.0) {
...
/// Append `shape` under `transform`, keeping its subpaths (rect/roundRect).
...
/// Append `shape` under `transform`. A leading `MoveTo` becomes `LineTo`
/// when `dest` is non-empty (arcTo continues the current subpath).
...
/// Shared `arcTo()`. `radius` is already finite and non-negative.
/// Inputs are in pre-transform space; `dest` stores post-transform points.
#[allow(
  clippy::too_many_arguments,

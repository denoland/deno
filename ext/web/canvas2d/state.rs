...
#[derive(WebIDL, Clone, Copy, Default, PartialEq, Eq)]
#[webidl(enum)]
...
impl CanvasFillRule {
  #[inline]
  pub(super) fn to_peniko(self) -> peniko::Fill {
    match self {
...
/// `beginLayer()` filter, as supplied (SVG primitives or CSS string).
...
/// `u16` max (`vello_cpu`); GPUs do not exceed this. Enforced at creation.

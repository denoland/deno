...
/// `<length>` resolution for a font. No root element, so `rem` == `em`.
...
/// Font-relative metrics: `cap`/`ex`/`lh` from the first face; `ch`/`ic`
/// scan for a glyph; missing values use CSS fallback ratios.

...
// Always available; excluded from descriptor matching in check()/load().
...
/**
 * CSS font-weight string to number. Number() rejects "700px".
 * @param {string} w
 * @returns {number}
 */
...
/**
 * CSS Font Loading "find the matching font faces": descriptors then unicode-range.
...
function matchFontFaces(set, font, text) {
  ...
  // Rust returns numeric weight; face.weight is a string.
...
class FontFace {
  ...
  constructor(family, source, descriptors = { __proto__: null }) {
    ...
    if (descriptors.stretch !== undefined) {
      ...
      if (isValidFontStretch(v)) {
        ...
        this.#invalidate(`${prefix}: Invalid value for 'stretch' descriptor.`);
      }
    }
  ...
  load() {
    ...
    this.#loadPromise = (async () => {
      try {
        const { handle, weight, style, stretch, unicodeCoverage } = await this
          .#loadSource();
        this.#handle = handle;
   ...
   * @returns {Promise<{ handle: number, weight: number, style: string, stretch: string, unicodeCoverage: [number, number][] }>}
   */
  async #loadSource() {
    if (this.#bytes !== null) {
  ...
  #register() {
    op_fontdb_add(
      ...
      this.#stretchUserSet ? this.#stretch : "",
    );
  }
...
// Value-iterable methods by identity with Array.prototype.
// https://webidl.spec.whatwg.org/#es-iterable
installValueIterable(FontFacePalette.prototype);
...
installValueIterable(FontFacePalettes.prototype);
...
/**
 * WebIDL iterable<T>: methods are === Array.prototype counterparts.
 * @param {object} proto
 */
function installValueIterable(proto) {
  // idlharness requires === Array.prototype methods (identity equality), so
  // the uncurried primordials cannot be used here.
  // deno-lint-ignore deno-internal/prefer-primordials
  const arrayProto = Array.prototype;
  const methodNames = ["entries", "keys", "values", "forEach"];
  for (let i = 0; i < methodNames.length; ++i) {
    const name = methodNames[i];
    ObjectDefineProperty(proto, name, {
      __proto__: null,
      value: arrayProto[name],
      writable: true,
      enumerable: true,
      configurable: true,
    });
  }
  ObjectDefineProperty(proto, SymbolIterator, {
    __proto__: null,
    // deno-lint-ignore deno-internal/prefer-primordials
    value: arrayProto[Symbol.iterator],
    writable: true,
    enumerable: false,
    configurable: true,
  });

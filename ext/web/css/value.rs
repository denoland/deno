...
impl FontMetrics {
  ...
  /// The values CSS mandates when a metric cannot be determined. `cap` has
  /// none, so this reuses the 0.8em ascent assumed elsewhere.
  /// https://www.w3.org/TR/css-values-4/#font-relative-lengths
  #[inline]
...
/// The context a relative `<length>` resolves against. `new` builds one with no
/// root element and no viewport: the element and root metrics are the same font,
/// and the initial containing block is zero-sized.
#[derive(Clone, Copy, Debug, PartialEq)]
...
impl LengthUnit {
  ...
  /// The size this unit is 1% of, `None` for a unit that is no percentage.
  /// Neither a retractable browser UI, a query container nor a writing mode is
  /// modelled, so every box-relative one reads the same box on an inline axis
...
impl Length {
  ...
  /// The pixel value of a length the calculation engine produced. Every
  /// dimension it sees is folded already -- a unit only survives at the top
  /// level, which the additive and multiplicative layers never see, both
  /// `Token::ParenthesisBlock` and the math functions needing
  /// `function_depth > 0` -- so an unfolded one here is a parser bug.
  #[inline]
...
/// The numeric types a single CSS value can have, i.e. the dimensions with a
/// canonical unit.
...
impl MathValue {
  ...
  /// The result of an operation. `build` runs -- and the tree allocates -- only
  /// when `keep` says an operand reads the resolution context, or when the
  /// dimension is compound and no literal could stand in. Anything else is
  /// exact in its canonical unit already.
...
impl NumericAccumulator {
  /// Reads one operand, recursing into the math functions. The expression
  /// layers below are only reachable from inside one, since both a
  /// parenthesis and a function need `function_depth`.
  fn parse<'i, 't>(
        ...
        let result = match_ignore_ascii_case! { &name,
          ...
          "sign" => {
            input.parse_nested_block(|arguments| {
              ...
              let value = try_extract!(acc, expect_operand(), arguments);
              arguments.expect_exhausted()?;
              // The sign is that of the computed value, not the number as
              // written. Every unit factor is positive, so the two only differ
              // where a metric resolves to zero, as a viewport unit does here.
...
/// to pixels, the literal riding along so it still serializes as written.
fn relative_length(
  length: Length,
...
pub enum NumericValue {
  /// The literal `0`, which `<transform-function>` accepts for a `<length>` or
  /// an `<angle>`. Only ever produced outside a math function.
  /// https://www.w3.org/TR/css-values-4/#zero-value
  Zero,
  Number(f64),
...
impl NumericValue {
  ...
  /// Extracts a `<length>`. A math function comes back as the pixel value it
  /// folded to at parse time; callers that have to survive a later font change
  /// want [`SpecifiedNumericValue::parse`] instead.
  #[inline]
  ...
  /// The unit a sum term sorts by. The empty string sorts a `<number>` first for
  /// free: it orders below `%`, itself below every unit's first letter.
...
/// A calculation kept in symbolic form, so a value depending on the resolution
/// context serializes as written and resolves when it is used.
///
/// A node resolves to the canonical unit of its own dimension -- `px`, `deg`,
/// or the bare number -- and carries none itself, [`Dimension`] tracking that.
/// Hence one tree can hold a dependency that leaves the length dimension, as
/// in `calc(sqrt(1em / 1px) * 1px)`.
...
enum CalcNode {
  ...
  Exp(Box<CalcNode>),
  /// `degrees` marks an `<angle>` argument, which resolves to `deg` and so needs
...
impl CalcNode {
  ...
  /// Folds literal `<number>` factors into the one remaining operand, so
  /// `1em * 2` is `2em`. Nested products flatten, multiplication being
  /// associative.
  ...
  /// Whether a relative `<length>` contributed anywhere, which is what makes
  /// the symbolic form worth retaining. A node that is not itself a `<length>`
  /// still answers yes when one reached it, as `atan2(1em, 1px)` does.
  fn is_relative_length(&self) -> bool {
    match self {
  ...
  /// The unit a term is sorted by. Anything but a literal returns `None` and
  /// lands in the trailing group in authored order, which is step 5 of "sort a
  ...
  /// The CSSOM serialization: `calc()` wraps an operator tree, a named function
  /// serializes as itself, and a tree simplified to one literal as that literal.
...
/// A numeric value as specified, for a property that serializes it back on
/// getting: relative units only resolve when the value is used. A single literal
/// needs no case of its own, being a `CalcNode` leaf, and `Rc` keeps cloning
/// the value along with its owner cheap.
#[derive(Clone, Debug, PartialEq)]
...
mod tests {
  ...
  fn box_relative_units_read_the_resolution() {
    // Every in-tree caller passes a zero-sized viewport, so these paths are
    // only observable through a resolution built by hand.
    // https://www.w3.org/TR/css-values-4/#viewport-relative-lengths
    // https://drafts.csswg.org/css-conditional-5/#container-lengths
  ...
  fn plain_parse_builds_no_tree() {
    // `NumericValue::parse` discards the symbolic form, so `retain_specified`
    // withholds the literal in `relative_length` and no math function allocates,
    // even for a relative unit. The folded value is unaffected.

...
pub fn op_event_dispatch<'a>(
  ...
  let event =
    cppgc::try_unwrap_cppgc_proto_object::<Event>(scope, event_object.into())
      .unwrap();
  event.dispatch(scope, state, event_object, &target, target_object, target_override)
}
...
impl MessageEvent {
  ...
  fn init_message_event<'a>(
    ...
    #[string] last_event_id: Option<String>,
...
fn report_exception<'a>(
  ...
  let allow_default = if callable {
    ...
    let (file_name, line_number, column_number) = if let Some(frame) = js_error.frames.first() {
      (frame.file_name.clone(), frame.line_number, frame.column_number)
    } else {
      let message = v8::String::empty(scope);
      let exception = v8::Exception::error(scope, message);
      let js_error = JsError::from_v8_exception(scope, exception);
      if let Some(frame) = js_error.frames.iter()
        .find(|frame| frame.file_name.as_ref().is_some_and(|file_name| !file_name.starts_with("ext:")))
      {
        (frame.file_name.clone(), frame.line_number, frame.column_number)
      } else {
        (None, None, None)
    ...
    let event_object = {
      let event = Event::new("error".into(), Some(EventInit { bubbles: false, cancelable: true, composed: false }));
    ...
    let event = cppgc::try_unwrap_cppgc_proto_object::<Event>(scope, event_object.into()).unwrap();
    let global = scope.get_current_context().global(scope);
    let target = cppgc::try_unwrap_cppgc_object::<EventTarget>(scope, global.into()).unwrap();
...
impl EventTarget {
  ...
  fn remove_event_listener<'a>(
    ...
    if let Some((index, listener)) = listeners.iter().enumerate().find(|(_, listener)| {

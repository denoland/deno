...
#[derive(Debug)]
pub struct Event {
  typ: RefCell<v8::Global<v8::String>>,
  bubbles: Cell<bool>,
...
impl Event {
  ...
  fn invoke<'a>(
    ...
    let Some(mut listeners) = listeners.get_mut(&*typ) else {
      return;
    };
...
impl Event {
  ...
  #[cppgc]
  fn new<'a>(
    #[global] typ: v8::Global<v8::String>,
  ...
  fn init_event(
    ...
    self.bubbles.replace(bubbles.unwrap_or(false));
    self.cancelable.replace(cancelable.unwrap_or(false));
  }

  #[getter]
  #[global]
  fn r#type(&self) -> v8::Global<v8::String> {
...
#[op2(fast)]
pub fn op_event_get_dispatched(#[cppgc] event: &Event) -> bool {
  event.dispatch_flag.get()
}
...
pub struct EventTarget {
  listeners: RefCell<HashMap<v8::Global<v8::String>, Vec<Rc<EventListener>>>>,
}
...
impl EventTarget {
  ...
  fn add_event_listener<'a>(
    ...
    let (
      ...
      passive,
      once,
      resist_stop_immediate_propagation, /* signal */
    ) = match options {
      Some(options) => {
        if let Some(options) = options.to_object(scope) {
          #[inline]
          fn to_bool<'a>(
            scope: &mut v8::HandleScope<'a>,
    ...
    let callback = match callback {
      ...
      Some(callback) => {
        ...
        let Some(callback) = callback.to_object(scope) else {
    ...
    let listeners = listeners.entry(typ.clone()).or_insert(Vec::new());
  ...
  fn dispatch_event<'a>(
    ...
    if this == global && *typ == v8::String::new(scope, "unload").unwrap() {

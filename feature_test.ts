...
function runAutoTests() {
  ...
  try {
    const before = win.getOpacity();
    win.setOpacity(0.5);
    const after = win.getOpacity();
    win.setOpacity(1);
    record(
      "opacity",
      before === 1 && Math.abs(after - 0.5) < 0.05,
      `before=${before} after=${after}`,
    );
  } catch (e) {
    record("opacity", false, String(e));
  }
...
// `navigator.clipboard` is the standard Web Clipboard API surface, backed by
// the native system clipboard through the WEF backend. Only the text methods
// (`writeText` / `readText`) are implemented.

win.bind("clipboardWrite", async (text: unknown) => {
  await navigator.clipboard.writeText(String(text));
  return true;
});

win.bind("clipboardRead", async () => {
...
<body>
  ...
  <div class="grid">
    ...
    <div class="card">
      <h2>Clipboard</h2>
      <div class="btn-row">
        <input id="clipboard-input" placeholder="text to copy" value="hello from laufey" style="background:#0d1117;border:1px solid #444;color:#e0e0e0;padding:2px 6px;border-radius:4px;width:160px;font-size:12px"/>
        <button onclick="clipboardWrite()">Write</button>
        <button onclick="clipboardRead()">Read</button>
      </div>
      <div id="clipboard-log" class="log">Write text, then read it back (or paste from another app).</div>
    </div>
...
<script>
  ...
  async function renderAutoResults() {
    try {
      ...
      const propKeys = [
        "windowId", "resizable", "alwaysOnTop", "opacity", "visibility",
  ...
  async function clipboardWrite() {
    ...
    await bindings.clipboardWrite(text);
    document.getElementById("clipboard-log").textContent =
      "writeText(" + JSON.stringify(text) + ") OK";
  }
  async function clipboardRead() {
    const result = await bindings.clipboardRead();
    document.getElementById("clipboard-log").textContent =
      "readText() returned: " + JSON.stringify(result);
  }

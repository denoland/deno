...
pub fn isolate_new() -> CoreIsolate {
  let startup_data = StartupData::Script(Script { source: include_str!("http_bench_json_ops.js"), filename: "http_bench_json_ops.js" });
...
fn op_close(state: &mut CoreIsolateState, args: serde_json::Value, _buf: &mut [ZeroCopyBuf]) -> Result<serde_json::Value, ErrBox> {
  let rid = args.get("rid").unwrap().as_u64().unwrap() as u32;
  debug!("close rid={}", rid);

  let resource_table = &mut state.resource_table.borrow_mut();
  resource_table.close(rid).map(|_| serde_json::json!(())).ok_or_else(bad_resource)
}

fn op_listen(state: &mut CoreIsolateState, _args: serde_json::Value, _buf: &mut [ZeroCopyBuf]) -> Result<serde_json::Value, ErrBox> {
  debug!("listen");
...
fn op_accept(state: &mut CoreIsolateState, args: serde_json::Value, _buf: &mut [ZeroCopyBuf]) -> impl Future<Output = Result<serde_json::Value, ErrBox>> {
  let rid = args.get("rid").unwrap().as_u64().unwrap() as u32;
  ...
  poll_fn(move |cx| {
    ...
    listener.poll_accept(cx)?.map(|(stream, _addr)| {
      let rid = state.resource_table.borrow_mut().add("tcpStream", Box::new(stream));
      Ok(serde_json::json!({ "rid": rid }))
    })
  })
}

fn op_read(state: &mut CoreIsolateState, args: serde_json::Value, bufs: &mut [ZeroCopyBuf]) -> impl Future<Output = Result<serde_json::Value, ErrBox>> {
  ...
  poll_fn(move |cx| -> Poll<Result<serde_json::Value, ErrBox>> {
    ...
    Pin::new(stream).poll_read(cx, &mut buf)?.map(|nread| Ok(serde_json::json!({ "nread": nread })))
  })
}

fn op_write(state: &mut CoreIsolateState, args: serde_json::Value, bufs: &mut [ZeroCopyBuf]) -> impl Future<Output = Result<serde_json::Value, ErrBox>> {
  assert_eq!(bufs.len(), 1, "Invalid number of arguments");
  ...
  poll_fn(move |cx| {
    let resource_table = &mut resource_table.borrow_mut();
    let stream = resource_table.get_mut::<TcpStream>(rid).ok_or_else(bad_resource)?;
    Pin::new(stream).poll_write(cx, &buf)?.map(|nwritten| Ok(serde_json::json!({ "nwritten": nwritten })))
  })
}

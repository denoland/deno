...
pub type OpDispatcher =
  dyn Fn(&mut CoreIsolateState, &[u8], Box<[ZeroCopyBuf]>) -> Op + 'static;
...
impl OpRegistry {
  ...
  where
    F: Fn(&mut CoreIsolateState, &[u8], Box<[ZeroCopyBuf]>) -> Op + 'static,
  {
...
fn test_op_registry() {
  ...
  let res = dispatch(&mut state, &[], Box::new([]));
...
fn register_op_during_call() {
  ...
  let state_rc = CoreIsolate::state(&isolate);
  let mut state = state_rc.borrow_mut();
  let res = dispatcher2(&mut state, &[], Box::new([]));
  if let Op::Sync(buf) = res {
    assert_eq!(buf.len(), 0);
  } else {
    unreachable!();

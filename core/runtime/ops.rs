...
pub fn to_str<'a, const N: usize>(
  ...
  let len_utf8 = string.utf8_length(scope);
  let len_utf16 = string.length();
  ...
  if string.is_onebyte() && len_utf8 == len_utf16 {
    if len_utf16 <= N {
      let length = string.write_one_byte(
        scope,
        // SAFETY: Temp code until we get the new Rusty v8 version of this to_str method
        unsafe {
          (buffer.as_mut_slice() as *mut [_] as *mut [u8])
            .as_mut()
            .unwrap()
        },
        0,
      ...
      // SAFETY: We know we wrote this many valid UTF-8 bytes
      unsafe {
    ...
    // SAFETY: We're allocating a raw buffer but checking the sizes carefully
    unsafe {
      // Create an uninitialized buffer of `capacity` bytes. We need to be careful here to avoid
      // accidentally creating a slice of u8 which would be invalid.
      ...
      let length = string.write_one_byte(
        scope,
        (buffer as *mut [u8]).as_mut().unwrap(),
        0,
  ...
  if len_utf8 <= N {
    // No malloc path
    let length = string.write_utf8_uninit(
      scope,
      buffer,
      None,
      WriteOptions::NO_NULL_TERMINATION | WriteOptions::REPLACE_INVALID_UTF8,

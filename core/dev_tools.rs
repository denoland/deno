...
/// it's a message.
pub type DevtoolsSessionProxySender = UnboundedSender<String>;
pub type DevtoolsSessionProxyReceiver = UnboundedReceiver<Vec<u8>>;
...
/// Polling `DevToolsAgent` allows to accept new incoming
...
pub struct DevToolsSession {
  proxy_tx: DevtoolsSessionProxySender,
  ...
  enabled_domains: Rc<RefCell<HashSet<String>>>,
}

/// This is a "pump" future takes care of receiving messages and dispatching
/// them to the V8 inspector or extension specific handlers. It resolves when receiver closes.
impl Future for DevToolsSession {
  type Output = ();
  fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
    while let Poll::Ready(maybe_data) = self.proxy_rx.poll_next_unpin(cx) {
      if let Some(data) = maybe_data {
        let msg: CdpMessage = serde_json::from_slice(&data).unwrap();
        self.dispatch(msg).unwrap();
      } else {
        return Poll::Ready(());
      }
    }

    // forward all messages coming from v8 inspector
    while let Poll::Ready(Some((_, msg))) = self.v8_proxy_rx.poll_next_unpin(cx)
    {
      // TODO(bartlomieju): this should error out?
      if self.proxy_tx.unbounded_send(msg).is_err() {
        break;
      }
    }
...
impl DevToolsSession {
  ...
  pub fn enable_domain(&self, domain: &str) {
    let mut domains = self.enabled_domains.borrow_mut();
  ...
  fn send(&self, msg: CdpMessage) -> Result<(), AnyError> {
    ...
    let _ = self.proxy_tx.unbounded_send(data);
    Ok(())
  }

  fn dispatch_to_v8(&self, msg: CdpMessage) -> Result<(), AnyError> {
    let data = serde_json::to_vec(&msg)?;
    self.v8_proxy_tx.unbounded_send(data).unwrap();
    Ok(())
  }
}

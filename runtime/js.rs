...
pub static BASE_SNAPSHOT: Lazy<Box<[u8]>> = Lazy::new(
  #[allow(clippy::uninit_vec)]
  #[cold]
  #[inline(never)]
  || {
    static COMPRESSED_BASE_SNAPSHOT: &[u8] =
      include_bytes!(concat!(env!("OUT_DIR"), "/BASE_SNAPSHOT.bin"));

    let size =
      u32::from_le_bytes(COMPRESSED_BASE_SNAPSHOT[0..4].try_into().unwrap())
        as usize;
    let mut vec = Vec::with_capacity(size);
    ...
    lzzzz::lz4::decompress(&COMPRESSED_BASE_SNAPSHOT[4..], &mut vec).unwrap();

    vec.into_boxed_slice()
  },
);

pub fn deno_base_snapshot() -> Snapshot {
  debug!("Deno isolate init with snapshots.");
  Snapshot::Static(&BASE_SNAPSHOT)
}

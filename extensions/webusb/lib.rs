  ...
  WP: WebUsbPermissions + 'static,
{
  ...
  for device in devices.iter() {
    ...
    if device_class == 9
      || permissions
        .check_usb(device_descriptor.product_id())
        .is_err()
    {
      continue;
    }
    ...
    let configuration = config_descriptor
      .map(|config_descriptor| {
        UsbConfiguration::from(config_descriptor, &handle)
      })?
      .ok();
    ...
    for idx in 0..num_configurations {
      let curr_config_descriptor = device.config_descriptor(idx)?;
      configurations

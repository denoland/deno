...
struct WebStorageConnectionResource(sled::Db);
...
pub fn op_webstorage_open(
  ...
  let db = if persistent {
    ...
    sled::open(path.0.join("local_storage"))?
  } else {
    sled::open(std::env::temp_dir())?
  };

  let rid = state.resource_table.add(WebStorageConnectionResource(db));
  Ok(rid)
}
...
pub fn op_webstorage_key(
  ...
  let key = resource
    ...
    .transpose()?
    .map(|(key, _)| String::from_utf8(key.to_vec()))
    .transpose()?;
...
pub fn op_webstorage_set(
  ...
  if resource.0.size_on_disk()? >= 5000000 {
    return Err(
